...
const { setCorsHeaders } = require('../lib/middleware/cors.cjs');
const { resolveUtilsAction } = require("../lib/utils-routing.cjs");
...
const getDbModule = () => require("../lib/db");
const getLiteHostModule = () => require("../lib/litehost-chat");
...
const handleMessagingEvent = async (event) => {
    ...
    try {
        ...
        const { runLiteHostChat } = getLiteHostModule();
...
async function handleExport(req, res) {
  ...
  try {
    ...
    let sessions;
    const { getSavedSessions, getSessionById } = getDbModule();
