...
const { parseRequestPathname, resolveUtilsAction } = require("../../lib/utils-routing.cjs");

describe("utils alias routing", () => {
  it("parses raw request paths without query strings", () => {
    expect(parseRequestPathname("/api/messenger?hub.challenge=123")).toBe("/api/messenger");
  });

  it("routes /api/health to the health action without ?action", () => {
    expect(resolveUtilsAction({
      method: "GET",
      url: "/api/health",
      query: {},
      headers: {},
