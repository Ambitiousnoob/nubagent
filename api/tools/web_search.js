...
const handler = async (args) => {
    ...
    if (requiresGoogle && hasSerper) {
        searches = await Promise.allSettled([
            fetchSerperResults(query, maxResults),
        ]);
    ...
    const promises = [
        fetchDuckDuckGoResults(query, maxResults),
        fetchTavilyResults(query, maxResults),
    ];

    if (hasSerper) {
        promises.push(fetchSerperResults(query, Math.min(maxResults, 10)));
    }

    if (getJinaApiKeys().length) {
        promises.push(fetchJinaResults(query, Math.min(maxResults, 10)));
    }

    if (hasBrave) {
        promises.push(fetchBraveResults(query, Math.min(maxResults, 10)));
    }
