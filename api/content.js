  ...
  if (req.method === "GET" || req.method === "HEAD") {
    sendJson(res, 200, {
      ...
      actions: {
        fetch: {
          method: "POST",
          description: "Simple URL fetch using web_fetch tool",
        },
        read: {
          method: "POST",
          description:
            "Smart content extraction with modes (article, full, outline)",
        },
        crawl: {
          method: "POST",
          description: "Multi-page crawling with depth control",
        },
      },
      example: {
...
async function handleRead(req, res) {
  ...
  try {
    ...
    if (!sourceUrl) {
      res.status(400).end(
        JSON.stringify({
          error: "A valid absolute http(s) url is required.",
        }),
      );
      return;
    }

    const mode = ["article", "full", "outline"].includes(body.mode)
      ? body.mode
      : "article";
    const maxChars = clamp(
      toFiniteNumber(body.maxChars ?? body.maxLength, 3200),
      200,
      MAX_CHARS_LIMIT,
    );
    ...
    if (resource.isHtml) {
      ...
      links = includeLinks
        ? extractLinks(resource.text, finalUrl).slice(0, 20)
        : [];
      ...
      if (mode === "outline") {
        contentSource = [
          ...
          ...headings.map(
            (item) => `${"#".repeat(Math.min(item.level, 3))} ${item.text}`,
          ),
        ]
          .filter(Boolean)
          .join("\n");
        ...
        const readableHtml =
          mode === "full" ? resource.text : extractReadableHtml(resource.text);
    ...
    res.status(200).end(
      JSON.stringify({
        ...
        wordCount: contentSource
          ? contentSource.split(/\s+/).filter(Boolean).length
          : 0,
    ...
    res
      .status(500)
      .end(
        JSON.stringify({ error: error?.message || "Reader request failed." }),
      );
  }
}
...
async function handleCrawl(req, res) {
  ...
  try {
    ...
    const maxPages = clamp(
      toFiniteNumber(body.maxPages, 6),
      1,
      MAX_PAGES_LIMIT,
    );
    const maxDepth = clamp(
      toFiniteNumber(body.maxDepth, 1),
      0,
      MAX_DEPTH_LIMIT,
    );
    const maxCharsPerPage = clamp(
      toFiniteNumber(body.maxCharsPerPage, 1800),
      200,
      MAX_CHARS_PER_PAGE_LIMIT,
    );
    const timeoutMs = clamp(
      toFiniteNumber(body.timeoutMs, DEFAULT_TIMEOUT_MS),
      2000,
      30000,
    );
    ...
    while (queue.length && pages.length < maxPages) {
      ...
      try {
        ...
        const title = page.isHtml
          ? extractTitle(page.text) || resolvedUrl
          : page.titleHint || resolvedUrl.split("/").pop() || resolvedUrl;
        ...
        if (current.depth < maxDepth) {
          for (const link of links) {
            ...
            if (
              ...
              !matchesPatterns(link, includePatterns)
            )
              continue;
            if (
              excludePatterns.length &&
              matchesPatterns(link, excludePatterns)
            )
              continue;
    ...
    res
      .status(500)
      .end(
        JSON.stringify({ error: error?.message || "Crawler request failed." }),
      );
  }
}
