...
module.exports = async (req, res) => {
  ...
  if (scope.scope !== "api_key" || !scope.stateKey) {
    sendJson(res, 400, {
      error:
        "Send X-API-Key or Authorization: Bearer <key> to access API-key memory.",
    });
    return;
  }
    ...
    try {
      ...
      sendJson(res, 200, {
        ...
        search: {
          ...
          helper_model:
            process.env.CEREBRAS_MEMORY_MODEL ||
      ...
      sendJson(res, 503, {
        error: error?.message || "Memory storage unavailable.",
      });
    }
    return;
  }
  ...
  try {
    if (action === "insert") {
      ...
      const assistantReply = String(
        body.assistant_reply || body.assistantReply || "",
      ).trim();
      const inserted = await saveApiKeyMemoryEntries(
        scope.stateKey,
        entries,
        assistantReply,
      );
    ...
    if (action === "search") {
      ...
      sendJson(res, 200, {
        ...
        context: body.include_context
          ? formatApiKeyMemoryContext(search.results)
          : undefined,
      });
      return;
    }
