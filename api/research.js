...
const writeCorsHeaders = (res) => {
  ...
  res.setHeader(
    ...
    "Content-Type, Authorization, X-State-Key, X-API-Key",
  );
};
...
const normalizeScopeKey = (req, body = {}) => {
  ...
  const explicit = normalizeStateKey(
    body?.stateKey || body?.scopeKey || req.query?.stateKey,
  );
  if (explicit) return explicit;
  return buildEphemeralScopeKey();
};

const normalizeAction = (req, body = {}) =>
  normalizeText(
    body?.action ||
      req.query?.action ||
      req.query?.mode ||
...
const wantsStream = (req, body = {}) => {
  const responseType = normalizeText(
    body?.responseType || body?.format || req.query?.responseType,
  ).toLowerCase();
...
const normalizeAttachments = (attachments = []) => {
  return (Array.isArray(attachments) ? attachments : []).map((attachment) => ({
    ...
    dataUrl:
      attachment?.kind === "image" ? String(attachment?.dataUrl || "") : "",
    textContent:
      attachment?.kind === "text" ? String(attachment?.textContent || "") : "",
...
const normalizeForcedOutputMode = (value) =>
...
const normalizeResearchModelChain = (value) => {
  const entries = Array.isArray(value)
    ...
    : typeof value === "string"
      ? value.split(",")
      : [];
  const normalized = entries.map((item) => normalizeText(item)).filter(Boolean);
...
const normalizeResearchRoundRobin = (value) => {
  ...
  const normalized = normalizeText(value).toLowerCase();
  if (["true", "1", "yes", "round_robin", "round-robin"].includes(normalized))
    return true;
...
const buildRuntimeRequestOptions = (req, body = {}) => ({
  depthPreference: normalizeDepthPreference(
    body?.depthPreference ||
      body?.depth ||
      req.query?.depthPreference ||
      req.query?.depth,
  ),
  forcedOutputMode: normalizeForcedOutputMode(
    body?.forcedOutputMode ||
      body?.outputMode ||
      req.query?.forcedOutputMode ||
      req.query?.outputMode,
  ),
  refinementBudget: normalizeRefinementBudget(
    body?.refinementBudget || req.query?.refinementBudget,
  ),
  ...
  researchProvider: normalizeResearchProvider(
    body?.researchProvider ||
      body?.provider ||
      req.query?.researchProvider ||
      req.query?.provider,
  ),
  researchModel: normalizeResearchModel(
    body?.researchModel ||
      body?.model ||
      req.query?.researchModel ||
      req.query?.model,
  ),
  researchModelChain: normalizeResearchModelChain(
    body?.researchModelChain ||
      body?.modelChain ||
      req.query?.researchModelChain ||
      req.query?.modelChain,
  ),
  researchRoundRobin: normalizeResearchRoundRobin(
    getFirstDefinedValue(
      body?.researchRoundRobin,
      body?.roundRobin,
      req.query?.researchRoundRobin,
      req.query?.roundRobin,
    ),
  ),
  researchProviderKeys: normalizeResearchProviderKeys(
    body?.researchProviderKeys ||
...
const metadataPayload = (scopeKey = "") => ({
  ...
  actions: {
    run: {
      method: "POST",
      stream: true,
      description: "Run the compiled research DAG.",
    },
    resume: {
      method: "POST",
      stream: true,
      description: "Resume a stored research run from the last checkpoint.",
    },
    control: {
      method: "POST",
      stream: false,
      description: "Queue a steering control for an active run.",
    },
    export: {
      method: "POST",
      stream: false,
      description: "Export a stored research run.",
    },
    get: {
      method: "GET|POST",
      stream: false,
      description: "Fetch a stored research run by id.",
    },
    list: {
      method: "GET",
      stream: false,
      description: "List research runs for the active scope.",
    },
    memory_search: {
      method: "GET|POST",
      stream: false,
      description: "Search the hierarchical research memory.",
    },
    memory_insert: {
      method: "POST",
      stream: false,
      description: "Insert custom research memory entries.",
    },
  },
  controls: [
...
module.exports = async (req, res) => {
  ...
  if (req.method === "GET" || req.method === "HEAD") {
    try {
      ...
      if (action === "list") {
        const runs = await listResearchRuns(scopeKey, {
      ...
      sendJson(res, 500, {
        error: error?.message || "Research metadata request failed.",
      });
    }
    return;
  }
  ...
  try {
    ...
    const runRequest = async (onEvent = null) => {
      ...
      if (action === "resume") {
        const runId = normalizeText(body?.runId || body?.id);
        if (!runId) {
          throw new ResearchRuntimeError("Provide runId or id.", {
      ...
      if (!query && !attachments.length) {
        throw new ResearchRuntimeError("Provide a query or attachments.", {
          failureType: "critical",
        });
      }
    ...
    if (stream) {
      ...
      try {
        ...
        const normalized =
          error instanceof ResearchRuntimeError
            ? error
            : new ResearchRuntimeError(
                error?.message || "Research request failed.",
              );
        writeSseEvent(res, {
          type: "error",
