...
const buildLiteHostModule = ({
  delegateResearchHandler = vi.fn(async () =>
  ...
  module: loadCommonJsModule(
    "/root/.bot/.downloads/nubagent/lib/litehost-chat.js",
    {
      ...
      "./tools/web_fetch": {
        definition: {
          function: { name: "web_fetch", description: "", parameters: {} },
        },
        handler: vi.fn(async () => ""),
      },
      "./tools/search_images": {
        definition: {
          function: { name: "search_images", description: "", parameters: {} },
        },
        handler: vi.fn(async () => "[]"),
      },
      "./tools/view_image": {
        definition: {
          function: { name: "view_image", description: "", parameters: {} },
        },
        handler: vi.fn(async () => ""),
      },
      "./tools/delegate_research": {
        definition: {
          ...
          function: {
            ...
            description:
              "Delegate grounded research to the orchestration subagents",
            parameters: {
              type: "object",
              properties: {
                query: { type: "string" },
...
describe("litehost chat provider routing", () => {
  ...
  it("defaults research OpenRouter requests to Nemotron when no explicit model is provided", () => {
    ...
    const normalized = normalizeChatBody({
      ...
      messages: [
        {
          role: "user",
          content: "How should the verifier swarm route models?",
        },
      ],
    });
  ...
  it("lets the main model call delegate_research and continue from the delegated result", async () => {
    const delegateResearchHandler = vi.fn(async (args, context) =>
      JSON.stringify({
        delegated: true,
    ...
    const result = await runLiteHostChat({
      ...
      use_tools: true,
      messages: [
        { role: "user", content: "Do the full research and then answer." },
      ],
      delegationContext: {
    ...
    expect(result.reply.content).toBe(
      "Final answer based on delegated research.",
    );
    ...
    expect(result.reply.toolsUsed).toEqual(
    ...
    expect(
      liteHost.createChatResponsePayload({ use_tools: true }, result.reply),
    ).toMatchObject({
  ...
  it("fails the main chat turn when delegated research does not complete", async () => {
    const delegateResearchHandler = vi.fn(async () => {
      throw new Error(
        "delegated research failed (delegated research did not reach a complete state)",
      );
    });
    ...
    ).rejects.toThrow(/delegated research failed/i);
  });

  it("fails the main chat turn when delegated research returns an error tool payload", async () => {
    const delegateResearchHandler = vi.fn(
      async () =>
        "Error: delegated research failed (delegated research did not reach a complete state)",
    );
