...
const extractMetaContent = (html = "", names = []) => {
  ...
  while ((match = metaPattern.exec(html)) !== null) {
    ...
    const key = (nameMatch?.[1] || nameMatch?.[2] || nameMatch?.[3] || "")
      .trim()
      .toLowerCase();
...
const extractPublishedTime = (html = "") => {
  const metaValue = extractMetaContent(html, [
...
const stripHtml = (html = "") =>
  decodeEntities(
    html
      ...
      .replace(/<br\s*\/?>/gi, "\n")
      .replace(
        /<\/(p|div|section|article|main|li|ul|ol|table|tr|td|th|h1|h2|h3|h4|h5|h6|pre|blockquote)>/gi,
        "\n",
      )
      .replace(/<[^>]+>/g, " ")
      .replace(/[ \t]+\n/g, "\n")
...
const extractHeadings = (html = "", max = 14) => {
  ...
  while (
    (match = headingPattern.exec(html)) !== null &&
    headings.length < max
  ) {
...
const extractReadableHtml = (html = "") => {
  ...
  const sectionPattern =
    /<(section|div)\b[^>]*(?:id|class)\s*=\s*(?:"([^"]*)"|'([^']*)')[^>]*>([\s\S]*?)<\/\1>/gi;
  for (const match of body.matchAll(sectionPattern)) {
    const attrs = `${match[2] || ""} ${match[3] || ""}`.toLowerCase();
    if (
      /(content|article|main|post|doc|docs|markdown|entry|page|body|guide)/.test(
        attrs,
      )
    ) {
...
const parseFetchToolPayload = (payload = "") => {
  const raw = String(payload || "");
  const metadataBlock = raw.match(/^(?:<!--[\s\S]*?-->\s*)+/)?.[0] || "";
  const metadata = {};

  for (const match of metadataBlock.matchAll(
    /<!--\s*([^:]+):\s*([\s\S]*?)\s*-->/g,
  )) {
  ...
  return {
    finalUrl: normalizeUrl(metadata.source || "") || "",
    title: metadata.title || "",
    description: metadata.description || "",
    publishedTime: metadata.published || "",
    via: metadata["fetched via"] || "",
    content: stripFetchMeta(raw),
  };
};

const isTextLikeContent = (contentType = "") =>
  !contentType ||
  /(text\/|application\/(json|xml|xhtml\+xml|javascript|ld\+json))/i.test(
    contentType,
  );
...
const assertPublicHttpUrl = async (urlString) => {
  ...
  if (net.isIP(parsed.hostname)) {
    if (isPrivateIp(parsed.hostname))
      throw new Error("Private or local IP addresses are blocked.");
    return normalized;
  }
...
const parseReaderProxyText = (text = "") => {
  ...
  const sourceUrl =
    normalizeUrl(
      normalizedText.match(/^URL Source:\s*(.+)$/m)?.[1]?.trim() || "",
    ) || "";
  const publishedTime =
    normalizedText.match(/^Published Time:\s*(.+)$/m)?.[1]?.trim() || "";
  ...
  return {
    ...
    sourceUrl,
    publishedTime,
    content: content.replace(/^Warning:.*$/gim, "").trim() || normalizedText,
  };
};

const fetchDirectTextResource = async (
  normalized,
  timeoutMs = 12000,
  redirectCount = 0,
) => {
  ...
  try {
    const response = await fetch(normalized, {
      ...
      headers: {
        accept:
          "text/html,application/xhtml+xml,text/plain;q=0.9,application/json;q=0.9,application/xml;q=0.8,*/*;q=0.5",
      },
    });
    ...
    if (response.status >= 300 && response.status < 400) {
      ...
      const redirectUrl = await assertPublicHttpUrl(
        new URL(location, normalized).toString(),
      );
    ...
    return {
      ...
      isHtml,
      via: "direct",
      titleHint: isHtml ? extractTitle(text) : "",
      description: isHtml ? extractMetaDescription(text) : "",
      canonicalUrl: isHtml ? extractCanonical(text, normalized) || "" : "",
      publishedTime: isHtml ? extractPublishedTime(text) : "",
      readableText: isHtml ? stripHtml(extractReadableHtml(text)) : text,
    };
  } finally {
...
const fetchReaderProxyResource = async (normalized, timeoutMs = 12000) => {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), timeoutMs);
  const proxyUrl = `${READER_PROXY_ORIGIN}/${normalized}`;
...
const fetchTextResource = async (url, timeoutMs = 12000) => {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `${directError?.message || "fetch failed"}; fallback reader failed: ${fallbackError?.message || "unknown error"}`,
      );
    }
  }
};
