...
const { getApiKeysFromEnv, getRotatingApiKey, getRotatingValue } = require("./api-key-rotation.cjs");
...
const { definition: delegateResearchDef, handler: delegateResearchHandler } = require("./tools/delegate_research");
...
const OPENROUTER_REFERER = process.env.OPENROUTER_HTTP_REFERER
...
const TOOL_DEFINITIONS = [calcDef, searchDef, fetchDef, imageSearchDef, viewImageDef, delegateResearchDef];
...
const normalizeApiKeyList = (value = []) => unique(
...
const isImageDataUrl = (value) => /^data:image\/[^;,]+;base64,[a-z0-9+/=\s]+$/i.test(normalizeText(value));
...
const normalizeInlineDataPart = (inlineData) => {
    ...
    if (!mimeType || !data || !mimeType.toLowerCase().startsWith("image/")) return null;
    return {
        type: "image_inline",
...
const injectBrandSystemMessage = (messages = [], options = {}) => {
    ...
    const researchMode = options.research_mode === true || options.research_synthesis === true;
    if (researchMode) {
    ...
    if (name === "delegate_research") {
        ...
        try {
            ...
            if (parsed && typeof parsed === "object" && parsed.delegated === true) {
                const outputMode = parsed.outputMode && typeof parsed.outputMode === "object"
                    ? {
                ...
                delegatedResearch = {
                    ...
                    heading: normalizeText(parsed.heading || "Research Answer") || "Research Answer",
                    ...
                    sourceCount: Number(parsed.sourceCount || sources.length || 0) || sources.length,
                    sources,
...
const normalizeModel = (model) => {
    ...
    if (lowered.startsWith("openrouter:")) return id.slice(id.indexOf(":") + 1).trim() || DEFAULT_OPENROUTER_MODEL;
...
const isGeminiModel = (modelId = "") => normalizeText(modelId).toLowerCase().startsWith("gemini-");
...
const normalizeProvider = (provider, model) => {
    ...
    if (requestedProvider === GEMINI_PROVIDER || requestedProvider === "google") return GEMINI_PROVIDER;
...
const getGeminiApiKeys = (body = {}) => unique([
...
const getGeminiApiKey = (body = {}) => {
    ...
    if (requestKeys.length) return getRotatingValue("gemini-request", requestKeys);
    return getRotatingApiKey("gemini", "GEMINI_API_KEYS", "GEMINI_API_KEY");
};
const getOpenRouterApiKeys = (body = {}) => unique([
...
const getOpenRouterApiKey = (body = {}) => {
    ...
    if (requestKeys.length) return getRotatingValue("openrouter-request", requestKeys);
    return getRotatingApiKey("openrouter", "OPENROUTER_API_KEYS", "OPENROUTER_API_KEY");
};
...
const getGeminiFallbackModelChain = (requestedModel, configuredChain = []) => {
    ...
    return [...new Set([normalizeModel(requestedModel), ...configured].filter((item) => isGeminiModel(item)))];
};

const getOpenRouterFallbackModelChain = (requestedModel, configuredChain = []) => {
    const configured = [
        ...normalizeApiKeyList(configuredChain),
        ...normalizeApiKeyList(process.env.OPENROUTER_FALLBACK_MODELS || process.env.OPENROUTER_MODEL_CHAIN),
    ]
        .map((item) => normalizeModel(item))
        .filter((item) => item && !isGeminiModel(item));
    return [...new Set([normalizeModel(requestedModel), ...configured].filter(Boolean))];
};
...
const runGeminiWithModel = async (body, modelId, streamCallback) => {
    ...
    for (let turn = 0; turn < MAX_TOOL_TURNS; turn += 1) {
        ...
        for (const call of functionCalls) {
            ...
            if (toolResult.delegatedResearch) delegatedResearch = toolResult.delegatedResearch;
            toolsUsed.push({
                name: call.name,
...
const runGeminiChat = async (body, streamCallback) => {
    const modelChain = getGeminiFallbackModelChain(body.model, body.modelChain || body.model_chain);
    let lastError = null;
...
const consumeOpenRouterStream = async (response, streamCallback) => {
    ...
    if (!reader) {
        throw createProviderError(502, "OpenRouter streaming response was unavailable.");
    }
...
const runOpenRouterWithModel = async (body, modelId, streamCallback) => {
    ...
    const basePayload = {
        ...
        ...(typeof body.temperature === "number" ? { temperature: body.temperature } : {}),
        ...(body.use_tools !== false ? { tools: TOOL_DEFINITIONS, tool_choice: "auto" } : {}),
    };
    ...
    for (let turn = 0; turn < MAX_TOOL_TURNS; turn += 1) {
        ...
        for (const call of toolCalls) {
            ...
            messages.push({
                role: "tool",
                tool_call_id: call?.id || `${call?.function?.name || call?.name || "tool"}-${turn}`,
...
const runOpenRouterChat = async (body, streamCallback) => {
    const modelChain = getOpenRouterFallbackModelChain(body.model, body.modelChain || body.model_chain);
    let lastError = null;
...
const normalizeChatBody = (body = {}) => {
    ...
    const requestedModel = normalizeText(body.model)
        || (normalizeText(body.provider).toLowerCase() === OPENROUTER_PROVIDER ? DEFAULT_OPENROUTER_MODEL : DEFAULT_MODEL);
    const normalizedProvider = normalizeProvider(body.provider, normalizeModel(requestedModel));
    const normalizedProviderApiKeys = normalizeProviderApiKeys(body.providerApiKeys || body.provider_api_keys);
    const hasCredentials = normalizedProvider === OPENROUTER_PROVIDER
        ? getOpenRouterApiKeys({ providerApiKeys: normalizedProviderApiKeys }).length > 0
        : getGeminiApiKeys({ providerApiKeys: normalizedProviderApiKeys }).length > 0;
    if (!hasCredentials) {
    ...
    const normalizedMessages = normalizeMessagesForModel(
        injectBrandSystemMessage(resolvedMessages, {
            research_mode: body.research_mode === true || body.research_synthesis === true,
        }),
    );
...
const createChatResponsePayload = (body, reply) => {
    const delegatedResearch = reply?.delegatedResearch && typeof reply.delegatedResearch === "object"
    ...
    return {
        ...
        ...(delegatedResearch
            ? {
                sources: Array.isArray(delegatedResearch.sources) ? delegatedResearch.sources : [],
                researchMeta,
