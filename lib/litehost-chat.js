...
const metadataPayload = () => ({
    ...
    memory: {
        ...
        behavior: "Scoped memory is stored server-side. Thin requests can send only the latest turn and the backend will search the key-scoped memory table for related context before answering.",
    },
});
...
const normalizeChatBody = (body = {}) => {
    ...
    if (!body.messages || !Array.isArray(body.messages) || body.messages.length === 0) {
        throw createHttpError(400, "Provide a non-empty messages array.");
    }

    if (!process.env.GEMINI_API_KEYS) {
        throw createHttpError(503, "GEMINI_API_KEYS is not configured on the server.");
    }
