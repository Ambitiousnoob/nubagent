...
const resolveIncomingMessages = (body = {}) => {
    ...
    return buildMessagesFromSimpleBody(body);
};
...
const fetchRemoteImageInlineData = async (url) => {
    ...
    if (!mimeType.startsWith("image/")) {
        throw new Error(`Remote resource is not an image (${mimeType || "unknown content-type"})`);
    }

    const contentLength = Number(response.headers.get("content-length") || 0);
    if (contentLength && contentLength > MAX_INLINE_IMAGE_BYTES) {
        throw new Error(`Remote image exceeds ${MAX_INLINE_IMAGE_BYTES} bytes`);
    }
...
const mapOpenAiContentToGeminiParts = async (content) => {
    ...
    for (const part of content) {
        ...
        if (part.type === "image_url" && typeof part.image_url?.url === "string") {
            ...
            try {
                ...
                parts.push({ text: `[Remote image URL could not be inlined: ${part.image_url.url} (${error.message})]` });
            }
            continue;
        }
...
const normalizeChatBody = (body = {}) => {
    ...
    return {
        ...
        messages: normalizeMessagesForModel(injectBrandSystemMessage(resolvedMessages)),
    };
};
