...
const dedupeConsecutiveMessages = (messages = []) => {
  ...
  for (const message of messages) {
    if (!message) continue;
    if (output.length && sameMessage(output[output.length - 1], message))
      continue;
    output.push(message);
  }
  return output;
...
const shouldUseScopedChatMemory = (body = {}) => {
  if (!body || body.use_persistent_memory === false || body.memory === false)
    return false;
...
const buildMessagesWithScopedMemory = (
  messages = [],
  memoryState = {},
  { retrievedContext = "" } = {},
) => {
  const original = Array.isArray(messages) ? messages : [];
  const systemMessages = original.filter(
    (message) => message?.role === "system",
  );
  const nonSystemMessages = original.filter(
    (message) => message?.role !== "system",
  );
  const memory = normalizeChatMemory(memoryState);
  const compactRetrievedContext = normalizeText(retrievedContext);
  const useSummary = memory.summary && !compactRetrievedContext;

  const summaryMessage = useSummary
    ? [
        {
          role: "system",
    ...
    : [];
  const retrievedMessage = compactRetrievedContext
    ? [
        {
          role: "system",
          content: compactRetrievedContext,
        },
      ]
    : [];
