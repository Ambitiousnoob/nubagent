...
const listConfiguredSearchProviders = (searchProviderKeys = {}) => unique([
...
const hasConfiguredResearchProvider = (provider = "", researchProviderKeys = {}) => {
    ...
    if (normalizedProvider === "google" || normalizedProvider === "gemini") {
        return Boolean(
            normalizeText(researchProviderKeys?.google || researchProviderKeys?.gemini)
...
const resolveDefaultResearchProvider = (researchProviderKeys = {}) => {
    if (hasConfiguredResearchProvider("openrouter", researchProviderKeys)) return "openrouter";
    if (hasConfiguredResearchProvider("google", researchProviderKeys)) return "google";
...
const normalizeResearchProvider = (value, fallback = DEFAULT_RESEARCH_PROVIDER) => {
...
const normalizeResearchModel = (value, fallback = DEFAULT_RESEARCH_MODEL) => {
    ...
    if (raw.includes("/") || lowered.startsWith("gemini-") || lowered.includes(":")) return raw;
    return fallback;
};
const normalizeResearchModelChain = (value, fallbackModel = DEFAULT_RESEARCH_MODEL) => {
    const entries = Array.isArray(value)
        ? value
        : typeof value === "string"
            ? value.split(",")
            : [];
    const normalized = unique(entries.map((item) => normalizeResearchModel(item, "")).filter(Boolean));
    return normalized.length ? normalized : [normalizeResearchModel(fallbackModel, DEFAULT_RESEARCH_MODEL)];
};
const getConfiguredResearchModelChain = (fallbackModel = DEFAULT_RESEARCH_MODEL) => normalizeResearchModelChain(
...
const inferResearchProviderFromModel = (model = "", fallback = DEFAULT_RESEARCH_PROVIDER) => (
    normalizeResearchModel(model, DEFAULT_RESEARCH_MODEL).toLowerCase().startsWith("gemini-")
        ? "google"
        : fallback
);
...
const normalizeResearchProviderKeys = (value = {}) => {
    const candidate = value && typeof value === "object" ? value : {};
    const normalized = {
        openrouter: normalizeText(candidate?.openrouter || candidate?.openRouter || candidate?.["open-router"]),
...
const listConfiguredResearchProviders = (researchProviderKeys = {}) => unique([
    ...Object.entries(RESEARCH_MODEL_PROVIDER_LABELS)
        .filter(([provider]) => normalizeText(researchProviderKeys?.[provider]))
        .map(([, label]) => label),
    process.env.OPENROUTER_API_KEY || process.env.OPENROUTER_API_KEYS ? "OpenRouter" : "",
...
const selectResearchModelForContext = (context, subagent = "") => {
    ...
    const roundRobinEnabled = context.researchRoundRobin === true && chain.length > 1;
    const modelIndex = roundRobinEnabled ? cursor % chain.length : 0;
    const model = chain[modelIndex] || chain[0] || DEFAULT_RESEARCH_MODEL;
    const provider = inferResearchProviderFromModel(model, context.requestedResearchProvider || DEFAULT_RESEARCH_PROVIDER);
    const providerKeyCount = provider === "google"
    ...
    context.state.researchModelSelection = {
        ...
        callCount: cursor + 1,
        configuredProviders: Array.isArray(context.availableModelProviders) ? context.availableModelProviders : [],
    };
...
const selectDeliverySources = (sources = [], citedNumbers = [], limit = 8) => {
    ...
    const citedSources = rankedSources.filter((source) => citedNumbers.includes(Number(source.citationIndex)));
    if (citedSources.length) return citedSources;

    const preferredFallback = rankedSources.filter((source) => ["core", "supporting"].includes(normalizeText(source?.tier).toLowerCase()));
    return (preferredFallback.length ? preferredFallback : rankedSources).slice(0, Math.max(1, Number(limit) || 8));
};
...
const buildNoGroundedSourcesReport = ({
    ...
    const sourceCount = Array.isArray(sourceMesh?.sources) ? sourceMesh.sources.length : 0;
    const tieredCount = Array.isArray(tieredSources) ? tieredSources.length : 0;
    const evidenceCount = Array.isArray(evidenceEntries) ? evidenceEntries.length : 0;
    const providerErrors = unique([
        ...((Array.isArray(sourceMesh?.providerErrors) ? sourceMesh.providerErrors : []).map((item) => normalizeText(item))),
    ...
    const heading = sourceCount || tieredCount ? "No Grounded Evidence Retrieved" : "No Sources Retrieved";
    const details = [];

    if (sourceCount) {
        details.push(`${sourceCount} source candidate${sourceCount === 1 ? "" : "s"} were collected, but only ${tieredCount} survived filtering and ${evidenceCount} readable page${evidenceCount === 1 ? "" : "s"} were fetched.`);
    } else {
    ...
    details.push("DuckDuckGo HTML fallback can be blocked by anti-bot challenges, so preview deployments should not rely on it as the only search path.");

    return [
        `# ${heading}`,
        "",
        normalizedQuery ? `I could not retrieve grounded sources for: ${normalizedQuery}` : "I could not retrieve grounded sources for this run.",
        "",
        details.join(" "),
        "",
        "## Next steps",
...
const buildPlanBrief = (plan = {}, memoryContextString = "") => {
    ...
    const equippedSubagents = (Array.isArray(plan.subagents) ? plan.subagents : [])
        .filter((entry) => entry?.label && Array.isArray(entry?.equipment) && entry.equipment.length)
        .map((entry) => `- ${entry.label}: ${entry.equipment.slice(0, 3).join(", ")}`);
...
const buildRuntimeMeta = ({ run = {}, plan = null, state = null } = {}) => {
    ...
    return {
        ...
        configuredProviders: Array.isArray(run.requestOptions?.searchProviders) ? run.requestOptions.searchProviders : [],
        researchProvider: run.requestOptions?.researchProvider || null,
        researchModel: run.requestOptions?.researchModel || null,
        researchModelChain: Array.isArray(run.requestOptions?.researchModelChain) ? run.requestOptions.researchModelChain : [],
        configuredModelProviders: Array.isArray(run.requestOptions?.modelProviders) ? run.requestOptions.modelProviders : [],
        activeResearchModel: resolvedState.researchModelSelection || null,
        providersUsed: Array.isArray(sourceMesh.providersUsed) ? sourceMesh.providersUsed : [],
        providerErrors: Array.isArray(sourceMesh.providerErrors) ? sourceMesh.providerErrors.slice(0, 6) : [],
...
const recompilePlanWithControls = (context, appliedControls = []) => {
    ...
    context.run.requestOptions = {
        ...
        refinementBudget: context.requestedRefinementBudget || context.plan?.refinementBudget || 3,
        maxQueries: context.requestedMaxQueries || context.run.requestOptions?.maxQueries || context.plan?.searchQueries?.length || 6,
        searchProviders: Array.isArray(context.availableSearchProviders) ? context.availableSearchProviders : [],
        researchProvider: context.requestedResearchProvider || DEFAULT_RESEARCH_PROVIDER,
        researchModel: context.requestedResearchModel || DEFAULT_RESEARCH_MODEL,
        researchModelChain: Array.isArray(context.researchModelChain) ? context.researchModelChain : [DEFAULT_RESEARCH_MODEL],
        researchRoundRobin: context.researchRoundRobin === true,
        modelProviders: Array.isArray(context.availableModelProviders) ? context.availableModelProviders : [],
    };
...
const buildRuntimeContext = async ({
    ...
    const normalizedSearchProviderKeys = normalizeSearchProviderKeys(searchProviderKeys);
    const normalizedResearchProviderKeys = normalizeResearchProviderKeys(researchProviderKeys);
    const availableSearchProviders = unique([
        ...listConfiguredSearchProviders(normalizedSearchProviderKeys),
        ...(Array.isArray(resumeRun?.requestOptions?.searchProviders) ? resumeRun.requestOptions.searchProviders : []),
    ]);
    const defaultResearchProvider = resolveDefaultResearchProvider(normalizedResearchProviderKeys);
    const defaultResearchModel = resolveDefaultResearchModel(
        resumeRun?.requestOptions?.researchProvider || researchProvider || defaultResearchProvider,
    );
    const requestedResearchModelChain = normalizeResearchModelChain(
        Array.isArray(researchModelChain) && researchModelChain.length
            ? researchModelChain
            : researchModelChain || resumeRun?.requestOptions?.researchModelChain || getConfiguredResearchModelChain(defaultResearchModel),
        researchModel || resumeRun?.requestOptions?.researchModel || defaultResearchModel,
    );
    const requestedResearchModel = normalizeResearchModel(
        researchModel || resumeRun?.requestOptions?.researchModel || requestedResearchModelChain[0] || defaultResearchModel,
        defaultResearchModel,
    );
    const requestedResearchProvider = normalizeResearchProvider(
        researchProvider || resumeRun?.requestOptions?.researchProvider || inferResearchProviderFromModel(requestedResearchModel, defaultResearchProvider),
        inferResearchProviderFromModel(requestedResearchModel, defaultResearchProvider),
    );
    ...
    const availableModelProviders = unique([
        ...
        ...(Array.isArray(resumeRun?.requestOptions?.modelProviders) ? resumeRun.requestOptions.modelProviders : []),
    ]);
...
const executeNode = async (context, node) => {
    ...
    case "adaptiveDeliveryHub": {
        const tieredSources = Array.isArray(context.state.tieredSources) ? context.state.tieredSources : [];
        const evidenceEntries = Array.isArray(context.state.evidenceEntries) ? context.state.evidenceEntries : [];
        const hasGroundedEvidence = tieredSources.length > 0 && evidenceEntries.length > 0;
        ...
        const hasCitedSources = tieredSources.some((source) => citedNumbers.includes(Number(source.citationIndex)));
        const resultSources = hasGroundedEvidence ? selectDeliverySources(tieredSources, citedNumbers) : [];
        const sourceSelectionMode = hasGroundedEvidence
            ? (hasCitedSources ? "cited" : "fallback")
            : "no_grounded_sources";
...
const executeResearchRuntimePass = async (context, resumeStartNode = "") => {
    ...
    for (const batch of batches) {
        ...
        for (const node of batch) {
            const alreadyCompleted = (context.run.completedNodes || []).includes(node.id);
            if (!shouldExecute) {
