...
const DECISION_OUTPUT_MODES = new Set([
  "decision_brief",
...
const VERIFIER_SPECS = Object.freeze({
  claim_support: {
    ...
    threshold: 0.8,
    system:
      "You are ClaimVerifier inside Research Framework v3.1. Return strict JSON with keys score, issues, and rewrite_brief. Check every substantive claim in the draft against the supplied evidence and identify unsupported, overreaching, or hallucinated claims.",
  },
  citation_integrity: {
    label: "CitationVerifier",
    threshold: 0.8,
    system:
      "You are CitationVerifier inside Research Framework v3.1. Return strict JSON with keys score, issues, and rewrite_brief. Check whether each bracketed citation is used faithfully, whether cited sources are Core or Supporting, and whether claims drift beyond the cited evidence.",
  },
  contradiction_handling: {
    label: "ContradictionVerifier",
    threshold: 0.76,
    system:
      "You are ContradictionVerifier inside Research Framework v3.1. Return strict JSON with keys score, issues, and rewrite_brief. Check whether the draft addresses counter-hypotheses, disconfirming evidence, and disagreement instead of implying false consensus.",
  },
  uncertainty_calibration: {
    label: "UncertaintyVerifier",
    threshold: 0.78,
    system:
      "You are UncertaintyVerifier inside Research Framework v3.1. Return strict JSON with keys score, issues, and rewrite_brief. Check whether confidence, residual uncertainty, caveats, and limits are calibrated to the available evidence.",
  },
  task_focus: {
    label: "TaskFocusVerifier",
    threshold: 0.62,
    system:
      "You are TaskFocusVerifier inside Research Framework v3.1. Return strict JSON with keys score, issues, and rewrite_brief. Check whether the draft stays centered on the original user question, preserves the declared scope, and avoids drifting into adjacent topics, generic filler, or mode-inappropriate advice.",
  },
});
...
const TASK_FOCUS_STOP_TERMS = new Set([
  "adopt",
  "agent",
  "analysis",
  "answer",
...
const RESEARCH_MODEL_PROVIDER_LABELS = Object.freeze({
  openrouter: "OpenRouter",
  google: "Google",
});
const DEFAULT_RESEARCH_PROVIDER = "openrouter";
const DEFAULT_RESEARCH_MODEL = "nvidia/nemotron-3-super-120b-a12b:free";
const DEFAULT_GEMINI_RESEARCH_MODEL = "gemini-2.5-flash-lite";
const RESEARCH_MODEL_ALIASES = Object.freeze({
  "nub-agent": DEFAULT_RESEARCH_MODEL,
  "nub agent": DEFAULT_RESEARCH_MODEL,
  nemotron: DEFAULT_RESEARCH_MODEL,
  "nemotron-free": DEFAULT_RESEARCH_MODEL,
  "research-nemotron": DEFAULT_RESEARCH_MODEL,
  "openrouter-nemotron": DEFAULT_RESEARCH_MODEL,
  "openrouter-nemotron-free": DEFAULT_RESEARCH_MODEL,
  "nvidia/nemotron-3-super-120b-a12b": DEFAULT_RESEARCH_MODEL,
  "nvidia/nemotron-3-super-120b-a12b:free": DEFAULT_RESEARCH_MODEL,
});
...
const normalizeSearchProviderKeys = (value = {}) => {
  ...
  return Object.fromEntries(
    Object.keys(SEARCH_PROVIDER_LABELS)
      .map((provider) => [provider, normalizeText(candidate?.[provider])])
      .filter(([, apiKey]) => apiKey),
  );
};
const listConfiguredSearchProviders = (searchProviderKeys = {}) =>
  unique([
    ...Object.entries(SEARCH_PROVIDER_LABELS)
      .filter(([provider]) => normalizeText(searchProviderKeys?.[provider]))
      .map(([, label]) => label),
    process.env.TAVILY_API_KEY || process.env.TAVILY_API_KEYS ? "Tavily" : "",
    process.env.SERPER_API_KEY || process.env.SERPER_API_KEYS ? "Serper" : "",
    process.env.BRAVE_API_KEY || process.env.BRAVE_API_KEYS ? "Brave" : "",
    process.env.JINA_API_KEY || process.env.JINA_API_KEYS ? "Jina" : "",
  ]);
const hasConfiguredResearchProvider = (
  provider = "",
  ...
  if (normalizedProvider === "openrouter") {
    return Boolean(
      normalizeText(researchProviderKeys?.openrouter) ||
  ...
  if (normalizedProvider === "google" || normalizedProvider === "gemini") {
    return Boolean(
      normalizeText(
        researchProviderKeys?.google || researchProviderKeys?.gemini,
      ) ||
...
const resolveDefaultResearchProvider = (researchProviderKeys = {}) => {
  if (hasConfiguredResearchProvider("openrouter", researchProviderKeys))
    return "openrouter";
  if (hasConfiguredResearchProvider("google", researchProviderKeys))
    return "google";
  return DEFAULT_RESEARCH_PROVIDER;
};
const resolveDefaultResearchModel = (provider = DEFAULT_RESEARCH_PROVIDER) =>
  normalizeText(provider).toLowerCase() === "google"
    ? DEFAULT_GEMINI_RESEARCH_MODEL
    : DEFAULT_RESEARCH_MODEL;
const normalizeResearchProvider = (
  value,
  fallback = DEFAULT_RESEARCH_PROVIDER,
) => {
...
const normalizeResearchModel = (value, fallback = DEFAULT_RESEARCH_MODEL) => {
  const raw = normalizeText(value);
  const lowered = raw.toLowerCase();
  if (!raw) return fallback;
  if (RESEARCH_MODEL_ALIASES[lowered]) return RESEARCH_MODEL_ALIASES[lowered];
  if (lowered.startsWith("openrouter/")) return raw.slice("openrouter/".length);
  if (
    raw.includes("/") ||
...
const normalizeResearchModelChain = (
  value,
  fallbackModel = DEFAULT_RESEARCH_MODEL,
) => {
  ...
  const normalized = unique(
    entries.map((item) => normalizeResearchModel(item, "")).filter(Boolean),
  );
  return normalized.length
    ? normalized
    : [normalizeResearchModel(fallbackModel, DEFAULT_RESEARCH_MODEL)];
};
const getConfiguredResearchModelChain = (
  fallbackModel = DEFAULT_RESEARCH_MODEL,
) =>
  normalizeResearchModelChain(
    process.env.RESEARCH_SUBAGENT_MODEL_CHAIN ||
      process.env.OPENROUTER_RESEARCH_MODEL_CHAIN ||
      process.env.RESEARCH_MODEL_CHAIN ||
      fallbackModel,
    fallbackModel,
  );
const inferResearchProviderFromModel = (
  model = "",
  fallback = DEFAULT_RESEARCH_PROVIDER,
) =>
  normalizeResearchModel(model, DEFAULT_RESEARCH_MODEL)
    .toLowerCase()
    .startsWith("gemini-")
    ? "google"
    : fallback;
const normalizeResearchRoundRobin = (value, fallback = false) => {
  if (typeof value === "boolean") return value;
  const resolved = normalizeText(value).toLowerCase();
  if (!resolved) return fallback;
  if (["1", "true", "yes", "on"].includes(resolved)) return true;
  if (["0", "false", "no", "off"].includes(resolved)) return false;
  return fallback;
};
const normalizeResearchProviderKeys = (value = {}) => {
  const candidate = value && typeof value === "object" ? value : {};
  const normalized = {
    openrouter: normalizeText(
      candidate?.openrouter ||
        candidate?.openRouter ||
        candidate?.["open-router"],
    ),
...
const listConfiguredResearchProviders = (researchProviderKeys = {}) =>
  unique([
    ...Object.entries(RESEARCH_MODEL_PROVIDER_LABELS)
      .filter(([provider]) => normalizeText(researchProviderKeys?.[provider]))
      .map(([, label]) => label),
    process.env.OPENROUTER_API_KEY || process.env.OPENROUTER_API_KEYS
      ? "OpenRouter"
      : "",
    process.env.GEMINI_API_KEY || process.env.GEMINI_API_KEYS ? "Google" : "",
  ]);
...
const isDecisionOutputMode = (outputModeId = "") =>
  DECISION_OUTPUT_MODES.has(normalizeText(outputModeId).toLowerCase());

const buildRunId = () =>
  `research-${Date.now()}-${crypto.randomUUID().slice(0, 8)}`;
...
const selectResearchModelForContext = (context, subagent = "") => {
  const chain = normalizeResearchModelChain(
    context.researchModelChain,
    context.requestedResearchModel || DEFAULT_RESEARCH_MODEL,
  );
  const cursor = Number(context.state?.researchModelCursor || 0) || 0;
  const roundRobinEnabled =
    context.researchRoundRobin === true && chain.length > 1;
  const modelIndex = roundRobinEnabled ? cursor % chain.length : 0;
  const model = chain[modelIndex] || chain[0] || DEFAULT_RESEARCH_MODEL;
  const provider = inferResearchProviderFromModel(
    model,
    context.requestedResearchProvider || DEFAULT_RESEARCH_PROVIDER,
  );
  const providerKeyCount =
    provider === "google"
      ? splitList(context.researchProviderKeys?.google).length
      : splitList(context.researchProviderKeys?.openrouter).length;
  context.state.researchModelCursor = roundRobinEnabled ? cursor + 1 : cursor;
  context.state.researchModelSelection = {
    provider,
    model,
    chain,
    chainLength: chain.length,
    providerKeyCount,
    roundRobinEnabled: roundRobinEnabled || providerKeyCount > 1,
    modelRoundRobinEnabled: roundRobinEnabled,
    keyRoundRobinEnabled: providerKeyCount > 1,
    modelIndex,
    lastSubagent: normalizeText(subagent),
    callCount: cursor + 1,
    configuredProviders: Array.isArray(context.availableModelProviders)
      ? context.availableModelProviders
      : [],
  };
  return context.state.researchModelSelection;
};
...
const runResearchModel = async ({
  ...
  const selection = selectResearchModelForContext(context, subagent);
  const normalizedMessages = Array.isArray(messages) ? [...messages] : [];
  const subagentContract = buildSubagentTaskContractBlock(
    context.plan,
    subagent,
  );
  if (subagentContract) {
    if (normalizedMessages[0]?.role === "system") {
      normalizedMessages[0] = {
        ...normalizedMessages[0],
        content:
          `${subagentContract}\n\n${String(normalizedMessages[0].content || "").trim()}`.trim(),
      };
    } else {
      normalizedMessages.unshift({
        role: "system",
        content: subagentContract,
      });
    }
  }
  const result = await runLiteHostChat({
    model: selection.model,
    modelChain: selection.chain,
    provider: selection.provider,
    providerApiKeys: context.researchProviderKeys,
    stream: false,
    use_tools: false,
    research_mode: true,
...
const buildSourceIndex = (sources = []) =>
  (Array.isArray(sources) ? sources : [])
    .map(
      (source) => `[${source.citationIndex}] ${source.title} — ${source.url}`,
    )
    .join("\n");

const selectDeliverySources = (sources = [], citedNumbers = [], limit = 8) => {
  const rankedSources = Array.isArray(sources) ? sources : [];
  if (!rankedSources.length) return [];

  const citedSources = rankedSources.filter((source) =>
    citedNumbers.includes(Number(source.citationIndex)),
  );
  if (citedSources.length) return citedSources;

  const preferredFallback = rankedSources.filter((source) =>
    ["core", "supporting"].includes(normalizeText(source?.tier).toLowerCase()),
  );
  return (preferredFallback.length ? preferredFallback : rankedSources).slice(
    0,
    Math.max(1, Number(limit) || 8),
  );
};

const buildNoGroundedSourcesReport = ({
  query = "",
  sourceMesh = {},
  partialFailures = [],
  tieredSources = [],
  ...
  const sourceCount = Array.isArray(sourceMesh?.sources)
    ? sourceMesh.sources.length
    : 0;
  const tieredCount = Array.isArray(tieredSources) ? tieredSources.length : 0;
  const evidenceCount = Array.isArray(evidenceEntries)
    ? evidenceEntries.length
    : 0;
  const providerErrors = unique([
    ...(Array.isArray(sourceMesh?.providerErrors)
      ? sourceMesh.providerErrors
      : []
    ).map((item) => normalizeText(item)),
    ...(Array.isArray(partialFailures) ? partialFailures : [])
      .map((failure) => normalizeText(failure?.message))
      .filter(Boolean),
  ]).slice(0, 6);
  const configuredSearchProviders = unique([
    ...(Array.isArray(configuredProviders) ? configuredProviders : []),
    ...listConfiguredSearchProviders(),
  ]);
  const providerStatus = configuredSearchProviders.length
    ? `Configured search providers for this run: ${configuredSearchProviders.join(", ")}.`
    : "No search provider keys were configured for this run. Add Tavily, Serper, Brave, or Jina in Settings or in the deployment environment for more reliable retrieval.";
  const heading =
    sourceCount || tieredCount
      ? "No Grounded Evidence Retrieved"
      : "No Sources Retrieved";
  const details = [];

  if (sourceCount) {
    details.push(
      `${sourceCount} source candidate${sourceCount === 1 ? "" : "s"} were collected, but only ${tieredCount} survived filtering and ${evidenceCount} readable page${evidenceCount === 1 ? "" : "s"} were fetched.`,
    );
  } else {
    details.push("Search providers returned no usable sources for this run.");
  }
  if (providerErrors.length) {
    details.push(`Provider signals: ${providerErrors.join(" | ")}.`);
  }
  details.push(providerStatus);
  details.push(
    "DuckDuckGo HTML fallback can be blocked by anti-bot challenges, so preview deployments should not rely on it as the only search path.",
  );
  ...
  return [
    ...
    "",
    normalizedQuery
      ? `I could not retrieve grounded sources for: ${normalizedQuery}`
      : "I could not retrieve grounded sources for this run.",
    "",
    details.join(" "),
    "",
    "## Next steps",
    "1. Configure at least one reliable search provider key in Settings or in the deployment environment.",
    "2. Rerun the query after the key is available to this runtime.",
    "3. If the problem persists, inspect provider errors in the research metadata for this run.",
  ].join("\n");
};
...
const buildPlanBrief = (plan = {}, memoryContextString = "") => {
  ...
  if (
    Array.isArray(plan.intentConfidence?.ambiguousAxes) &&
    plan.intentConfidence.ambiguousAxes.length
  ) {
    lines.push(
      `Ambiguous axes: ${plan.intentConfidence.ambiguousAxes.join(", ")}`,
    );
  }
  if (plan.continuity?.active && plan.continuity?.overlap) {
    lines.push(
      `Session continuity overlap: ${Math.round(plan.continuity.overlap * 100)}%`,
    );
  }
  if (memoryContextString) {
    lines.push(`Research memory linked: yes`);
  }
  if (resolvedPlanHasTaskFocus(plan)) {
    lines.push(
      `Task focus: ${normalizeText(plan.taskFocus.summary || plan.taskFocus.scopeContract || plan.taskFocus.primaryQuestion)}`,
    );
    if (
      Array.isArray(plan.taskFocus.anchorTerms) &&
      plan.taskFocus.anchorTerms.length
    ) {
      lines.push(`Focus anchors: ${plan.taskFocus.anchorTerms.join(", ")}`);
    }
  }

  const equippedSubagents = (
    Array.isArray(plan.subagents) ? plan.subagents : []
  )
    .filter(
      (entry) =>
        entry?.label &&
    ...
    .map(
      (entry) => `- ${entry.label}: ${entry.equipment.slice(0, 3).join(", ")}`,
    );
  if (equippedSubagents.length) {
    lines.push("Subagent equipment:");
    lines.push(...equippedSubagents);
  }
...
const resolvedPlanHasTaskFocus = (plan = {}) =>
  Boolean(
    plan?.taskFocus &&
    (normalizeText(plan.taskFocus.summary) ||
      normalizeText(plan.taskFocus.primaryQuestion) ||
      (Array.isArray(plan.taskFocus.anchorTerms) &&
        plan.taskFocus.anchorTerms.length)),
  );

const buildTaskFocusBlock = (plan = {}) => {
  if (!resolvedPlanHasTaskFocus(plan)) {
    const fallbackQuestion = normalizeText(plan?.query || "");
    return fallbackQuestion
      ? `Primary question:\n- ${fallbackQuestion}\n\nAnti-drift rules:\n- Keep every major section tied to the primary question.\n- Remove generic filler or neighboring topics that do not change the answer.\n- State evidence gaps instead of guessing.`
      : "";
  }

  const taskFocus = plan.taskFocus || {};
  const lines = [
    "Primary question:",
    `- ${normalizeText(taskFocus.primaryQuestion || plan.query || "Answer the current task directly.")}`,
  ];

  if (normalizeText(taskFocus.scopeContract)) {
    lines.push(
      "",
      "Scope contract:",
      `- ${normalizeText(taskFocus.scopeContract)}`,
    );
  }
  if (Array.isArray(taskFocus.anchorTerms) && taskFocus.anchorTerms.length) {
    lines.push(
      "",
      "Focus anchors:",
      ...taskFocus.anchorTerms.map((term) => `- ${term}`),
    );
  }
  if (
    Array.isArray(taskFocus.requiredOutcomes) &&
    taskFocus.requiredOutcomes.length
  ) {
    lines.push(
      "",
      "Required outcomes:",
      ...taskFocus.requiredOutcomes.map((item) => `- ${item}`),
    );
  }
  if (
    Array.isArray(taskFocus.forbiddenDriftBehaviors) &&
    taskFocus.forbiddenDriftBehaviors.length
  ) {
    lines.push(
      "",
      "Forbidden drift behaviors:",
      ...taskFocus.forbiddenDriftBehaviors.map((item) => `- ${item}`),
    );
  }
...
const buildSubagentTaskContractBlock = (plan = {}, subagent = "") => {
  const normalizedSubagent = normalizeText(subagent).toLowerCase();
  if (!normalizedSubagent && !resolvedPlanHasTaskFocus(plan)) return "";
  ...
  const descriptor = (
    ...
    return normalizedSubagent === id || normalizedSubagent === label;
  });

  const taskContract = normalizeText(
    descriptor?.taskContract || plan?.taskFocus?.scopeContract || "",
  );
  const anchorTerms = Array.isArray(plan?.taskFocus?.anchorTerms)
    ? plan.taskFocus.anchorTerms.slice(0, 5)
    : [];
  const driftWarnings = Array.isArray(plan?.taskFocus?.forbiddenDriftBehaviors)
    ? plan.taskFocus.forbiddenDriftBehaviors.slice(0, 2)
    : [];
  const lines = [
    "Stay on the original user request and do not go off track.",
    normalizeText(plan?.taskFocus?.primaryQuestion || plan?.query)
      ? `Primary question: ${normalizeText(plan.taskFocus?.primaryQuestion || plan.query)}`
      : "",
    taskContract ? `Subagent contract: ${taskContract}` : "",
    anchorTerms.length ? `Focus anchors: ${anchorTerms.join(", ")}` : "",
    driftWarnings.length ? `Forbidden drift: ${driftWarnings.join(" | ")}` : "",
    "If the evidence does not support a point, say the evidence is insufficient instead of broadening the topic.",
  ].filter(Boolean);
...
const computeTermOverlap = (anchorTerms = [], text = "") => {
  const terms = unique(
    (Array.isArray(anchorTerms) ? anchorTerms : [])
      .map((term) => normalizeText(term).toLowerCase())
      .filter(Boolean),
  );
  if (!terms.length) return 0;
  const otherTerms = new Set(
    extractQueryTerms(normalizeText(text).toLowerCase(), 48),
  );
  ...
  return overlap / Math.max(Math.min(terms.length, 3), 1);
};
...
const classifyFailure = (error, nodeId = "") => {
  ...
  const message = normalizeText(
    error?.message || error || "Research runtime failure",
  );
  const failureType = /timed out|timeout|429|503|overloaded|retry/i.test(
    message,
  )
    ? "soft"
...
const emitRuntimeEvent = async (context, type, payload = {}) => {
  ...
  context.run.events = [
    ...(Array.isArray(context.run.events) ? context.run.events : []),
    event,
  ].slice(-80);
...
const writeCheckpoint = async (context, id, payload = {}) => {
  ...
  const phase =
    (Array.isArray(context.plan?.dag?.nodes)
      ? context.plan.dag.nodes
      : []
...
const parseTribunalScores = (text = "") => {
  ...
  const critics =
    parsed.critics && typeof parsed.critics === "object" ? parsed.critics : {};
  return {
    internal_consistency: Math.max(
      0,
      Math.min(
        1,
        Number(
          critics.internal_consistency ??
            critics.internalConsistency ??
            parsed.internal_consistency ??
            parsed.internalConsistency ??
            0.78,
        ) || 0.78,
      ),
    ),
    coverage: Math.max(
      0,
      Math.min(1, Number(critics.coverage ?? parsed.coverage ?? 0.78) || 0.78),
    ),
    user_goal_alignment: Math.max(
      0,
      Math.min(
        1,
        Number(
          critics.user_goal_alignment ??
            critics.userGoalAlignment ??
            parsed.user_goal_alignment ??
            parsed.userGoalAlignment ??
            0.82,
        ) || 0.82,
      ),
    ),
    targeted_dimension: normalizeText(
      ...
      .replace(/\s+/g, "_")
      .toLowerCase(),
    rewrite_brief: normalizeText(
      parsed.rewrite_brief || parsed.rewriteBrief || parsed.guidance || "",
    ),
  };
};
  ...
  const issues = unique(
    [
      ...
      ...(Array.isArray(parsed.unsupported_claims)
        ...
        : []),
      ...(Array.isArray(parsed.overreach_claims)
        ? parsed.overreach_claims
        : []),
      ...(Array.isArray(parsed.broken_citations)
        ? parsed.broken_citations
        : []),
      ...(Array.isArray(parsed.weak_citations) ? parsed.weak_citations : []),
      ...(Array.isArray(parsed.omitted_counterevidence)
        ? parsed.omitted_counterevidence
        : []),
      ...(Array.isArray(parsed.false_consensus_flags)
        ? parsed.false_consensus_flags
        : []),
      ...(Array.isArray(parsed.overconfident_statements)
        ? parsed.overconfident_statements
        : []),
      ...(Array.isArray(parsed.missing_caveats) ? parsed.missing_caveats : []),
      ...(Array.isArray(parsed.off_topic_sections)
        ? parsed.off_topic_sections
        : []),
      ...(Array.isArray(parsed.drift_sections) ? parsed.drift_sections : []),
      ...(Array.isArray(parsed.scope_violations)
        ? parsed.scope_violations
        : []),
      ...(Array.isArray(parsed.mode_violations) ? parsed.mode_violations : []),
    ]
      .map((item) =>
        normalizeText(
          typeof item === "string"
            ? item
            : item?.claim || item?.issue || item?.text || "",
        ),
      )
      .filter(Boolean),
  );
...
const buildEvidenceDigest = (evidenceEntries = []) =>
  (Array.isArray(evidenceEntries) ? evidenceEntries : [])
    .slice(0, 6)
    .map((entry) => {
      const citation = entry?.source?.citationIndex
        ? `[${entry.source.citationIndex}] `
        : "";
      const title = normalizeText(entry?.source?.title || "Untitled source");
      const excerpt = normalizeText(
        entry?.content || entry?.excerpt || entry?.evidenceBlock || "",
      ).slice(0, 280);
      return `${citation}${title}\n${excerpt}`;
    })
    .join("\n\n");

const buildTaskFocusAssessment = ({
  plan = {},
  currentDraft = "",
  evidenceEntries = [],
} = {}) => {
  const fallbackAnchorTerms = extractQueryTerms(
    normalizeText(plan.query),
    10,
  ).slice(0, 8);
  const taskFocus = plan.taskFocus || {};
  const anchorTerms = unique(
    (Array.isArray(taskFocus.anchorTerms)
      ? taskFocus.anchorTerms
      : fallbackAnchorTerms
    )
      .map((term) => normalizeText(term).toLowerCase())
      .filter((term) => term && !TASK_FOCUS_STOP_TERMS.has(term)),
  );
  const resolvedAnchorTerms = anchorTerms.length
    ? anchorTerms
    : unique(
        fallbackAnchorTerms
          .map((term) => normalizeText(term).toLowerCase())
  ...
  const overlapThreshold = Number.isFinite(Number(taskFocus.overlapThreshold))
    ? Number(taskFocus.overlapThreshold)
    : 0.22;
  const draftOverlap = computeTermOverlap(resolvedAnchorTerms, currentDraft);
  const evidenceOverlap = computeTermOverlap(
    resolvedAnchorTerms,
    buildEvidenceDigest(evidenceEntries),
  );
  const sections = String(currentDraft || "")
    .split(/\n(?=#+\s|[-*]\s|\d+\.\s)/)
    .map((section) => normalizeText(section))
    .filter(Boolean);
  const offTrackSections = sections.filter((section) => {
    if (section.length < 140) return false;
    const normalizedSection = section.toLowerCase();
    return !resolvedAnchorTerms.some((term) =>
      normalizedSection.includes(term),
    );
  });
  const offTrackPenalty = Math.min(0.22, offTrackSections.length * 0.08);
  const minimumAnchorSignal = resolvedAnchorTerms.length >= 6 ? 0.1 : 0.16;
  const hasAnchorSignal =
    draftOverlap >= minimumAnchorSignal ||
    evidenceOverlap >= minimumAnchorSignal;
  let score =
    0.12 + draftOverlap * 0.52 + evidenceOverlap * 0.26 - offTrackPenalty;
  if (hasAnchorSignal) {
    score += 0.08;
  } else {
    score = Math.min(score, 0.42);
  }
  if (draftOverlap >= overlapThreshold) score += 0.08;
  if (evidenceOverlap >= overlapThreshold) score += 0.06;
  if (evidenceOverlap >= minimumAnchorSignal) score += 0.16;
  if (
    draftOverlap >= minimumAnchorSignal &&
    evidenceOverlap >= minimumAnchorSignal
  )
    score += 0.1;
  if (
    !offTrackSections.length &&
    !evidenceEntries.length &&
    draftOverlap < Math.max(overlapThreshold + 0.04, 0.36)
  ) {
    score = Math.min(score, 0.62);
  }
  score = clampScore(score, 0.78);
  const issues = [];

  if (draftOverlap < overlapThreshold) {
    issues.push(
      `Draft drifted away from the query anchors (${resolvedAnchorTerms.slice(0, 5).join(", ")}).`,
    );
  }
  if (
    !resolvedAnchorTerms.some((term) =>
      normalizeText(currentDraft).toLowerCase().includes(term),
    )
  ) {
    issues.push("Draft does not stay anchored to the user's key terms.");
  }
  if (
    normalizeText(currentDraft).split(/\s+/).length > 140 &&
    draftOverlap < Math.max(0.18, overlapThreshold + 0.08)
  ) {
    issues.push(
      "Draft spends too much space on background that does not materially answer the question.",
    );
  }
  if (offTrackSections.length) {
    issues.push(
      "One or more long sections drift away from the query anchors and should be removed or rewritten.",
    );
  }
  ...
  return {
    ...
    issues,
    rewrite_brief: issues.length
      ? `Pull the answer back to the primary question: ${normalizeText(taskFocus.primaryQuestion || plan.query)}. Keep these anchors visible: ${resolvedAnchorTerms.slice(0, 6).join(", ")}. Remove sections that do not change the answer.`
      : "",
  };
};
...
const buildVerifierSummary = ({
  ...
  const unsupportedCitationPenalty = Math.min(
    0.35,
    (citationValidation.unsupported || []).length * 0.08,
  );
  const dimensions = {
    claim_support: clampScore(claimAssessment.score, 0.78),
    citation_integrity: clampScore(
      (citationAssessment.score ?? 0.78) - unsupportedCitationPenalty,
      0.72,
    ),
    ...
    task_focus: clampScore(taskFocusAssessment.score, 0.8),
  };
  ...
  return {
    ...
    assessments: {
      ...
      taskFocusVerifier: taskFocusAssessment,
    },
  };
};
...
const buildRewriteGuidance = ({
  ...
  const dimensionToAssessment = {
    claim_support:
      verifierSummary.assessments?.claim_support ||
      verifierSummary.assessments?.claimVerifier,
    citation_integrity:
      verifierSummary.assessments?.citation_integrity ||
      verifierSummary.assessments?.citationVerifier,
    contradiction_handling:
      verifierSummary.assessments?.contradiction_handling ||
      verifierSummary.assessments?.contradictionVerifier,
    uncertainty_calibration:
      verifierSummary.assessments?.uncertainty_calibration ||
      verifierSummary.assessments?.uncertaintyVerifier,
    task_focus:
      verifierSummary.assessments?.task_focus ||
      verifierSummary.assessments?.taskFocusVerifier,
  };
...
const buildExtractionSnapshot = (extraction = {}) => {
  ...
  const repositories = Array.isArray(extraction.repositories)
    ? extraction.repositories
    : [];
  const supplementary = Array.isArray(extraction.supplementary)
    ? extraction.supplementary
    : [];
  const concepts = Array.isArray(extraction.concepts)
    ? extraction.concepts
    : [];
  const contradictions = Array.isArray(extraction.contradictions)
  ...
  const reproducibilityAverage = Number(
    extraction.codeArtifacts?.reproducibilityAverage || 0,
  );
  const claimPreview = claims
    .slice(0, 4)
    .map((claim) =>
      `${claim.metric || claim.type || "claim"} ${claim.operator || ""} ${claim.value ?? ""}`.trim(),
    )
    .filter(Boolean);
  const contradictionPreview = contradictions
    .slice(0, 3)
    .map((item) =>
      normalizeText(item?.claim || item?.summary || item?.issue || item),
    )
    .filter(Boolean);
...
const runVerifierSwarm = async ({
  ...
  const taskFocusBlock = buildTaskFocusBlock(plan);
  const verifierInput = [
    `Research plan:\n${planBrief}`,
    ...(taskFocusBlock ? [`Task focus:\n${taskFocusBlock}`] : []),
  ...
  const verifierEntries = await Promise.all(
    Object.entries(VERIFIER_SPECS).map(async ([dimension, spec]) => {
      ...
      const assessment = parseVerifierAssessment(response, {
        score: spec.threshold,
      });
      if (
        dimension === "citation_integrity" &&
        citationValidation.unsupported.length
      ) {
        ...
        assessment.rewrite_brief =
          `${assessment.rewrite_brief} Remove or replace claims that cite peripheral, discarded, or missing sources.`.trim();
      }
  ...
  const taskFocusAssessment = buildTaskFocusAssessment({
    plan,
    currentDraft,
    evidenceEntries,
  });
  const mergedTaskFocusAssessment = assessments.task_focus
    ? {
        ...assessments.task_focus,
        score: Math.min(
          clampScore(assessments.task_focus.score, 0.8),
          taskFocusAssessment.score,
        ),
        issues: unique([
          ...(Array.isArray(assessments.task_focus.issues)
            ? assessments.task_focus.issues
            : []),
          ...(Array.isArray(taskFocusAssessment.issues)
            ? taskFocusAssessment.issues
            : []),
        ]),
        rewrite_brief: unique(
          [
            normalizeText(assessments.task_focus.rewrite_brief),
            normalizeText(taskFocusAssessment.rewrite_brief),
          ].filter(Boolean),
        )
          .join(" ")
          .trim(),
    ...
    : taskFocusAssessment;
  assessments.task_focus = mergedTaskFocusAssessment;
  assessments.taskFocusVerifier = mergedTaskFocusAssessment;
  const scores = Object.fromEntries(
    verifierEntries.map(([dimension, assessment]) => [
      dimension,
      assessment.score,
    ]),
  );
  scores.task_focus = mergedTaskFocusAssessment.score;
  const rewriteHints = verifierEntries
    ...
    .filter(Boolean);
  if (normalizeText(mergedTaskFocusAssessment.rewrite_brief)) {
    rewriteHints.push(
      `task_focus: ${normalizeText(mergedTaskFocusAssessment.rewrite_brief)}`,
    );
  }
  ...
  return {
    ...
    unsupportedCitationCount: citationValidation.unsupported.length,
    issues: unique([
      ...verifierEntries.flatMap(([dimension, assessment]) =>
        buildVerifierIssueList(dimension, assessment.issues),
      ),
      ...buildVerifierIssueList("task_focus", mergedTaskFocusAssessment.issues),
    ]).slice(0, 16),
    rewriteBrief: rewriteHints.join(" ").trim(),
    aggregateScore: Number(
      averageScore(Object.values(scores), 0.78).toFixed(2),
    ),
  };
};
...
const buildClaimLedger = ({
  ...
  const sourceByCitation = new Map(
    (Array.isArray(tieredSources) ? tieredSources : []).map((source) => [
      Number(source.citationIndex),
      source,
    ]),
  );
  const keyClaims = positionMaps.flatMap((map) =>
    Array.isArray(map?.key_claims) ? map.key_claims : [],
  );
  const extractedClaims = Array.isArray(extraction.claims)
    ? extraction.claims
    : [];
  const effectClaims = extractedClaims
    .filter((claim) => claim.type === "effect_size" || claim.type === "p_value")
    .slice(0, 12);
  const contradictionPenalty =
    verifierSummary?.scores?.contradiction_handling != null
      ? 1 - clampScore(verifierSummary.scores.contradiction_handling, 0.76)
      : null;
  const uncertaintyPenalty =
    verifierSummary?.scores?.uncertainty_calibration != null
      ? 1 - clampScore(verifierSummary.scores.uncertainty_calibration, 0.78)
      : null;

  const structuredClaims = keyClaims
    .map((claim) => {
      ...
      const contradictionScore =
        contradictionPenalty == null
          ? tribunal?.targeted_dimension === "internal_consistency"
            ? 0.24
            : 0.16
          : Math.max(0.1, contradictionPenalty);
      const sensitivity =
        uncertaintyPenalty == null
          ? /uncertain|limited|small sample|pilot/i.test(claim?.claim || "")
            ? 0.44
            : 0.24
          : Math.max(
              /uncertain|limited|small sample|pilot/i.test(claim?.claim || "")
  ...
  const quantitativeClaims = effectClaims.map((claim) => ({
    claim:
      `${claim.metric || claim.type} ${claim.operator || ""} ${claim.value}`.trim(),
    citations: [],
    confidence: claim.sampleSizeFlag
      ? 0.52
      : Number(
          (
            0.46 +
            (1 - Math.min(0.45, uncertaintyPenalty ?? 0.22)) * 0.26
          ).toFixed(2),
        ),
    evidence_weight: claim.sampleSizeFlag ? 0.48 : 0.64,
    contradiction_score: Number(
      Math.max(0.12, contradictionPenalty ?? 0.18).toFixed(2),
    ),
    sensitivity: claim.sampleSizeFlag
      ? 0.5
      : Number(Math.max(0.18, uncertaintyPenalty ?? 0.26).toFixed(2)),
  }));
...
const buildDecisionLayer = ({
  ...
  const epistemicRisk = Number(
    (convergence?.residual_uncertainty ?? 0.28).toFixed(2),
  );
  const technicalRisk = Number(
    Math.min(
      0.9,
      0.18 +
        (safety?.manipulationCount || 0) * 0.08 +
        (safety?.dualUseCount || 0) * 0.12,
    ).toFixed(2),
  );
  const verifierScores = Object.values(
    verifierSummary?.scores || verifierSummary?.dimensions || {},
  );
  const verifierRisk = Number(
    Math.max(
      0.08,
      1 - clampScore(averageScore(verifierScores, 0.78), 0.78),
    ).toFixed(2),
  );
  const epistemicComposite = Number(
    Math.max(epistemicRisk, verifierRisk).toFixed(2),
  );
  ...
  return {
    ...
    confidence,
    reversibility:
      epistemicRisk > 0.45 ? "low" : epistemicRisk > 0.28 ? "medium" : "high",
    support: {
      targeted_dimension: tribunal?.targeted_dimension || "",
      verifier_average: Number(averageScore(verifierScores, 0.78).toFixed(2)),
      verifier_issues: Array.isArray(verifierSummary?.issues)
        ? verifierSummary.issues.length
        : 0,
    },
  };
};
...
const buildMarkdownExport = (run = {}) => {
  ...
  const concepts = (run.extraction?.concepts || [])
    .slice(0, 12)
...
const buildRuntimeMeta = ({ run = {}, plan = null, state = null } = {}) => {
  ...
  const tieredSources = Array.isArray(resolvedState.tieredSources)
    ? resolvedState.tieredSources
    : [];
  const evidenceEntries = Array.isArray(resolvedState.evidenceEntries)
    ? resolvedState.evidenceEntries
    : [];
  const extraction = run.extraction || resolvedState.extraction || {};
  const partialFailures = Array.isArray(run.partialFailures)
    ? run.partialFailures
    : [];
  ...
  return {
    ...
    depthPreference:
      resolvedPlan.depthPreference ||
      run.requestedDepthPreference ||
      run.requestOptions?.depthPreference ||
      null,
    ...
    taskFocus: resolvedPlan.taskFocus || null,
    safety:
      run.safety ||
      resolvedState.safety ||
      extraction.safety ||
      resolvedPlan.safety ||
      null,
    ...
    searchCount: Array.isArray(resolvedPlan.searchQueries)
      ? resolvedPlan.searchQueries.length
      : 0,
    rankedSites: Array.isArray(sourceMesh.sources)
      ? sourceMesh.sources.length
      : tieredSources.length,
    configuredProviders: Array.isArray(run.requestOptions?.searchProviders)
      ? run.requestOptions.searchProviders
      : [],
    researchProvider: run.requestOptions?.researchProvider || null,
    researchModel: run.requestOptions?.researchModel || null,
    researchModelChain: Array.isArray(run.requestOptions?.researchModelChain)
      ? run.requestOptions.researchModelChain
      : [],
    configuredModelProviders: Array.isArray(run.requestOptions?.modelProviders)
      ? run.requestOptions.modelProviders
      : [],
    activeResearchModel: resolvedState.researchModelSelection || null,
    providersUsed: Array.isArray(sourceMesh.providersUsed)
      ? sourceMesh.providersUsed
      : [],
    providerErrors: Array.isArray(sourceMesh.providerErrors)
      ? sourceMesh.providerErrors.slice(0, 6)
      : [],
    fetchPlanned: tieredSources.filter((source) => source?.fetchMode !== "skip")
      .length,
    fetchedSites: evidenceEntries.length,
    fetchAttempts: evidenceEntries.length + partialFailures.length,
    synthesisWorkers: Math.max(
      1,
      Array.isArray(resolvedState.positionMaps)
        ? resolvedState.positionMaps.length
        : 0,
    ),
    tribunal: run.tribunal || resolvedState.tribunal || null,
    convergence: run.convergence || resolvedState.convergence || null,
    verifierSummary:
      run.verifierSummary || resolvedState.verifierSummary || null,
    ...
    dataset_json: JSON.stringify(
      {
        ...
        verifierSummary:
          run.verifierSummary || run.state?.verifierSummary || null,
...
const resolveControls = (controls = []) =>
  Array.isArray(controls)
    ? controls.filter((control) => control && typeof control === "object")
    : [];
...
const getForcedOutputMode = (controls = [], fallback = "") => {
  const forced = [...resolveControls(controls)]
    ...
    .find(
      (control) =>
        control.type === "force_mode" &&
        normalizeText(control.mode || control.value),
    );
...
const getDepthOverride = (controls = [], fallback = "") => {
  const latestControl = [...resolveControls(controls)]
    .reverse()
    .find((control) =>
      ["increase_depth", "go_deeper", "prioritize_speed"].includes(
        normalizeText(control.type),
      ),
    );
  if (
    latestControl?.type === "increase_depth" ||
    latestControl?.type === "go_deeper"
  )
    return "deep";
...
const applyControlFilters = (sources = [], controls = []) => {
  const excludedSourceTypes = new Set(
    resolveControls(controls)
      .filter(
        (control) =>
          control.type === "exclude_source" &&
          normalizeText(control.sourceType || control.typeId),
      )
      .map((control) =>
        normalizeText(control.sourceType || control.typeId).toLowerCase(),
      ),
  );
...
const normalizeAppliedControls = (controls = []) =>
  resolveControls(controls).map((control) => {
    const payload =
      control?.payload && typeof control.payload === "object"
        ? control.payload
        : control;
    return {
...
const getRestartNodeForPlanChange = ({
  ...
  if (
    normalizeText(previousPlan?.depthPreference) !==
    normalizeText(nextPlan?.depthPreference)
  ) {
    restartNode = compareRestartNodes(restartNode, "adversarialQueryForge");
  }
  if (
    normalizeText(previousPlan?.outputMode?.id) !==
    normalizeText(nextPlan?.outputMode?.id)
  ) {
    restartNode = compareRestartNodes(
      restartNode,
      "dialecticalSynthesisEngine",
    );
  }
  if (
    normalizeRefinementBudget(previousPlan?.refinementBudget, 3) !==
    normalizeRefinementBudget(nextPlan?.refinementBudget, 3)
  ) {
    restartNode = compareRestartNodes(
      restartNode,
      "recursiveSelfImprovementLoop",
  ...
  for (const control of normalizeAppliedControls(controls)) {
    restartNode = compareRestartNodes(
      restartNode,
      getRestartNodeForControl(control),
    );
  }
...
const rollbackStateFromNode = (context, startNode = "", options = {}) => {
  ...
  context.run.completedNodes = (
    Array.isArray(context.run.completedNodes) ? context.run.completedNodes : []
...
const recompilePlanWithControls = (context, appliedControls = []) => {
  ...
  context.plan = compileResearchPlan({
    ...
    depthPreference: getDepthOverride(
      mergedControls,
      context.requestedDepthPreference || context.plan?.depthPreference || "",
    ),
    forcedOutputMode: getForcedOutputMode(
      mergedControls,
      context.requestedForcedOutputMode || context.plan?.outputMode?.id || "",
    ),
    refinementBudget:
      context.requestedRefinementBudget || context.plan?.refinementBudget || 3,
    maxQueries:
      context.requestedMaxQueries ||
      context.run.requestOptions?.maxQueries ||
      context.plan?.searchQueries?.length ||
      6,
    memoryContext: context.memoryContext,
    steering: mergedControls,
  });
  const activeNodeIds = new Set(
    (Array.isArray(context.plan?.dag?.nodes) ? context.plan.dag.nodes : []).map(
      (node) => node.id,
    ),
  );
  ...
  context.run.requestOptions = {
    ...
    maxQueries:
      ...
      context.plan?.searchQueries?.length ||
      6,
    searchProviders: Array.isArray(context.availableSearchProviders)
      ? context.availableSearchProviders
      : [],
    researchProvider:
      context.requestedResearchProvider || DEFAULT_RESEARCH_PROVIDER,
    researchModel: context.requestedResearchModel || DEFAULT_RESEARCH_MODEL,
    researchModelChain: Array.isArray(context.researchModelChain)
      ? context.researchModelChain
      : [DEFAULT_RESEARCH_MODEL],
    researchRoundRobin: context.researchRoundRobin === true,
    modelProviders: Array.isArray(context.availableModelProviders)
      ? context.availableModelProviders
      : [],
  };
...
const processPendingControls = async (context) => {
  ...
  await markResearchControlsApplied(
    appliedControls.map((control) => control.id).filter(Boolean),
  );
  await persistRuntimeState(context);
  return restartNode;
};
...
const buildRuntimeContext = async ({
  ...
  const requestedDepthPreference = normalizeText(
    depthPreference ||
      resumeRun?.requestOptions?.depthPreference ||
      resumeRun?.plan?.depthPreference ||
      "",
  ).toLowerCase();
  const requestedForcedOutputMode = normalizeText(
    forcedOutputMode ||
      resumeRun?.requestOptions?.forcedOutputMode ||
      resumeRun?.plan?.outputMode?.id ||
      "",
  ).toLowerCase();
  const requestedRefinementBudget = normalizeRefinementBudget(
    refinementBudget ||
      resumeRun?.requestOptions?.refinementBudget ||
      resumeRun?.plan?.refinementBudget ||
      3,
    3,
  );
  const requestedMaxQueries = Math.max(
    ...
    Number(
      maxQueries ||
        resumeRun?.requestOptions?.maxQueries ||
        resumeRun?.plan?.searchQueries?.length ||
        6,
    ) || 6,
  );
  const normalizedSearchProviderKeys =
    normalizeSearchProviderKeys(searchProviderKeys);
  const normalizedResearchProviderKeys =
    normalizeResearchProviderKeys(researchProviderKeys);
  const availableSearchProviders = unique([
    ...listConfiguredSearchProviders(normalizedSearchProviderKeys),
    ...(Array.isArray(resumeRun?.requestOptions?.searchProviders)
      ? resumeRun.requestOptions.searchProviders
      : []),
  ]);
  const defaultResearchProvider = resolveDefaultResearchProvider(
    normalizedResearchProviderKeys,
  );
  const defaultResearchModel = resolveDefaultResearchModel(
    resumeRun?.requestOptions?.researchProvider ||
      researchProvider ||
      defaultResearchProvider,
  );
  const requestedResearchModelChain = normalizeResearchModelChain(
    Array.isArray(researchModelChain) && researchModelChain.length
      ? researchModelChain
      : researchModelChain ||
          resumeRun?.requestOptions?.researchModelChain ||
          getConfiguredResearchModelChain(defaultResearchModel),
    researchModel ||
      resumeRun?.requestOptions?.researchModel ||
      defaultResearchModel,
  );
  const requestedResearchModel = normalizeResearchModel(
    researchModel ||
      resumeRun?.requestOptions?.researchModel ||
      requestedResearchModelChain[0] ||
      defaultResearchModel,
    defaultResearchModel,
  );
  const requestedResearchProvider = normalizeResearchProvider(
    researchProvider ||
      resumeRun?.requestOptions?.researchProvider ||
      inferResearchProviderFromModel(
  ...
  const requestedResearchRoundRobin = normalizeResearchRoundRobin(
    researchRoundRobin ?? resumeRun?.requestOptions?.researchRoundRobin,
    requestedResearchModelChain.length > 1,
  );
  const availableModelProviders = unique([
    ...listConfiguredResearchProviders(normalizedResearchProviderKeys),
    ...(Array.isArray(resumeRun?.requestOptions?.modelProviders)
      ? resumeRun.requestOptions.modelProviders
      : []),
  ]);
  ...
  const run = resumeRun || {
    ...
    requestOptions: {
      ...
      refinementBudget: requestedRefinementBudget,
      maxQueries: requestedMaxQueries,
      searchProviders: availableSearchProviders,
      researchProvider: requestedResearchProvider,
      researchModel: requestedResearchModel,
      researchModelChain: requestedResearchModelChain,
      researchRoundRobin: requestedResearchRoundRobin,
  ...
  return {
    ...
    requestedMaxQueries,
    searchProviderKeys: normalizedSearchProviderKeys,
    requestedResearchProvider,
    requestedResearchModel,
    researchModelChain: requestedResearchModelChain,
    researchRoundRobin: requestedResearchRoundRobin,
    researchProviderKeys: normalizedResearchProviderKeys,
    ...
    run: {
      ...
      runId:
        normalizeText(run.runId || run.id || resolvedRunId) || resolvedRunId,
      query,
      scopeKey,
      attachments,
      controls: filteredControls,
...
const resolveResumeStartNode = (run = {}) => {
  ...
  if (checkpoint === "tribunal") {
    return isDecisionOutputMode(run?.plan?.outputMode?.id)
      ? "decisionIntelligenceLayer"
      : "adaptiveDeliveryHub";
  }
  ...
  switch (node.id) {
    ...
    case "intelligentCrawlerMesh": {
      const sourceMesh = await searchResearchSources({
        ...
        plan,
        maxResults: 28,
        providerApiKeys: context.searchProviderKeys,
      });
      context.state.sourceMesh = sourceMesh;
      if (!sourceMesh.sources.length && sourceMesh.providerErrors.length) {
        await emitRuntimeEvent(context, "warning", {
          nodeId: node.id,
          detail: `Search providers returned no usable sources. ${sourceMesh.providerErrors.slice(0, 3).join(" | ")}`,
        });
      }
      await emitRuntimeEvent(context, "status", {
        nodeId: node.id,
    ...
    case "tieredEpistemicFilter": {
      const filteredSources = applyControlFilters(
        context.state.sourceMesh?.sources || [],
        context.controls,
      )
      ...
      context.state.inventory = {
        ...
        supporting: tieredSources.filter(
          (source) => source.tier === "supporting",
        ).length,
        peripheral: tieredSources.filter(
          (source) => source.tier === "peripheral",
        ).length,
      };
    ...
    case "deepComprehensionEngine": {
      const evidenceEntries = await fetchTieredEvidence(
        context.state.tieredSources || [],
        context.query,
      );
      const rankedEvidenceEntries = rankEvidenceEntriesForQuery(
        context.query,
        evidenceEntries,
      ).slice(0, 24);
      if (!rankedEvidenceEntries.length) {
        throw new ResearchRuntimeError(
          "No readable research evidence was fetched.",
          {
            nodeId: node.id,
            failureType: "partial",
          },
    ...
    case "dialecticalSynthesisEngine": {
      ...
      for (let index = 0; index < evidenceChunks.length; index += 1) {
        const evidenceBlock = evidenceChunks[index]
          .map((entry) => entry.evidenceBlock)
          .join("\n\n---\n\n");
        const positionText = await runResearchModel({
          context,
          subagent: "PositionMapper",
          maxTokens: 1600,
          messages: [
            {
              role: "system",
              content:
                "You are PositionMapper inside Research Framework v3.1. Return strict JSON with keys dominant_position, counter_position, key_claims, quantitative_signals, and open_gaps. key_claims must be an array of objects with claim and citations. Use only bracketed citation numbers from the evidence.",
            },
            {
              role: "user",
      ...
      const positionMapBlock = positionMaps
        .map(
          (map, index) =>
            `### Position Map ${index + 1}\n${JSON.stringify(map, null, 2)}`,
        )
        .join("\n\n");

      const thesis = await runResearchModel({
        context,
        subagent: "ThesisAgent",
        maxTokens: 1400,
        messages: [
          {
            role: "system",
            content:
              "You are ThesisAgent in Research Framework v3.1. Build the strongest case for the dominant position using only the position maps and cited sources. Use bracketed citations [n] only.",
          },
          {
            role: "user",
            content: `Research plan:\n${planBrief}\n\nTask focus:\n${taskFocusBlock}${promptPatchSection}\n\nSource index:\n${sourceIndex}\n\nPosition maps:\n${positionMapBlock}`,
          },
        ],
      });
      ...
      const antithesis = await runResearchModel({
        context,
        subagent: "AntithesisAgent",
        maxTokens: 1400,
        messages: [
          {
            role: "system",
            content:
              "You are AntithesisAgent in Research Framework v3.1. Build the strongest counter-case using disconfirming evidence, counter-hypotheses, and the supplied position maps. Use bracketed citations [n] only.",
          },
          {
            role: "user",
    ...
    case "recursiveSelfImprovementLoop": {
      ...
      for (let cycle = 1; cycle <= plan.refinementBudget; cycle += 1) {
        const citationValidation = buildCitationValidation(
          currentDraft,
          context.state.tieredSources || [],
        );
        const [criticText, cycleVerifierSummary] = await Promise.all([
          runResearchModel({
            context,
            subagent: "Recursive Self-Improvement Loop",
            maxTokens: 900,
            messages: [
              {
                role: "system",
                content:
                  "You are the Recursive Self-Improvement Loop. Return strict JSON with keys internal_consistency, coverage, user_goal_alignment, targeted_dimension, and rewrite_brief. Score whether major claims are citation-supported, whether hypotheses and counter-hypotheses are covered, whether the output matches the requested format, and whether the draft stays centered on the original user question.",
              },
              {
                role: "user",
                content: `Research plan:\n${planBrief}\n\nTask focus:\n${taskFocusBlock}\n\nConstraints:\n${constraintBlock}\n\nCounter-hypotheses:\n${counterHypothesisBlock}${promptPatchSection}\n\nSource index:\n${sourceIndex}\n\nUnsupported citations count: ${citationValidation.unsupported.length}\n\nDraft:\n${currentDraft}`,
        ...
        verifierSummary = {
          ...cycleVerifierSummary,
          issues: unique([
            ...(Array.isArray(verifierSummary.issues)
              ? verifierSummary.issues
              : []),
            ...(Array.isArray(cycleVerifierSummary.issues)
              ? cycleVerifierSummary.issues
              : []),
          ]),
          rewriteBrief: unique(
            [
              normalizeText(verifierSummary.rewriteBrief),
              normalizeText(cycleVerifierSummary.rewriteBrief),
            ].filter(Boolean),
          )
            .join(" ")
            .trim(),
        ...
        const verifierCoverage = clampScore(
          verifierSummary.dimensions?.contradiction_handling,
          0.76,
        );
        const verifierAlignment = clampScore(
          clampScore(
            verifierSummary.dimensions?.uncertainty_calibration,
            0.78,
          ) *
            0.75 +
            clampScore(verifierSummary.dimensions?.task_focus, 0.62) * 0.25,
          0.78,
        );

        critic.internal_consistency = Math.min(
          critic.internal_consistency,
          verifierConsistency,
        );
        critic.coverage = Math.min(critic.coverage, verifierCoverage);
        critic.user_goal_alignment = Math.min(
          critic.user_goal_alignment,
          verifierAlignment,
        );

        if (
          citationValidation.unsupported.length ||
          verifierSummary.unsupportedCitationCount
        ) {
          critic.internal_consistency = Math.min(
            critic.internal_consistency,
            0.56,
        ...
        const verifierThresholdBreach = Object.entries(
          verifierSummary.dimensions || {},
        ).some(
          ([dimension, score]) =>
            score < (VERIFIER_SPECS[dimension]?.threshold || 0.78),
        );
        const needsRewrite =
        ...
        const targetedDimension =
          tribunal.targeted_dimension ||
          critic.targeted_dimension ||
          verifierSummary.targetedDimension ||
          "coverage";
        const rewriteBrief =
      ...
      context.state.postmortem = {
        ...
        phases_that_degraded_score: unique([
          ...
          ...(verifierSummary.issues || [])
            .slice(0, 3)
            .map((issue) => `Phase 6 - ${issue}`),
        ]),
        prompt_patches_applied:
          tribunal.refinement_cycles > 1
            ? unique([
    ...
    case "decisionIntelligenceLayer": {
      ...
      try {
        const decisionResponse = await runResearchModel({
          context,
          subagent: "Decision Intelligence Layer",
          maxTokens: 1200,
          messages: [
            {
              role: "system",
              content:
                "You are the Decision Intelligence Layer in Research Framework v3.1. Return strict JSON with keys decision, expected_outcome, risk_profile, confidence, reversibility, rationale, and recommended_actions. Keep all numeric risk and confidence values between 0 and 1.",
            },
            {
              role: "user",
        ...
        if (parsedDecision) {
          decisionLayer = {
            ...fallbackDecision,
            decision:
              normalizeText(parsedDecision.decision) ||
              fallbackDecision.decision,
            expected_outcome:
              normalizeText(
                parsedDecision.expected_outcome ||
                  parsedDecision.expectedOutcome,
              ) || fallbackDecision.expected_outcome,
            risk_profile: {
              technical: clampScore(
                parsedDecision.risk_profile?.technical ??
                  parsedDecision.riskProfile?.technical,
                fallbackDecision.risk_profile.technical,
              ),
              epistemic: clampScore(
                parsedDecision.risk_profile?.epistemic ??
                  parsedDecision.riskProfile?.epistemic,
            ...
            reversibility:
              normalizeText(parsedDecision.reversibility) ||
              fallbackDecision.reversibility,
            rationale: normalizeText(
              parsedDecision.rationale || parsedDecision.summary || "",
            ),
            recommended_actions: unique(
              [
                ...(Array.isArray(parsedDecision.recommended_actions)
                  ? parsedDecision.recommended_actions
                  : []),
                ...(Array.isArray(parsedDecision.actions)
                  ? parsedDecision.actions
                  : []),
              ]
                .map((item) => normalizeText(item))
                .filter(Boolean),
    ...
    case "adaptiveDeliveryHub": {
      const tieredSources = Array.isArray(context.state.tieredSources)
        ? context.state.tieredSources
        : [];
      const evidenceEntries = Array.isArray(context.state.evidenceEntries)
        ? context.state.evidenceEntries
        : [];
      const hasGroundedEvidence =
        tieredSources.length > 0 && evidenceEntries.length > 0;
      const finalText = hasGroundedEvidence
        ? context.state.finalText || context.state.draft || ""
        : buildNoGroundedSourcesReport({
            query: context.query,
            sourceMesh: context.state.sourceMesh,
            partialFailures: context.run.partialFailures,
            tieredSources,
            evidenceEntries,
            configuredProviders: context.availableSearchProviders,
          });
      ...
      const hasCitedSources = tieredSources.some((source) =>
        citedNumbers.includes(Number(source.citationIndex)),
      );
      const resultSources = hasGroundedEvidence
        ? selectDeliverySources(tieredSources, citedNumbers)
        : [];
      const sourceSelectionMode = hasGroundedEvidence
        ? hasCitedSources
          ? "cited"
          : "fallback"
        : "no_grounded_sources";
      const decision =
        context.state.decisionLayer ||
        buildDecisionLayer({
          plan,
          convergence: context.state.convergence,
          safety: {
      ...
      context.run.result = {
        ...
        slides: hasGroundedEvidence
          ...
          : "",
        datasetCsv: hasGroundedEvidence
          ? buildDatasetCsv({
              extraction: context.state.extraction,
            })
          : "",
        ...
        verifierSummary: context.state.verifierSummary,
        decision: hasGroundedEvidence ? decision : null,
        sourceSelection: {
          mode: sourceSelectionMode,
          citedNumbers,
          totalTieredSources: tieredSources.length,
        },
      };
...
const runNodeWithRecovery = async (context, node) => {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      context.run.completedNodes = unique([
        ...(context.run.completedNodes || []),
        node.id,
      ]);
      context.run.state = context.state;
      ...
      if (
        RETRYABLE_NODE_FAILURES.has(failure.failureType) &&
        attempt < maxAttempts
      ) {
        await emitRuntimeEvent(context, "warning", {
          nodeId: node.id,
      ...
      if (failure.failureType === "partial") {
        context.run.partialFailures = [
          ...(Array.isArray(context.run.partialFailures)
            ? context.run.partialFailures
            : []),
          {
            nodeId: node.id,
...
const executeResearchRuntimePass = async (context, resumeStartNode = "") => {
  ...
  for (const batch of batches) {
    ...
    for (const node of batch) {
      const alreadyCompleted = (context.run.completedNodes || []).includes(
        node.id,
      );
      if (!shouldExecute) {
        if (alreadyCompleted && node.id !== resumeStartNode) continue;
        shouldExecute = true;
      }
...
const resumeResearch = async ({
  ...
  if (!existingRun) {
    throw new ResearchRuntimeError("Research run not found.", {
  ...
  const context = await buildRuntimeContext({
    ...
    controls: [
      ...(Array.isArray(existingRun.controls) ? existingRun.controls : []),
      ...resolveControls(controls),
    ],
    depthPreference: depthPreference || existingRun.plan?.depthPreference || "",
    forcedOutputMode:
      forcedOutputMode || existingRun.plan?.outputMode?.id || "",
    refinementBudget:
      refinementBudget || existingRun.plan?.refinementBudget || 3,
    maxQueries:
      maxQueries ||
      existingRun.requestOptions?.maxQueries ||
      existingRun.plan?.searchQueries?.length ||
      6,
    searchProviderKeys,
    researchProvider,
    researchModel,
  ...
  context.state =
    existingRun.state && typeof existingRun.state === "object"
      ? { ...existingRun.state }
      : {};
  context.run.completedNodes = Array.isArray(existingRun.completedNodes)
    ? existingRun.completedNodes
    : [];
  ...
  const resumeStartNode = compareRestartNodes(
    checkpointResumeNode,
    overrideRestartNode,
  );
