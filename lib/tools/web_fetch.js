...
const renderHtmlAsMarkdown = (html, baseUrl) => {
  ...
  const outboundLinks = collectUniqueLinks(
    extractPageLinks(readableHtml, baseUrl),
  );
  ...
  const markdown = stripHtml(
    withMarkdownLinks
      ...
      .replace(
        /<code\b[^>]*>([\s\S]*?)<\/code>/gi,
        (_, inner) => `\`${normalizeText(stripHtml(inner))}\``,
      )
      ...
      .replace(/<br\s*\/?>/gi, "\n")
      .replace(
        /<\/?(?:p|div|section|article|main|header|footer|aside|nav|ul|ol|table|tr|td|th)\b[^>]*>/gi,
        "\n",
      ),
  )
    .replace(/[ \t]+\n/g, "\n")
...
const fetchWithJina = async (url, format, maxChars, signal) => {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Jina returned HTTP ${response.status}${detail ? `: ${detail}` : ""}`,
    );
  }
...
const fetchDirectly = async (
  ...
  if (!response.ok) {
    const detail = await readErrorDetail(response);
    throw new Error(
      `Direct fetch returned HTTP ${response.status}${detail ? `: ${detail}` : ""}`,
    );
  }
  ...
  const readable = isHtml
    ? renderHtmlAsMarkdown(rawText, normalized)
    : {
  ...
  const plainText = isHtml
    ? normalizeText(stripHtml(readable.readableHtml || rawText))
  ...
  return {
    ...
    via: "direct",
    outboundLinks: isHtml
      ? readable.outboundLinks
      : extractLinksFromMarkdown(content, normalized),
  };
};
...
const definition = {
  ...
  function: {
    ...
    strict: true,
    description:
      "Fetches a webpage and returns its main content as clean Markdown. Handles JavaScript-heavy pages, paywalls, and anti-bot measures. Use for articles, documentation, research papers, or any URL needing content extraction.",
    parameters: {
      type: "object",
      properties: {
        url: {
          type: "string",
          description:
            "The exact HTTP/HTTPS URL to fetch. Must be publicly accessible.",
        },
        format: {
          type: "string",
          enum: ["markdown", "text", "html"],
          description:
            "Output format. 'markdown' (default): clean MD with headings/links. 'text': plain text only. 'html': raw HTML.",
        },
        max_chars: {
          type: "number",
          description:
            "Maximum characters to return. Default: 20000. Max: 80000. Use lower for summaries, higher for full articles.",
        },
        extract_links: {
          type: "boolean",
          description:
            "If true, includes a summary of outbound links at the end.",
        },
      },
      required: ["url"],
...
const handler = async (args) => {
  ...
  const maxChars = Math.min(
    Number(args.max_chars) || DEFAULT_MAX_CHARS,
    HARD_MAX_CHARS,
  );
  ...
  const fullLength = Number.isFinite(Number(result.fullLength))
    ? Number(result.fullLength)
    : content.length;
