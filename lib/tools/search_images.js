/**
 * Image Search Tool
 * Searches the web for relevant images and returns URLs with snippets
 */
...
const definition = {
  ...
  function: {
    ...
    strict: true,
    description:
      "Searches the web for relevant images and returns up to 10 results with URLs and snippets. Use when visuals genuinely help (people, places, products, diagrams).",
    parameters: {
      type: "object",
      properties: {
        query: {
          type: "string",
          description:
            "Image search query. Include details like subject, style, angle.",
        },
        count: {
          type: "integer",
          minimum: 1,
...
/**
 * Image search handler function
 * @param {object} args - Search arguments
 * @param {string} args.query - Search query
 * @param {number} [args.count] - Number of results (1-10)
...
const handler = async (args) => {
  ...
  try {
    ...
    const res = await fetch(searchUrl.toString(), {
      ...
      headers: { "User-Agent": "NubAgent/1.0" },
    });
    clearTimeout(timer);
