...
const RESULT_LINK_RE =
  /<a\b[^>]*class=(?:"[^"]*\b(?:result__a|result-link)\b[^"]*"|'[^']*\b(?:result__a|result-link)\b[^']*')[^>]*href=(?:"([^"]+)"|'([^']+)')[^>]*>([\s\S]*?)<\/a>/gi;
const RESULT_SNIPPET_RE =
  /<(?:a|div|span)\b[^>]*class=(?:"[^"]*\b(?:result__snippet|result-snippet)\b[^"]*"|'[^']*\b(?:result__snippet|result-snippet)\b[^']*')[^>]*>([\s\S]*?)<\/(?:a|div|span)>/i;
const GOOGLE_ONLY_OPERATORS_RE =
  /\b(intitle:|inurl:|intext:|before:|after:|filetype:(?!pdf))/i;
const ANY_OPERATOR_RE =
  /\b(site:|filetype:|intitle:|inurl:|intext:|before:|after:)/i;
...
const decodeDuckDuckGoUrl = (value) => {
  ...
  try {
    ...
    if (redirected) {
      return (
        normalizeUrl(decodeURIComponent(redirected)) ||
...
const needsGoogle = (query) =>
  GOOGLE_ONLY_OPERATORS_RE.test(String(query || ""));

const getTavilyApiKey = () =>
  getRotatingApiKey("tavily", "TAVILY_API_KEYS", "TAVILY_API_KEY");
const getSerperApiKeys = () =>
  getApiKeysFromEnv("SERPER_API_KEYS", "SERPER_API_KEY");
const getSerperApiKey = () =>
  getRotatingApiKey("serper", "SERPER_API_KEYS", "SERPER_API_KEY");
const getJinaApiKeys = () => getApiKeysFromEnv("JINA_API_KEYS", "JINA_API_KEY");
const getJinaApiKey = () =>
  getRotatingApiKey("jina", "JINA_API_KEYS", "JINA_API_KEY");
const getBraveApiKeys = () =>
  getApiKeysFromEnv("BRAVE_API_KEYS", "BRAVE_API_KEY");
const getBraveApiKey = () =>
  getRotatingApiKey("brave", "BRAVE_API_KEYS", "BRAVE_API_KEY");
const resolveProviderApiKey = (explicitValue, fallback) =>
  normalizeText(explicitValue) || fallback();
...
const parseDuckDuckGoResults = (html, limit = MAX_RESULTS) => {
  ...
  while (
    (match = RESULT_LINK_RE.exec(String(html || ""))) !== null &&
    ...
    const description = cleanHtmlText(
      snippetWindow.match(RESULT_SNIPPET_RE)?.[1] || "",
    );
...
const fetchDuckDuckGoResults = async (query, limit = MAX_RESULTS) =>
  withTimeout(async (signal) => {
    ...
    if (
      /Unfortunately, bots use DuckDuckGo too\./i.test(html) ||
...
const fetchTavilyResults = async (
  query,
  limit = MAX_RESULTS,
  explicitApiKey = "",
) => {
  ...
  return withTimeout(async (signal) => {
    ...
    if (data?.answerBox?.answer || data?.answerBox?.snippet) {
      results.push({
        ...
        url:
          normalizeUrl(data.answerBox.link) ||
          String(data.answerBox.link || "").trim(),
...
const mergeResultEntries = (existing, next) => {
  ...
  const providers = [
    ...new Set([...existingProviders, ...nextProviders].filter(Boolean)),
  ];
  const existingDescriptionLength = normalizeText(
  ...
  return {
    ...
    ...next,
    title:
      normalizeText(next?.title || "").length >
      normalizeText(existing?.title || "").length
        ? next.title
        : existing.title,
    description:
      nextDescriptionLength > existingDescriptionLength
        ? next.description
        : existing.description,
    ...
    score:
      Math.max(
        getNumericScore(existing?.score),
        getNumericScore(next?.score),
      ) || undefined,
    source:
      providers.length > 1
        ? "multi"
        : providers[0] || existing?.source || next?.source || "",
    providers,
  ...
  for (const source of sources) {
    for (const item of Array.isArray(source) ? source : []) {
      ...
      const normalizedItem =
        canonicalUrl === item.url ? item : { ...item, url: canonicalUrl };
      ...
      const mergeKey =
        ...
        canonicalUrl;
      const merged = mergeResultEntries(
        mergedByKey.get(mergeKey),
        normalizedItem,
      );
      merged.url =
        pickPreferredUrl(mergedByKey.get(mergeKey)?.url, normalizedItem.url) ||
        merged.url;
      merged.source =
        merged.providers.length > 1
          ? "multi"
          : merged.providers[0] || merged.source || "";
...
const rankMergedResults = (query, items = [], limit = MAX_RESULTS) => {
  ...
  const scored = list.map((item, position) => {
    ...
    const providerScore = Math.max(
      0,
      Math.min(1.5, getNumericScore(item?.score)),
    );
    const descriptionBoost = Math.min(
      0.35,
      normalizeText(item?.description || "").length / 320,
    );
    const recency =
      Math.max(0, (list.length - position) / Math.max(list.length, 1)) * 0.2;
    ...
    parameters: {
      ...
      properties: {
        query: {
          type: "string",
          description:
            "The search query. Include timezone/location for time-sensitive queries.",
        },
        maxResults: {
          type: "number",
          description:
...
const handler = async (args) => {
  ...
  const maxResults = Math.max(
    1,
    Math.min(Number(args.maxResults) || MAX_RESULTS, MAX_RESULTS),
  );
  const providerApiKeys =
    args?.providerApiKeys && typeof args.providerApiKeys === "object"
  ...
  const hasSerper =
    Boolean(normalizeText(providerApiKeys.serper)) ||
    getSerperApiKeys().length > 0;
  const hasBrave =
    Boolean(normalizeText(providerApiKeys.brave)) ||
    getBraveApiKeys().length > 0;
  const hasJina =
    Boolean(normalizeText(providerApiKeys.jina)) || getJinaApiKeys().length > 0;
  ...
  if (requiresGoogle && hasSerper) {
    ...
    return formatResults(
      ...
      rankMergedResults(
        query,
        mergeResults(serper.status === "fulfilled" ? serper.value : []),
        maxResults,
      ),
      searches,
    );
  ...
  if (hasJina) {
    promises.push(
      fetchJinaResults(query, Math.min(maxResults, 10), providerApiKeys.jina),
    );
  }

  if (hasBrave) {
    promises.push(
      fetchBraveResults(query, Math.min(maxResults, 10), providerApiKeys.brave),
    );
  }
