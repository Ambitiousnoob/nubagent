...
const fetchMetadata = async (url, signal) => {
  ...
  return {
    ...
    sizeBytes: res.headers.get("content-length")
...
const definition = {
  ...
  function: {
    ...
    strict: true,
    description:
      "Analyze a specific image URL and describe what is in it, including text when available. Use this for screenshots, charts, memes, product photos, or any image the user wants inspected.",
    parameters: {
      type: "object",
      properties: {
    ...
    const metadata =
      metadataResult.status === "fulfilled"
        ...
        : {
            ...
            reachable: false,
            metadataError:
              metadataResult.reason?.message || "metadata fetch failed",
          };
    ...
    const analysis =
      analysisResult.status === "fulfilled"
        ...
        : {
            ...
            summary: "",
            analysisError:
              analysisResult.reason?.message || "analysis fetch failed",
          };
