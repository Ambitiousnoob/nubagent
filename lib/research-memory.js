...
const STOP_WORDS = new Set([
    ...
    "paper", "papers", "study", "studies", "research", "evidence", "analysis", "query", "question",
]);
const TERM_RE = /[a-z0-9_/-]{3,}/g;
const VALID_MEMORY_LAYERS = new Set(["episode", "concept", "relation", "artifact", "abstraction", "postmortem"]);
const VALID_CONTROL_STATUSES = new Set(["pending", "claimed", "applied", "ignored"]);
...
const serializeJson = (value, fallback = {}) => JSON.stringify(
...
const getRetentionPolicy = (layer) => RETENTION_BY_LAYER[normalizeLayer(layer)] || RETENTION_BY_LAYER.episode;
...
const buildFreshnessScore = (layer, updatedAt) => {
    ...
    return Math.exp((-Math.log(2) * ageHours) / Math.max(1, policy.halfLifeHours || (24 * 30)));
};
...
const buildRelationDigest = (relations = []) => normalizeRelations(relations)
    .map((relation) => [relation.type, relation.target, relation.evidence].filter(Boolean).join(" "))
    .join(" ");
...
const normalizeArtifacts = (artifacts = [], limit = RELATION_LIMIT) => (
    flattenValues(artifacts)
        .map((artifact) => {
            ...
            return {
                type: normalizeText(artifact.type || artifact.kind || "artifact").replace(/\s+/g, "_").toLowerCase(),
                label,
                value: truncateText(normalizeText(artifact.value || artifact.url || artifact.note || ""), 320),
            };
        })
        .filter(Boolean)
        .slice(0, limit)
...
const buildArtifactDigest = (artifacts = []) => normalizeArtifacts(artifacts)
    .map((artifact) => [artifact.type, artifact.label, artifact.value].filter(Boolean).join(" "))
    .join(" ");

const buildCompactJson = (value, max = 1200) => truncateText(JSON.stringify(value ?? null, null, 2), max);

const matchConceptLabels = (value = "", labels = [], limit = 8) => {
    const normalized = normalizeText(value).toLowerCase();
    if (!normalized) return [];
    return labels.filter((label) => normalized.includes(label.toLowerCase())).slice(0, limit);
};
...
const normalizeMemoryMetadata = (metadata = {}, entry = {}) => {
    ...
    return {
        ...
        patternType: truncateText(normalizeText(source.patternType || source.abstractionKind || ""), 96),
        ...
        promptPatches: normalizeStringArray(source.promptPatches || source.prompt_patches_applied || [], 12),
        relations,
        artifactType: truncateText(normalizeText(source.artifactType || source.kind || ""), 96),
        sourceConcept: truncateText(normalizeText(source.sourceConcept || source.source || ""), 160),
        targetConcept: truncateText(normalizeText(source.targetConcept || source.target || ""), 160),
        relationType: truncateText(normalizeIdentifier(source.relationType || source.edgeType || source.type || "", 48), 48),
        confidence: clampScore(source.confidence, 0),
        evidenceWeight: clampScore(source.evidenceWeight ?? source.evidence_weight, 0),
        contradictionScore: clampScore(source.contradictionScore ?? source.contradiction_score, 0),
...
const summarizeStructuredCollection = (values = [], limit = 5) => (
    flattenValues(values)
        .map((value) => {
            ...
            const label = normalizeText(value.label || value.name || value.title || value.topic || value.id || "");
            const status = normalizeText(value.status || value.signal || value.trend || value.direction || "");
            const numeric = [value.score, value.weight, value.count, value.velocity, value.value]
                .find((candidate) => candidate !== undefined && candidate !== null && candidate !== "");
            const summary = normalizeText(value.summary || value.description || value.note || value.reason || "");
            return [label, status, numeric !== undefined ? String(numeric) : "", summary]
                .filter(Boolean)
                .join(" | ");
        })
        .filter(Boolean)
...
const inferConceptLabels = (structured = {}) => {
    ...
    for (const concept of Array.isArray(structured.concepts) ? structured.concepts : []) {
        const label = normalizeText(concept?.label || concept?.name);
        if (!label) continue;
        labels.set(label.toLowerCase(), label);
    }

    for (const claim of Array.isArray(structured.epistemicClaims) ? structured.epistemicClaims : []) {
        for (const concept of normalizeStringArray(claim?.concepts || claim?.relatedConcepts || [], 12)) {
...
const buildGraphArtifacts = (payload = {}) => {
    ...
    const labelLookup = new Map(conceptLabels.map((label) => [label.toLowerCase(), label]));
    const edges = [];
    ...
    for (const concept of concepts) {
        ...
        for (const relation of relations) {
            const canonicalTarget = labelLookup.get(relation.target.toLowerCase()) || relation.target;
            edges.push({
                source,
    ...
    for (const claim of Array.isArray(structured.epistemicClaims) ? structured.epistemicClaims : []) {
        ...
        if (!text) continue;
        const matchedLabels = conceptLabels.filter((label) => text.toLowerCase().includes(label.toLowerCase()));
    ...
    for (const edge of dedupedEdges) {
        ...
        centrality.set(edge.source, (centrality.get(edge.source) || 0) + (edge.weight || 0.5));
        centrality.set(edge.target, (centrality.get(edge.target) || 0) + (edge.weight || 0.5));
    }
...
const buildArtifactIndex = (payload = {}) => {
    ...
    const structured = payload.structuredData || {};
    const checkpoints = payload.checkpoints && typeof payload.checkpoints === "object" ? payload.checkpoints : {};
    ...
    return normalizeArtifacts([
        { type: "domain", label: normalizeText(plan.domain?.label || plan.domain?.id) },
        { type: "scope", label: normalizeText(plan.scope?.label || plan.scope?.id) },
        { type: "output_mode", label: normalizeText(plan.outputMode?.label || plan.outputMode?.id) },
        ...(Array.isArray(structured.paperInventory) ? structured.paperInventory : []).slice(0, 8).map((source) => ({
            type: "source",
            label: normalizeText(source?.title || source?.url),
            value: normalizeText(source?.url || source?.providerLabel || source?.provider),
        })),
        ...(Array.isArray(structured.repositories) ? structured.repositories : []).slice(0, 6).map((repo) => ({
            type: "repository",
            label: normalizeText(repo?.url || repo?.name),
            value: normalizeText(repo?.notes || ""),
        })),
        ...(Array.isArray(structured.supplementary) ? structured.supplementary : []).slice(0, 4).map((item) => ({
            type: "supplementary",
            label: normalizeText(item?.title || item?.url),
            value: normalizeText(item?.url || item?.excerpt || ""),
        })),
        ...(Array.isArray(structured.concepts) ? structured.concepts : []).slice(0, 8).map((concept) => ({
            type: "concept",
            label: normalizeText(concept?.label || concept?.name),
            value: normalizeText(concept?.canonicalId || concept?.ontology || ""),
        })),
        ...Object.keys(checkpoints).slice(0, 8).map((checkpointId) => ({
            type: "checkpoint",
            label: checkpointId,
        })),
        ...Object.keys(outputs).filter((key) => normalizeText(outputs[key])).slice(0, 8).map((key) => ({
            type: "export",
            label: key,
        })),
...
const scoreMemoryEntry = (queryTerms, row = {}) => {
    ...
    const haystack = [row.title, row.content, row.searchText].filter(Boolean).join(" ");
    ...
    const graphTerms = buildTermSet([
        ...
        ...(Array.isArray(metadata.relatedConcepts) ? metadata.relatedConcepts : []),
    ...
    const coverageBoost = (overlap + (graphOverlap * 0.75)) / Math.max(1, queryTerms.size);
    const relationBoost = Math.min(0.45, normalizeRelations(metadata.relations || [], RELATION_LIMIT).length * 0.05);
    const artifactBoost = Math.min(0.28, normalizeArtifacts(metadata.artifacts || [], RELATION_LIMIT).length * 0.04);
    const queryBoost = metadata.query && normalizeText(metadata.query).toLowerCase().includes([...queryTerms][0] || "")
        ? 0.18
        : 0;
    const exactTitleBoost = [...queryTerms].some((term) => normalizeText(row.title).toLowerCase().includes(term)) ? 0.22 : 0;
    const confidenceBoost = clampScore(metadata.confidence ?? metadata.evidenceWeight, 0) * 0.18;
    const stalenessPenalty = ageHours > ((getRetentionPolicy(row.layer).halfLifeHours || 720) * 2)
        ? 0.08
        : 0;
    const layerBoost = row.layer === "abstraction"
        ? 0.42
        : row.layer === "concept"
            ? 0.22
            : row.layer === "postmortem"
                ? 0.16
                : 0;
    const contradictionPenalty = clampScore(metadata.contradictionScore, 0) * 0.16;
    return overlap + (graphOverlap * 0.7) + coverageBoost + freshness + layerBoost + relationBoost + artifactBoost + queryBoost + exactTitleBoost + confidenceBoost - contradictionPenalty - stalenessPenalty;
};

const normalizeMemoryEntry = (entry = {}) => {
    const layer = normalizeLayer(entry.layer);
    const title = truncateText(normalizeText(entry.title || entry.name || ""), 320);
    const content = truncateText(normalizeText(entry.content || entry.summary || ""), MEMORY_TEXT_LIMIT);
    if (!content) return null;

    const metadata = normalizeMemoryMetadata(entry.metadata, { layer, title, content });
    ...
    const searchText = truncateText(
        [
            ...
            Array.isArray(metadata.relatedConcepts) ? metadata.relatedConcepts.join(" ") : "",
    ...
    const baseKey = normalizeText(entry.entryKey || entry.id || title || content.slice(0, 180));
    return {
        layer,
        ...
        metadata: {
            ...
            artifacts,
            relations,
            relatedConcepts: normalizeStringArray(metadata.relatedConcepts || metadata.concepts || [], 24),
...
const normalizeRunRecord = (record = {}) => {
    ...
    return {
        ...
        status: normalizeText(record.status || payload.status || "completed").slice(0, 32) || "completed",
        query: truncateText(normalizeText(record.query || payload.query || ""), 2000),
        title: truncateText(normalizeText(record.title || payload.title || payload.heading || payload.query || ""), 320),
        domain: truncateText(normalizeText(record.domain || payload.plan?.domain?.id || payload.researchMeta?.domain?.id || ""), 96),
        outputMode: truncateText(normalizeText(record.outputMode || payload.plan?.outputMode?.id || payload.researchMeta?.outputMode?.id || ""), 96),
        payload,
    };
};
...
const normalizeControlRecord = (runId, control = {}) => {
    ...
    return {
        ...
        type,
        payload: control && typeof control === "object" ? control : { value: control },
    };
};
...
const resolveRunMemoryPayload = (run = {}) => {
    const payload = run.payload && typeof run.payload === "object" && !Array.isArray(run.payload)
        ? run.payload
        : asObject(run);
    const plan = asObject(payload.plan || run.plan);
    const state = asObject(payload.state || run.state);
    const extraction = asObject(payload.extraction || state.extraction || run.extraction);
    ...
    const structured = {
        paperInventory: pickArray(structuredSource.paperInventory, pickArray(state.tieredSources)),
        deepComprehension: pickArray(structuredSource.deepComprehension, pickArray(state.evidenceEntries)),
        claims: pickArray(structuredSource.claims, pickArray(extraction.claims)),
        repositories: pickArray(structuredSource.repositories, pickArray(extraction.repositories)),
        supplementary: pickArray(structuredSource.supplementary, pickArray(extraction.supplementary)),
        concepts: pickArray(structuredSource.concepts, pickArray(extraction.concepts)),
        abstractions: pickArray(structuredSource.abstractions, pickArray(extraction.abstractions)),
        evidencePyramid: pickArray(structuredSource.evidencePyramid, pickArray(extraction.evidencePyramid)),
        metaAnalysis: structuredSource.metaAnalysis || extraction.metaAnalysis || null,
        statisticalVerification: structuredSource.statisticalVerification || extraction.statisticalVerification || null,
        citationIntelligence: structuredSource.citationIntelligence || state.sourceMesh?.citationIntelligence || null,
        authorNetwork: structuredSource.authorNetwork || state.sourceMesh?.authorNetwork || null,
        temporalTrends: structuredSource.temporalTrends || state.sourceMesh?.temporalTrends || null,
        epistemicClaims: pickArray(
            structuredSource.epistemicClaims,
            pickArray(state.claimLedger, pickArray(final.claimLedger)),
        ),
        verifierSummary: structuredSource.verifierSummary || payload.verifierSummary || run.verifierSummary || state.verifierSummary || final.verifierSummary || null,
        decision: structuredSource.decision || result.decision || state.decisionLayer || final.decision || null,
        safety: structuredSource.safety || payload.safety || run.safety || extraction.safety || state.safety || null,
        tribunal: structuredSource.tribunal || payload.tribunal || run.tribunal || state.tribunal || final.tribunal || null,
        convergence: structuredSource.convergence || payload.convergence || run.convergence || state.convergence || final.convergence || null,
        postmortem: structuredSource.postmortem || payload.postmortem || run.postmortem || state.postmortem || null,
    };
    ...
    return {
        ...
        title: normalizeText(payload.title || run.title || result.heading || final.heading || payload.query || run.query),
        domain: normalizeText(plan.domain?.label || plan.domain?.id || researchMeta.domain?.label || researchMeta.domain?.id || run.domain?.label || run.domain?.id || run.domain || ""),
        domainId: normalizeText(plan.domain?.id || researchMeta.domain?.id || run.domain?.id || run.domain || ""),
        scope: normalizeText(plan.scope?.label || plan.scope?.id || researchMeta.scope?.label || researchMeta.scope?.id || run.scope?.label || run.scope?.id || run.scope || ""),
        scopeId: normalizeText(plan.scope?.id || researchMeta.scope?.id || run.scope?.id || run.scope || ""),
        outputMode: normalizeText(plan.outputMode?.label || plan.outputMode?.id || researchMeta.outputMode?.label || researchMeta.outputMode?.id || run.outputMode?.label || run.outputMode?.id || run.outputMode || ""),
        outputModeId: normalizeText(plan.outputMode?.id || researchMeta.outputMode?.id || run.outputMode?.id || run.outputMode || ""),
        ...
        heading: normalizeText(final.heading || result.heading || payload.title || run.title || payload.query || run.query),
        answer: normalizeText(final.answer || final.markdown || result.finalText || result.answer || ""),
    };
};
...
const hydrateRunRecord = (row = {}) => {
    ...
    const runId = normalizeText(row.run_id || row.runId || payload.runId || payload.id);
    if (!runId) return null;
    ...
    return {
        ...
        runId,
        scopeKey: normalizeScopeKey(row.scope_key || row.scopeKey || payload.scopeKey || ""),
        status: normalizeText(row.status || payload.status || "completed") || "completed",
        ...
        payload,
        createdAt: coerceUpdatedAt(row.created_at || row.createdAt || payload.createdAt),
        updatedAt: coerceUpdatedAt(row.updated_at || row.updatedAt || payload.updatedAt),
    };
};
...
const buildMemoryGraphSnapshot = (results = []) => {
    ...
    const addNode = (label, kind = "concept", weight = 1) => {
        ...
        const existing = nodeMap.get(key) || { label: normalized, kind, weight: 0, mentions: 0 };
    ...
    const addEdge = (source, relation = {}) => {
        ...
        if (!target) return;
        const type = normalizeIdentifier(relation.type || "related_to", 48) || "related_to";
        ...
        existing.weight = Number((existing.weight + clampScore(relation.weight, 0.5)).toFixed(2));
        if (!existing.evidence) existing.evidence = normalizeText(relation.evidence || "");
    ...
    for (const row of Array.isArray(results) ? results : []) {
        ...
        const metadata = normalizeMemoryMetadata(row?.metadata, row);
        if (layer === "relation" && metadata.sourceConcept && metadata.targetConcept) {
        ...
        if (title) addNode(title, layer === "abstraction" ? "abstraction" : layer === "postmortem" ? "postmortem" : "concept", 1.1);
    ...
    return {
        nodes,
        edges,
        centralConcepts: nodes.filter((node) => node.kind === "concept").slice(0, 12).map((node) => node.label),
    };
};
...
const buildResearchMemoryEntriesFromRun = (run = {}) => {
    ...
    const final = {
        ...(view.final || {}),
        heading: view.heading || view.final?.heading || view.title || view.query || "Research run",
    ...
    const selectArtifacts = (types = [], limit = 8) => artifactIndex
    ...
    const buildRelationMetadata = (relatedConcepts = [], limit = 6) => {
        ...
        return graphArtifacts.edges
            .filter((edge) => normalizedConcepts.includes(edge.source) || normalizedConcepts.includes(edge.target))
            .slice(0, limit)
            .map((edge) => ({
                type: edge.type,
                target: edge.target,
    ...
    for (const concept of Array.isArray(structured.concepts) ? structured.concepts : []) {
        const label = normalizeText(concept?.label || concept?.name);
        if (!label) continue;
        const relations = graphArtifacts.relationsBySource.get(label.toLowerCase()) || [];
        entries.push({
            layer: "concept",
            title: label,
            content: normalizeText(
                ...
                || [
                    ...
                    concept?.ontology,
                    concept?.canonicalId,
                    relations.length ? `relations=${relations.map((relation) => `${relation.type}:${relation.target}`).join(", ")}` : "",
                ].filter(Boolean).join(" | "),
            ),
            metadata: {
                ...baseMetadata,
                ...
                concepts: [label],
                artifacts: artifactIndex.filter((artifact) => ["concept", "source", "repository"].includes(artifact.type)).slice(0, 8),
                relatedConcepts: relations.map((relation) => relation.target),
                relations,
                aliases: normalizeStringArray(concept?.aliases || concept?.synonyms || [], 12),
    ...
    for (const claim of Array.isArray(structured.epistemicClaims) ? structured.epistemicClaims : []) {
        ...
        entries.push({
            ...
            metadata: {
                ...baseMetadata,
                claims: [text],
                artifacts: artifactIndex.filter((artifact) => ["source", "concept", "decision"].includes(artifact.type)).slice(0, 8),
                relatedConcepts,
                relations: relatedConcepts.map((target) => ({
                    type: clampScore(claim?.contradiction_score, 0.2) >= 0.35 ? "contests" : "supports",
                    target,
    ...
    for (const edge of graphArtifacts.edges.slice(0, 28)) {
        entries.push({
            ...
            content: truncateText(edge.evidence || `${edge.source} ${edge.type} ${edge.target}`, MEMORY_TEXT_LIMIT),
            metadata: {
                ...baseMetadata,
    ...
    const artifactCandidates = [
        ...
        structured.metaAnalysis
            ? {
                ...
                content: [
                    ...
                    summarizeStructuredCollection(structured.metaAnalysis.assumption_conflicts || [], 4),
                ].filter(Boolean).join(" | "),
                evidenceWeight: clampScore(
                    structured.metaAnalysis.i_squared !== undefined
                        ? 1 - Math.min(1, Number(structured.metaAnalysis.i_squared || 0) / 100)
                        : 0.6,
                    0.6,
                ),
            }
        ...
        Object.keys(view.safety || {}).length
            ...
            : null,
    ].filter((artifact) => normalizeText(artifact?.title || "") && normalizeText(artifact?.content || ""));

    for (const artifact of artifactCandidates.slice(0, 24)) {
        const relatedConcepts = normalizeStringArray([
            matchConceptLabels(`${artifact.title}\n${artifact.content}`, graphArtifacts.conceptLabels, 10),
        ], 10);
        entries.push({
            layer: "artifact",
    ...
    const derivedAbstractions = [
        structured?.metaAnalysis && typeof structured.metaAnalysis === "object"
            ...
            : null,
        Array.isArray(structured.evidencePyramid) && structured.evidencePyramid.length
            ? {
            ...
            ? {
                text: `Statistical verification: ${summarizeStructuredCollection(
                    [
                        ...
                        ...(Array.isArray(structured.statisticalVerification.assumption_conflicts)
    ...
    const abstractionPatterns = [
        ...(Array.isArray(postmortem.prompt_patches_applied) ? postmortem.prompt_patches_applied : []),
        ...(Array.isArray(postmortem.phases_that_degraded_score) ? postmortem.phases_that_degraded_score : []),
        ...(Array.isArray(structured.abstractions) ? structured.abstractions : []),
        ...derivedAbstractions,
        ...(graphArtifacts.edges.slice(0, 10).map((edge) => `${edge.source} ${edge.type} ${edge.target}`)),
    ...
    for (const patternEntry of abstractionPatterns.slice(0, 12)) {
        const pattern = normalizeText(patternEntry?.text || patternEntry?.summary || patternEntry?.label);
        if (!pattern) continue;
        ...
        entries.push({
            ...
            metadata: {
                ...baseMetadata,
                patternType: normalizeText(patternEntry?.patternType || "")
                ...
                relatedConcepts: graphArtifacts.centralConcepts
                    ...
                    .slice(0, 8),
                artifacts: artifactIndex.filter((artifact) => ["checkpoint", "export", "decision", "domain", "scope"].includes(artifact.type)).slice(0, 8),
                relations: graphArtifacts.edges
                    .filter((edge) => normalizedPattern.includes(edge.source.toLowerCase()) || normalizedPattern.includes(edge.target.toLowerCase()))
                    .slice(0, 6)
                    .map((edge) => ({
                        type: edge.type,
                        target: edge.target,
    ...
    if (Object.keys(postmortem || {}).length) {
        entries.push({
            layer: "postmortem",
            title: truncateText(`Postmortem: ${view.query || view.runId || "run"}`, 240),
            content: truncateText(JSON.stringify(postmortem), MEMORY_TEXT_LIMIT),
            metadata: {
                ...baseMetadata,
                artifacts: artifactIndex.filter((artifact) => artifact.type !== "concept").slice(0, 10),
                promptPatches: Array.isArray(postmortem.prompt_patches_applied) ? postmortem.prompt_patches_applied : [],
...
const pruneResearchMemoryScope = async (scopeKey) => {
    ...
    for (const [layer, retention] of Object.entries(RETENTION_BY_LAYER)) {
        ...
        const cutoff = buildSqlTimestamp(new Date(Date.now() - (maxAgeDays * 24 * 60 * 60 * 1000)));
...
const markResearchControlsStatus = async (controlIds = [], status = "applied") => {
    const normalizedStatus = normalizeControlStatus(status);
    const ids = unique((Array.isArray(controlIds) ? controlIds : [])
...
const markResearchControlsApplied = async (controlIds = []) => markResearchControlsStatus(controlIds, "applied");
const markResearchControlsIgnored = async (controlIds = []) => markResearchControlsStatus(controlIds, "ignored");
...
const searchResearchMemoryDetailed = async (scopeKey, query, options = {}) => {
    ...
    const candidateLimit = clampInteger(options.candidateLimit || options.candidate_limit, 8, 200, RESEARCH_MEMORY_CANDIDATE_LIMIT);
    ...
    const perLayerCandidateLimit = clampInteger(
        ...
        4,
        candidateLimit,
        Math.max(4, Math.ceil(candidateLimit / Math.max(1, effectiveLayers.length))),
    );
    const rows = (
        await Promise.all(effectiveLayers.map(async (layer) => {
    ...
    return {
        ...
        meta: {
            ...
            graph_edges: ranked.reduce((count, row) => count + normalizeRelations(row.metadata?.relations || [], RELATION_LIMIT).length, 0),
        },
    };
};
    ...
    return [
        ...
        ...rows.map((row, index) => (
            [
                ...
                row.content,
                normalizeRelations(row.metadata?.relations || [], MEMORY_CONTEXT_RELATION_LIMIT).length
                    ? `Relations: ${normalizeRelations(row.metadata?.relations || [], MEMORY_CONTEXT_RELATION_LIMIT).map((relation) => `${relation.type}:${relation.target}`).join(", ")}`
                    : "",
                normalizeArtifacts(row.metadata?.artifacts || [], 3).length
                    ? `Artifacts: ${normalizeArtifacts(row.metadata?.artifacts || [], 3).map((artifact) => `${artifact.type}:${artifact.label}`).join(", ")}`
                    : "",
            ].join("\n")
        )),
...
const extractPromptPatches = (rows = []) => {
    ...
    for (const row of Array.isArray(rows) ? rows : []) {
        if (row?.layer === "abstraction") {
            ...
            const metadataPatches = normalizeStringArray(row.metadata?.promptPatches || [], 8);
        ...
        const patches = Array.isArray(parsed?.prompt_patches_applied)
            ...
            : [];
        promptPatches.push(...patches.map((value) => normalizeText(value)).filter(Boolean));
    }
...
const findRelevantResearchContext = async (scopeKey, query, options = {}) => {
    ...
    const search = await searchResearchMemoryDetailed(normalizedScopeKey, normalizedQuery, {
    ...
    return {
        ...
        concepts: results
            ...
            .map((row) => ({
                ...
                relations: normalizeRelations(row.metadata?.relations || [], RELATION_LIMIT),
                relatedConcepts: normalizeStringArray(row.metadata?.relatedConcepts || [], 16),
            })),
        ...
        artifacts: dedupeRows(
            results.flatMap((row) => normalizeArtifacts(row.metadata?.artifacts || [], RELATION_LIMIT).map((artifact) => ({
                layer: row.layer,
...
const formatResearchContext = (memoryContext = {}) => {
    ...
    if (Array.isArray(memoryContext.results)) {
        ...
        if (Array.isArray(memoryContext.graph?.centralConcepts) && memoryContext.graph.centralConcepts.length) {
            blocks.push(`Cross-run concepts: ${memoryContext.graph.centralConcepts.slice(0, 6).join(", ")}`);
        }
        if (Array.isArray(memoryContext.promptPatches) && memoryContext.promptPatches.length) {
            blocks.push(`Carry forward prompt patches:\n${memoryContext.promptPatches.slice(0, 4).map((patch) => `- ${patch}`).join("\n")}`);
        }
...
const buildRunPostmortem = (run = {}) => {
    ...
    const postmortem = payload.postmortem && typeof payload.postmortem === "object"
        ? payload.postmortem
        : {};

    return {
        query_type: normalizeText(postmortem.query_type || plan.scope?.id || plan.scope?.label || ""),
        domain: normalizeText(postmortem.domain || plan.domain?.id || plan.domain?.label || ""),
        phases_that_degraded_score: Array.isArray(postmortem.phases_that_degraded_score)
...
const persistResearchRunArtifacts = async (run = {}) => {
    ...
    const scopeKey = normalizeScopeKey(normalized.scopeKey || normalized.payload.scopeKey || "");
    if (scopeKey) {
...
const indexResearchRun = async (scopeKey, run = {}) => saveResearchRun({
    ...(run && typeof run === "object" ? run : {}),
    scopeKey: normalizeScopeKey(scopeKey || run?.scopeKey || ""),
});

const updateResearchMemoryFromRun = async (scopeKey, run = {}) => persistResearchRunArtifacts({
    ...(run && typeof run === "object" ? run : {}),
    scopeKey: normalizeScopeKey(scopeKey || run?.scopeKey || ""),
});
