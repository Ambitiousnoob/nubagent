...
const estimatePayloadTokens = (payload = {}) => {
  ...
  const toolTokens =
    Array.isArray(payload.tools) && payload.tools.length
      ...
      : 0;
  const maxTokens = clampCompletionTokens(
    payload.max_tokens ?? payload.maxTokens,
  );
  return messageTokens + toolTokens + maxTokens;
};

const clampCompletionTokens = (
  value,
  fallback = DEFAULT_MAX_COMPLETION_TOKENS,
) => {
...
class SlidingWindowTokenLimiter {
  constructor({
    ...
    this.tokensPerMinute = Math.max(
      1000,
      safeNumber(tokensPerMinute, DEFAULT_TPM),
    );
    this.remoteHeadroom = Math.max(
      0,
      safeNumber(remoteHeadroom, DEFAULT_REMOTE_HEADROOM),
    );
    this.window = [];
  ...
  prune(now = Date.now()) {
    while (
      this.window.length &&
      this.window[0].timestamp <= now - ONE_MINUTE_MS
    ) {
      this.window.shift();
    }
...
class ExactResponseCache {
  constructor({
    ...
    this.maxEntries = Math.max(
      1,
      safeNumber(maxEntries, DEFAULT_CACHE_MAX_ENTRIES),
    );
    this.store = new Map();
  }
