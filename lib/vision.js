...
const LOCATION_READY_REPLY =
  "I saved your location. You can now ask things like 'cafes near me', 'pharmacies nearby', or 'where am I?'.";
...
export function extractLocationAttachment(event) {
  return (event?.message?.attachments ?? []).find((attachment) => {
    return attachment?.type === "location";
  });
}

export function extractLocationCoordinates(attachment) {
  const coordinates = attachment?.payload?.coordinates;
  const latitude =
    readCoordinateValue(coordinates?.lat) ??
    readCoordinateValue(coordinates?.latitude);
  const longitude =
    readCoordinateValue(coordinates?.long) ??
    readCoordinateValue(coordinates?.longitude);
...
export function buildLocationReadyReply(location) {
  if (
    !Number.isFinite(location?.latitude) ||
    !Number.isFinite(location?.longitude)
  ) {
    return LOCATION_READY_REPLY;
  }
