...
/location - show the latest shared location
...
const PRIVACY_REPLY = `NubAgent stores recent chat turns, the latest image context, the latest shared location, a rolling conversation summary, and saved long-term memory in Postgres.
...
function formatLocationReply(location) {
  if (!location) {
    return "I do not have a saved location for you right now. Share a Messenger location pin and I will use it for Google Maps grounded prompts.";
  }
