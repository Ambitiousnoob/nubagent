...
function buildMessengerApiError(payload, status) {
  const detail =
    payload?.error?.message ||
    payload?.error?.error_user_msg ||
    payload?.message ||
  ...
  const error = new Error(summarizeMessengerError(payload, status));
...
async function postMessenger(body, config) {
  ...
  if (!response.ok) {
    throw buildMessengerApiError(payload, response.status);
  }
...
export async function sendLocationQuickReply(recipientId, text, config) {
  const normalizedText = takeLeadingCharacters(
    normalizeText(text) || "Share your location so I can use it for nearby results.",
    MAX_MESSAGE_LENGTH,
  );
...
export function isRetryableMessengerError(error) {
  ...
  return (
    ...
    retryableStatuses.has(error?.httpStatus) ||
