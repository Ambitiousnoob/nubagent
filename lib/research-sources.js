const { handler: webSearchHandler } = require("../api/tools/web_search");
const {
...
const fetchGenericWebResults = async (query, limit) => {
    const raw = await webSearchHandler({ query, maxResults: limit });
    if (typeof raw !== "string" || raw.startsWith("Error:")) return [];
    let parsed = [];
    try {
...
const searchOpenAlexWorks = async (query, limit) => {
    ...
    url.searchParams.set("mailto", process.env.OPENALEX_EMAIL || "research@nubagent.local");
...
const searchCrossrefWorks = async (query, limit) => {
    ...
    url.searchParams.set("mailto", process.env.CROSSREF_EMAIL || "research@nubagent.local");
...
const searchResearchSources = async ({
    ...
    ].filter(Boolean)).slice(0, plan?.depthPreference === "deep" ? 6 : plan?.depthPreference === "speed" ? 2 : 4);
    const laneLimit = Math.max(4, Math.ceil(limit / Math.max(1, laneQueries.length)) + 2);
    const providerTasks = [
        { name: "web_search", enabled: true, run: (laneQuery, taskLimit) => fetchGenericWebResults(laneQuery, taskLimit) },
    ...
    const settled = await Promise.allSettled(
        laneQueries
            .flatMap((laneQuery, laneIndex) => providerTasks
    ...
    return {
        ...
        searchLanes: laneQueries.map((laneQuery, laneIndex) => ({ laneIndex, query: laneQuery })),
        auditLog: {
            tierCounts,
            quarantined,
            sampleSizeTelemetry,
