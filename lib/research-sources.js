...
const SAMPLE_SIZE_RE =
  /\b(?:n|N)\s*[:=]\s*(\d{1,7})\b|\b(?:sample size|cohort(?: size)?|dataset(?: size)?|trial(?: size)?)\s*[:=]?\s*(\d{1,7})\b|\b(\d{1,7})\s+(participants|patients|subjects|samples|records|observations|images|respondents|users|documents)\b/gi;
const FUNDING_RE =
  /\b(funded by|funding|sponsored by|supported by|grant from|industry[- ]funded|conflict of interest|competing interests?)\b/i;
const PREDATORY_RE =
  /\b(predatory journal|questionable publisher|dubious journal|paper mill|pay[- ]to[- ]publish)\b/i;
const DUAL_USE_RE =
  /\b(biosecurity|surveillance|weapon|weapons|exploit|malware|pathogen|offensive cyber)\b/i;
const NEAR_THRESHOLD_P_RE = /\bp\s*(?:=|<|>)\s*0\.0?(4[5-9]|5[0-5])\b/i;
const POSITIVE_FINDING_RE =
  /\b(significant(?:ly)?|improve(?:d|ment)?|outperform(?:s|ed)?|effective|benefit(?:s)?|higher|increase(?:d)?|reduces? error|state[- ]of[- ]the[- ]art)\b/i;
const NEGATIVE_FINDING_RE =
  /\b(no significant|null result|not effective|fails? to|worse|lower|decrease(?:d)?|did not improve|no improvement|harm(?:ful)?)\b/i;
const MIXED_FINDING_RE =
  /\b(mixed|inconsistent|however|but|conflicting|uncertain|counterevidence)\b/i;
const SCHOLARLY_LANE_RE =
  /\b(scholar\.google\.com|semanticscholar\.org|openalex\.org|site:\.edu|site:edu|arxiv\.org|pubmed|jstor\.org|ssrn\.com|paperswithcode\.com|osf\.io)\b/i;
const SCHOLARLY_PROVIDER_RE =
  /\b(openalex(?:_forward_citation)?|crossref|ieee_xplore|acm|jstor|papers_with_code)\b/i;
...
const getScholarlyRoutingSignals = (plan = {}) => {
  ...
  const scholarlyLanes = unique(
    [
      ...(Array.isArray(matrix.scholarlyDiscoveryLanes)
        ? matrix.scholarlyDiscoveryLanes
        : []),
      ...(Array.isArray(plan?.searchQueries) ? plan.searchQueries : []).filter(
        (value) => SCHOLARLY_LANE_RE.test(normalizeText(value)),
      ),
    ]
      .map((value) => normalizeText(value))
      .filter(Boolean),
  );
  const providerBias = unique(
    (Array.isArray(matrix.scholarlyProviderBias)
      ? matrix.scholarlyProviderBias
      : []
    )
  ...
  return {
    active:
      scholarlyLanes.length > 0 || Boolean(plan?.scholarlyHarvest?.active),
    scholarlyLanes,
    providerBias,
  };
};
...
const getScholarlyBoost = (source = {}, plan = {}) => {
  ...
  const providerBiasHit = providers.some((provider) =>
    routing.providerBias.includes(provider),
  );
  const providerHit = providers.some((provider) =>
    SCHOLARLY_PROVIDER_RE.test(provider),
  );
...
const flattenAuthors = (authors = []) =>
  unique(
    (Array.isArray(authors) ? authors : [])
      .map((author) =>
        normalizeText(
          author?.name || author?.author?.display_name || author?.display_name,
        ),
      )
      .filter(Boolean),
  ).slice(0, 12);
...
const getDomainSampleThreshold = (domain = {}) =>
  DOMAIN_SAMPLE_THRESHOLDS[domain?.id] ||
  DOMAIN_SAMPLE_THRESHOLDS.general_research;

const collectSourceText = (source = {}) =>
  normalizeText(
    [
...
const inferSourceSampleSignals = (source = {}, domain = {}) => {
  ...
  const sampleSizeCandidates = unique(
  ...
  const sampleSizeAdequacy = sampleSize
    ? Number((sampleSize / sampleSizeThreshold).toFixed(2))
    : null;
  return {
    sampleSize,
    ...
    sampleSizeConfidence: sampleSize
      ? Number(
          (metadataSample > 0
            ? 0.9
            : Math.min(0.75, 0.45 + sampleSizeCandidates.length * 0.08)
          ).toFixed(2),
        )
      : 0,
  };
...
const buildSourceSafetySignals = (source = {}) => {
  ...
  if (source?.isRetracted) {
    flags.push({
      ...
      detail: normalizeText(
        source?.retractionReason || "Retraction signal present.",
      ),
    });
  }
  ...
  if (
    PREDATORY_RE.test(text) ||
    Boolean(source?.metadata?.predatory || source?.metadata?.isPredatory)
  ) {
    flags.push({
  ...
  if (
    NEAR_THRESHOLD_P_RE.test(text) ||
    (text.match(/\bp\s*(?:=|<|>)\s*0?\.\d+/gi) || []).length >= 4
  ) {
    flags.push({
      ...
      detail:
        "Borderline p-values or dense significance-only reporting detected.",
    });
  }
...
const normalizeResearchSource = (source = {}) => {
  ...
  const normalized = {
    ...
    providerLabel: normalizeText(
      source.providerLabel || source.provider || source.source || "Research",
    ),
    kind: normalizeText(source.kind || source.type || "web") || "web",
    doi: normalizeText(
      source.doi || extractDoi(source.url) || extractDoi(source.description),
    ),
    authors: flattenAuthors(source.authors),
    publishedAt: normalizeDate(
      source.publishedAt || source.published || source.date,
    ),
    citationCount: Math.max(
      0,
      Number(source.citationCount || source.citedByCount || 0) || 0,
    ),
    ...
    openAccessUrl: canonicalizeSourceUrl(
      source.openAccessUrl || source.fullTextUrl || source.pdfUrl || "",
    ),
...
const runProviderBatch = async ({
        ...
        try {
          ...
          merged.push(
            ...(Array.isArray(results) ? results : []).map((result) =>
              attachLaneMetadata(result, laneQuery, laneIndex),
            ),
          );
        } catch (error) {
          const label = prefix ? `${prefix}:${task.name}` : task.name;
          providerErrors.push(
            `${label} ${normalizeText(error?.message || error || "provider failed")}`.trim(),
          );
        }
      }
    }
  ...
  const settled = await Promise.allSettled(
    laneQueries.flatMap((laneQuery, laneIndex) =>
      tasks.map(async (task) => ({
        name: task.name,
        laneQuery,
        laneIndex,
        results: (await task.run(laneQuery, laneLimit)).map((result) =>
          attachLaneMetadata(result, laneQuery, laneIndex),
        ),
      })),
    ),
  ...
  for (const item of settled) {
    if (item.status === "fulfilled") {
      ...
      providerErrors.push(
        normalizeText(item.reason?.message || item.reason || "provider failed"),
      );
    }
  }
...
const fetchGenericWebResults = async (query, limit, providerApiKeys = {}) => {
  const raw = await webSearchHandler({
    query,
    maxResults: limit,
    providerApiKeys,
  });
...
const parseOpenAlexWork = (work) =>
  normalizeResearchSource({
    title: work?.display_name,
    url:
      work?.primary_location?.landing_page_url || work?.ids?.doi || work?.doi,
    ...
    authors: (Array.isArray(work?.authorships) ? work.authorships : []).map(
      (authorship) => ({
    ...
    metadata: {
      ...
      referencedWorks: Array.isArray(work?.referenced_works)
        ? work.referenced_works.slice(0, 24)
        : [],
    },
  });
  ...
  return (Array.isArray(data?.message?.items) ? data.message.items : [])
    .map((item) =>
      normalizeResearchSource({
        ...
        url:
          Array.isArray(item?.link) && item.link[0]?.URL
            ? item.link[0].URL
            : item?.URL || item?.DOI
              ? `https://doi.org/${item.DOI}`
              : "",
        description: Array.isArray(item?.["container-title"])
  ...
  return items
    .map((item) =>
      normalizeResearchSource({
        ...
        pdfUrl: item?.url_pdf,
        repoUrls: Array.isArray(item?.repositories)
          ? item.repositories.map((repo) => repo?.url)
          : [],
      }),
    )
    .filter(Boolean);
  ...
  return (Array.isArray(data?.articles) ? data.articles : [])
    .map((item) =>
      normalizeResearchSource({
        title: item?.title,
        url:
          item?.html_url || item?.pdf_url || item?.doi
            ? `https://doi.org/${item.doi}`
            : "",
        ...
        authors: Array.isArray(item?.authors?.authors)
          ? item.authors.authors.map((author) => ({
...
const fetchUnpaywallLink = async (source) => {
  ...
  const url = new URL(
    `https://api.unpaywall.org/v2/${encodeURIComponent(source.doi)}`,
  );
  ...
  try {
    ...
    return {
      ...
      pdfUrl: canonicalizeSourceUrl(
        data?.best_oa_location?.url_for_pdf || source.pdfUrl,
      ),
      metadata: {
        ...source.metadata,
...
const applyRetractionSignals = async (sources = []) => {
  ...
  for (const source of sources) {
    ...
    try {
      ...
      const record = Array.isArray(data?.results)
        ? data.results[0]
        : data?.result || data;
      enriched.push({
        ...source,
        isRetracted: Boolean(
          source.isRetracted || record?.retracted || record?.is_retracted,
        ),
        retractionReason: normalizeText(
          record?.reason ||
            record?.retraction_reason ||
            source.retractionReason,
        ),
      });
    } catch {
      enriched.push(source);
  ...
  for (const source of sources) {
    ...
    if (seen.has(key)) {
      ...
      const mergedFlags = uniqueBy(
        [
          ...(Array.isArray(output[index].safetyFlags)
            ? output[index].safetyFlags
            : []),
          ...(Array.isArray(normalized.safetyFlags)
      ...
      output[index] = {
        ...
        providerCount:
          Math.max(1, Number(output[index].providerCount || 1)) + 1,
        repoUrls: unique([
          ...(output[index].repoUrls || []),
          ...(normalized.repoUrls || []),
        ]).slice(0, 6),
        citationCount: Math.max(
          Number(output[index].citationCount || 0),
          Number(normalized.citationCount || 0),
        ),
        authors: unique([
        ...
        retractionReason:
          output[index].retractionReason || normalized.retractionReason,
        ...
        providerLabel: unique([
          output[index].providerLabel,
        ...
        sampleSize:
          Math.max(
            Number(output[index].sampleSize || 0),
            Number(normalized.sampleSize || 0),
          ) || null,
        sampleSizeCandidates: unique([
          ...(output[index].sampleSizeCandidates || []),
          ...(normalized.sampleSizeCandidates || []),
        ])
          .map((value) => Number(value))
          .filter((value) => value > 0)
          .sort((left, right) => right - left)
          .slice(0, 6),
        sampleSizeConfidence: Math.max(
          Number(output[index].sampleSizeConfidence || 0),
          Number(normalized.sampleSizeConfidence || 0),
        ),
        safetyFlags: mergedFlags,
        safetySummary: mergedFlags.map((flag) => flag.id),
        contradictionHint:
          output[index].contradictionHint === "mixed"
            ? "mixed"
...
const buildTemporalTrends = (sources = []) => {
  ...
  return {
    points,
    direction:
      recent > prior ? "emerging" : recent < prior ? "saturating" : "steady",
  };
};
...
const buildCounterevidenceMap = (sources = []) => {
  ...
  return {
    stanceCounts: Object.fromEntries(
      Object.entries(byHint).map(([key, value]) => [key, value.length]),
    ),
    conflictClusters,
  };
};
...
const scoreTieredSources = (query, sources = [], plan = {}) => {
  ...
  return ranked.map((source, index) => {
    ...
    const lexical = scoreTextForTerms(
      [source.title, source.description, source.url].join(" "),
      terms,
    );
    const authority = domainAuthorityBoost(source.url);
    const providerBoost = Math.min(
      0.2,
      (Number(source.providerCount || 1) - 1) * 0.06,
    );
    const citationBoost = Math.min(
      0.24,
      Math.log10((Number(source.citationCount || 0) || 0) + 1) * 0.08,
    );
    const freshnessFactor = applyFieldRecencyDecay(source, plan.domain);
    const retractionPenalty = source.isRetracted ? 0.55 : 0;
    const repoBoost =
      Array.isArray(source.repoUrls) && source.repoUrls.length ? 0.06 : 0;
    const scholarlyBoost = getScholarlyBoost(source, plan);
    const sampleSizePenalty = sampleSignals.sampleSize
      ? clamp(
          (1 - Math.min(1, sampleSignals.sampleSizeAdequacy || 0)) * 0.16,
          0,
          0.16,
        )
      : source.kind === "paper" &&
          ["biomedical", "social_science", "interdisciplinary"].includes(
            plan?.domain?.id,
          )
        ? 0.03
        : 0;
    const sampleSizeBoost =
      sampleSignals.sampleSizeAdequacy && sampleSignals.sampleSizeAdequacy > 1
        ? Math.min(0.08, (sampleSignals.sampleSizeAdequacy - 1) * 0.04)
        : 0;
    const predatoryPenalty = safetySignals.summary.includes(
      "predatory_candidate",
    )
      ? 0.3
      : 0;
    const fundingPenalty =
      safetySignals.summary.includes("funding_conflict") &&
      plan?.scope?.id === "contested_topic"
        ? 0.05
        : 0;
    const manipulationPenalty = safetySignals.summary.includes(
      "statistical_manipulation_candidate",
    )
      ? 0.08
      : 0;
    ...
    const quarantineReason = source.isRetracted
      ...
      : safetySignals.summary.includes("predatory_candidate")
        ? "Predatory venue signal"
        : "";
    const rawScore =
      (normalizedRank * 0.42 +
      ...
      manipulationPenalty -
      sampleSizePenalty;
    const score = quarantineReason
      ? 0
      : Math.max(0, Math.min(1, Number(rawScore.toFixed(2))));
    ...
    const verdictRationale = [
      ...
      sampleSignals.sampleSize
        ? `sample:${sampleSignals.sampleSize}/${sampleSignals.sampleSizeThreshold}`
        : "sample:unknown",
      safetySignals.summary.length
        ? `flags:${safetySignals.summary.join("|")}`
        : "flags:none",
    ...
    return {
      ...
      safetySummary: safetySignals.summary,
      contradictionHint:
        source.contradictionHint || classifySourceStance(source),
      quarantineReason,
...
const searchResearchSources = async ({
  ...
  const laneQueries = unique(
    [
      normalizeText(query),
      ...(Array.isArray(plan?.searchQueries) ? plan.searchQueries : []).map(
        (value) => normalizeText(value),
      ),
      ...scholarlyRouting.scholarlyLanes,
      ...(Array.isArray(plan?.queryMatrix?.versions)
        ? plan.queryMatrix.versions.map((version) => version?.query)
        : []
      ).map((value) => normalizeText(value)),
    ].filter(Boolean),
  ).slice(
    0,
    plan?.depthPreference === "deep"
      ? scholarlyRouting.active
        ? 7
        : 6
      : plan?.depthPreference === "speed"
        ? scholarlyRouting.active
          ? 3
          : 2
        : scholarlyRouting.active
  ...
  const laneLimit = Math.max(
    4,
    Math.ceil(limit / Math.max(1, laneQueries.length)) + 2,
  );
  const providerTasks = [
    {
      name: "web_search",
      enabled: true,
      run: (laneQuery, taskLimit) =>
        fetchGenericWebResults(laneQuery, taskLimit, providerApiKeys),
    },
    {
      name: "openalex",
      enabled: true,
      run: (laneQuery, taskLimit) =>
        searchOpenAlexWorks(laneQuery, Math.min(taskLimit, 16)),
    },
    {
      name: "crossref",
      enabled: true,
      run: (laneQuery, taskLimit) =>
        searchCrossrefWorks(laneQuery, Math.min(taskLimit, 16)),
    },
    {
      name: "github",
      enabled:
        plan?.domain?.id === "cs_ml" ||
        /\b(code|implementation|repo|repository|github|benchmark)\b/i.test(
          query,
        ),
      run: (laneQuery, taskLimit) =>
        searchGitHubRepositories(laneQuery, Math.min(taskLimit, 10)),
    },
    {
      name: "papers_with_code",
      enabled: plan?.domain?.id === "cs_ml",
      run: (laneQuery, taskLimit) =>
        searchPapersWithCode(laneQuery, Math.min(taskLimit, 10)),
    },
    {
      name: "ieee_xplore",
      enabled: Boolean(process.env.IEEE_XPLORE_API_KEY),
      run: (laneQuery, taskLimit) =>
        searchIeeeXplore(laneQuery, Math.min(taskLimit, 12)),
    },
    {
      name: "acm",
      enabled: Boolean(process.env.ACM_SEARCH_API_URL),
      run: (laneQuery, taskLimit) =>
        searchConfiguredScholarlyProvider(
          "ACM",
          laneQuery,
          Math.min(taskLimit, 12),
        ),
    },
    {
      name: "jstor",
      enabled: Boolean(process.env.JSTOR_SEARCH_API_URL),
      run: (laneQuery, taskLimit) =>
        searchConfiguredScholarlyProvider(
          "JSTOR",
          laneQuery,
  ...
  if (!merged.length) {
    const recoveryBatch = await runProviderBatch({
      laneQueries: unique([normalizeText(query), ...laneQueries])
        .filter(Boolean)
        .slice(0, 2),
      providerTasks: enabledProviderTasks.filter((task) =>
        ["openalex", "crossref", "web_search"].includes(task.name),
      ),
  ...
  const oaResolved = await Promise.all(
    deduped.map((source) => fetchUnpaywallLink(source)),
  );
  ...
  const forwardCitationSeeds = initialRanked
    .filter(
      (source) =>
        source.provider.startsWith("openalex") &&
        source.tier !== "discard" &&
        source.citedByApiUrl,
    )
    .slice(0, MAX_FORWARD_CITATION_SEEDS);
  const forwardCitations = await Promise.allSettled(
    forwardCitationSeeds.map((source) =>
      fetchForwardCitationsFromOpenAlex(source, Math.min(6, limit / 2)),
    ),
  );
  ...
  const sourceSet = scoreTieredSources(
    query,
    dedupeSources(
      [...retractionResolved, ...forwardCitationResults],
      limit * 4,
    ),
    plan,
  ).slice(0, limit * 2);
  ...
  const tierCounts = sourceSet.reduce(
    (accumulator, source) => {
  ...
  const safetyTelemetry = {
    fundingConflictCount: sourceSet.filter((source) =>
      source.safetySummary?.includes("funding_conflict"),
    ).length,
    predatoryCandidateCount: sourceSet.filter((source) =>
      source.safetySummary?.includes("predatory_candidate"),
    ).length,
    dualUseCount: sourceSet.filter((source) =>
      source.safetySummary?.includes("dual_use"),
    ).length,
    manipulationCount: sourceSet.filter((source) =>
      source.safetySummary?.includes("statistical_manipulation_candidate"),
    ).length,
    retractedCount: sourceSet.filter((source) =>
      source.safetySummary?.includes("retracted"),
    ).length,
  };
  ...
  return {
    ...
    searchLanes: laneQueries.map((laneQuery, laneIndex) => ({
      laneIndex,
      query: laneQuery,
      laneType: scholarlyRouting.scholarlyLanes.includes(
        normalizeText(laneQuery),
      )
        ? "scholarly"
        : "general",
    })),
