...
const normalizeApiKey = (value) => {
  const key = String(value || "").trim();
  if (!key || key.length < 8 || key.length > 1024) return "";
  // Reject keys with potentially dangerous control characters
  if (/[\u0000-\u001F\u007F]/.test(key)) return "";
  return key;
};
...
const getApiKeyFromRequest = (req) =>
  normalizeApiKey(getHeaderValue(req, API_KEY_HEADER)) ||
  getAuthorizationApiKey(req);
