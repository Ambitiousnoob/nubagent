...
const DEFAULT_THINKING_LEVEL_SETTING = "low";
export const USER_THINKING_LEVEL_OPTIONS = [
  "off",
  "minimal",
  "low",
  "medium",
...
export function normalizeThinkingLevelInput(value) {
  const normalized =
    typeof value === "string" ? value.trim().toLowerCase() : "";
  return THINKING_LEVEL_ALIASES[normalized] || null;
}

export function resolveThinkingLevelSetting(
  value,
  fallback = DEFAULT_THINKING_LEVEL_SETTING,
) {
  return normalizeThinkingLevelInput(value) || fallback;
}
...
export function toGeminiThinkingLevel(
  ...
  const setting = resolveThinkingLevelSetting(value, fallback);
  return GEMINI_THINKING_LEVELS[setting];
}

export function applyThinkingLevelPreference(config, preferredThinkingLevel) {
  const defaultSetting = resolveThinkingLevelSetting(
    config?.geminiThinkingLevelSetting,
    DEFAULT_THINKING_LEVEL_SETTING,
  );
  const effectiveSetting =
    normalizeThinkingLevelInput(preferredThinkingLevel) || defaultSetting;

  return {
    ...config,
    geminiThinkingLevelSetting: effectiveSetting,
    geminiThinkingLevel: GEMINI_THINKING_LEVELS[effectiveSetting],
  };
}
...
export function getRuntimeConfig() {
  ...
  const geminiThinkingLevelSetting = resolveThinkingLevelSetting(
    readEnv("GEMINI_CHAT_THINKING_LEVEL"),
    DEFAULT_THINKING_LEVEL_SETTING,
  );
  ...
  return {
    ...
    googleGeocodingApiKey:
      readEnv("GOOGLE_GEOCODING_API_KEY") || readEnv("GOOGLE_MAPS_API_KEY"),
    ...
    geminiThinkingLevelSetting,
    geminiThinkingLevel: GEMINI_THINKING_LEVELS[geminiThinkingLevelSetting],
    reliabilityRetryLimit: readIntegerEnv("RELIABILITY_RETRY_LIMIT", 2),
    reliabilityRetryBaseMs: readIntegerEnv("RELIABILITY_RETRY_BASE_MS", 350),
    reliabilityRetentionDays: readIntegerEnv("RELIABILITY_RETENTION_DAYS", 14),
    locationCaptureTtlMinutes: readIntegerEnv(
      "LOCATION_CAPTURE_TTL_MINUTES",
      15,
    ),
    healthLookbackHours: readIntegerEnv("HEALTH_LOOKBACK_HOURS", 24),
    memoryMaxItems: readIntegerEnv("MEMORY_MAX_ITEMS", 6),
    memoryMaxChars: readIntegerEnv("MEMORY_MAX_CHARS", 700),
    summaryMaxChars: readIntegerEnv("SUMMARY_MAX_CHARS", 600),
    promptContextMaxChars: readIntegerEnv("PROMPT_CONTEXT_MAX_CHARS", 1400),
...
export function supportsGoogleMapsGrounding(modelName) {
  ...
  return (
    isGemini25TextModel(normalized) || isGemini20FlashTextModel(normalized)
  );
}
...
export function getCachedContentForModel(config, modelName) {
  ...
  return (
    ...
    (configuredModels.length <= 1
      ? config?.geminiCachedContent?.fallback
      : "") ||
    ""
  );
}
