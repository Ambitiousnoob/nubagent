...
const OUTPUT_MODE_RULES = [
  ...
  },
  {
    ...
    patterns: /\b(state of the field|overview|landscape|what do we know)\b/i,
  },
  {
    id: "controversy_map",
...
const SUBAGENT_SPECS = attachSubagentEquipment({
  cognitiveCommandLayer: {
    label: "Cognitive Command Layer",
    scope:
      "intent decomposition, graph compilation, Pareto steering, continuity",
    phaseId: "cognitiveCommandLayer",
  },
  devilsAdvocateDecomposer: {
    label: "DevilsAdvocateDecomposer",
    scope: "counter-hypothesis generation",
    phaseId: "adversarialQueryForge",
  },
  domainDetector: {
    label: "DomainDetector",
    scope: "domain taxonomy and ontology selection",
    phaseId: "adversarialQueryForge",
  },
  queryVersionController: {
    label: "QueryVersionController",
    scope: "auditable query transformations",
    phaseId: "adversarialQueryForge",
  },
  forwardCitationTracer: {
    label: "ForwardCitationTracer",
    scope: "forward citation expansion",
  ...
  authorNetworkMapper: {
    ...
    scope: "author graph and echo-chamber analysis",
    phaseId: "intelligentCrawlerMesh",
  },
  temporalTrendAnalyzer: {
    label: "TemporalTrendAnalyzer",
    scope: "topic acceleration and saturation analysis",
    phaseId: "intelligentCrawlerMesh",
  },
  temporalRelevanceDecay: {
    label: "TemporalRelevanceDecay",
    scope: "field-specific recency weighting",
    phaseId: "tieredEpistemicFilter",
  },
  retractedPaperGuard: {
    label: "RetractedPaperGuard",
    scope: "retraction and quarantine checks",
    phaseId: "tieredEpistemicFilter",
  },
  sampleSizeFilter: {
    label: "SampleSizeFilter",
    scope: "sample-size confidence weighting",
    phaseId: "tieredEpistemicFilter",
  },
  statisticalClaimExtractor: {
    label: "StatisticalClaimExtractor",
    scope: "structured statistical claim extraction",
    phaseId: "deepComprehensionEngine",
  },
  codeRepoAnalyzer: {
    label: "CodeRepoAnalyzer",
    scope: "linked repository reproducibility analysis",
    phaseId: "deepComprehensionEngine",
  },
  supplementaryMaterialParser: {
    label: "SupplementaryMaterialParser",
    scope: "supplementary appendix extraction",
    phaseId: "deepComprehensionEngine",
  },
  conceptEntityLinker: {
    label: "ConceptEntityLinker",
    scope: "concept graph linking",
    phaseId: "deepComprehensionEngine",
  },
  statisticalVerifier: {
    label: "StatisticalVerifier",
    scope: "cross-paper quantitative checks",
    phaseId: "deepComprehensionEngine",
  },
  thesisAgent: {
    label: "ThesisAgent",
    scope: "dominant-view argument",
    phaseId: "dialecticalSynthesisEngine",
  },
  antithesisAgent: {
    label: "AntithesisAgent",
    scope: "counter-view argument",
    phaseId: "dialecticalSynthesisEngine",
  },
  synthesisMediator: {
    label: "SynthesisMediator",
    scope: "uncertainty-aware reconciliation",
    phaseId: "dialecticalSynthesisEngine",
  },
  narrativeArchitect: {
    label: "NarrativeArchitect",
    scope: "mode-specific report compilation",
    phaseId: "dialecticalSynthesisEngine",
  },
  quantitativeSynthesizer: {
    label: "QuantitativeSynthesizer",
    scope: "meta-analysis and heterogeneity synthesis",
    phaseId: "dialecticalSynthesisEngine",
  },
  evidencePyramidBuilder: {
    label: "EvidencePyramidBuilder",
    scope: "evidence weighting by type",
    phaseId: "dialecticalSynthesisEngine",
  },
  evolvingNarrativeTracker: {
    label: "EvolvingNarrativeTracker",
    scope: "consensus shift tracking",
    phaseId: "dialecticalSynthesisEngine",
  },
  internalConsistencyCritic: {
    label: "InternalConsistencyCritic",
    scope: "claim-to-evidence enforcement",
    phaseId: "recursiveSelfImprovementLoop",
  },
  claimVerifier: {
    label: "ClaimVerifier",
    scope: "claim-by-claim evidence support and hallucination filtering",
    phaseId: "recursiveSelfImprovementLoop",
  },
  citationVerifier: {
    label: "CitationVerifier",
    scope: "citation integrity and excerpt alignment",
    phaseId: "recursiveSelfImprovementLoop",
  },
  contradictionVerifier: {
  ...
  uncertaintyVerifier: {
    ...
    scope: "confidence calibration and residual uncertainty",
    phaseId: "recursiveSelfImprovementLoop",
  },
  taskFocusVerifier: {
  ...
  coverageAuditor: {
    ...
    scope: "hypothesis coverage scoring",
    phaseId: "recursiveSelfImprovementLoop",
  },
  userGoalAlignmentCritic: {
    label: "UserGoalAlignmentCritic",
    scope: "goal and output-mode alignment",
    phaseId: "recursiveSelfImprovementLoop",
  },
  decisionIntelligenceLayer: {
    label: "Decision Intelligence Layer",
  ...
  adaptiveDeliveryHub: {
    label: "Adaptive Delivery Hub",
    scope: "streaming, export packaging, delivery",
    phaseId: "adaptiveDeliveryHub",
  },
  activeSafetyAndEthics: {
    label: "Active Safety & Ethics",
    scope: "cross-cutting safety and ethics",
    phaseId: "activeSafetyAndEthics",
  },
  causalRiskAnalyzer: {
    label: "CausalRiskAnalyzer",
    scope: "causal misuse chain analysis",
    phaseId: "activeSafetyAndEthics",
  },
});
...
const hydrateSubagentDescriptors = (descriptors = []) =>
  (Array.isArray(descriptors) ? descriptors : [])
    .map((descriptor) => {
      const spec = SUBAGENT_SPECS[descriptor?.id];
      if (!spec) return descriptor;
      return {
        ...descriptor,
        label: descriptor.label || spec.label,
        scope: descriptor.scope || spec.scope,
        phaseId: descriptor.phaseId || spec.phaseId,
        equipment: uniqueList([
          ...(Array.isArray(spec.equipment) ? spec.equipment : []),
          ...(Array.isArray(descriptor.equipment) ? descriptor.equipment : []),
        ]).slice(0, 6),
      };
    })
    .filter(Boolean);
...
const buildSearchQueries = (query, options = {}) => {
  ...
  const push = (...values) =>
    variants.push(...values.map(normalizeText).filter(Boolean));

  if (signals.currentIntent) push(`${base} latest`, `${base} official source`);
  if (signals.docsIntent)
    push(`${base} official documentation`, `${base} reference`);
  if (signals.researchIntent)
    push(`${base} peer reviewed research`, `${base} evidence`);
  ...
  if (!Object.values(signals).some(Boolean))
    push(`${base} overview`, `${base} official source`);
...
const detectDepthPreference = (query, explicit = "") => {
  ...
  if (
    /\b(quick|brief|fast|speed|high level|high-level|tl;dr)\b/.test(normalized)
  )
    return "speed";
  if (
    /\b(deep|deeper|thorough|exhaustive|comprehensive|detailed|deep dive)\b/.test(
      normalized,
    )
  )
    return "deep";
...
const detectResearchDomain = (query) => {
  ...
  return match
    ...
    : { ...DOMAIN_FALLBACK, confidence: 0.48 };
};
...
const detectResearchOutputMode = (query, forcedOutputMode = "") => {
  ...
  if (forced) {
    const match = OUTPUT_MODE_RULES.find(
  ...
  return match
    ...
    : { ...OUTPUT_MODE_FALLBACK, confidence: 0.52 };
};
...
const buildScholarlyProviderBias = (domain, scope) => {
  ...
  if (domain?.id === "cs_ml") providers.push("papers_with_code");
  if (domain?.id === "social_science") providers.push("jstor");
  if (domain?.id === "biomedical" || scope?.id === "replication_study")
    providers.push("ieee_xplore");
  return uniqueList(providers).slice(0, 5);
};

const buildQueryVersionLog = (
  query,
  domain,
  scope,
  outputMode,
  ...
  return [
    ...
    {
      version: "v4",
      query: `${normalized} ${depthLabel}`.trim(),
...
const buildResearchQueryMatrix = (query, options = {}) => {
  ...
  const outputMode =
    options.outputMode ||
    detectResearchOutputMode(normalized, options.forcedOutputMode);
  ...
  return {
    ...
    scholarlyDiscoveryLanes: buildScholarlyDiscoveryLanes(
      normalized,
      domain,
      scope,
      outputMode,
    ),
    scholarlyProviderBias: buildScholarlyProviderBias(domain, scope),
    scholarlyHarvestActive: shouldActivateScholarlyHarvest(
      normalized,
      domain,
      scope,
      outputMode,
    ),
    versions: buildQueryVersionLog(
      normalized,
      domain,
      scope,
      outputMode,
...
const inferSessionContinuity = (query, memoryContext = {}) => {
  ...
  const ranked = (
    Array.isArray(memoryContext.episodes) ? memoryContext.episodes : []
  )
    .map((episode) => ({
      episode,
      overlap: computeOverlapScore(
        queryTerms,
        [episode.query, episode.title, episode.summary].join(" "),
      ),
    }))
  ...
  return {
    ...
    reason:
      "Prior research context was linked into the Cognitive Command Layer.",
...
const buildSafetyChecks = (query, domain) => {
  ...
  const checks = [
    ...
    },
    {
      ...
      active: ["biomedical", "social_science", "cs_ml"].includes(domain.id),
    },
  ];
...
const buildDag = (plan) => {
  ...
  return {
    nodes: filteredNodes,
    edges: filteredNodes.flatMap((node) =>
  ...
  [
    {
      ...
      constraint:
        "All narrative claims must map to at least one Core or Supporting source.",
    ...
    },
    {
      ...
      constraint:
        "Counter-hypotheses must be addressed explicitly when they exist.",
      enforcedAt: "coverageAuditor",
    },
    {
      id: "stay_on_primary_question",
      constraint: `Every major section must directly advance the primary question: ${plan.taskFocus?.primaryQuestion || plan.query}.`,
      enforcedAt: "taskFocusVerifier",
    },
    {
      id: "avoid_scope_drift",
      constraint:
        "If a section cannot be tied to the query anchors or evidence, remove it instead of broadening the scope.",
      enforcedAt: "narrativeArchitect",
    },
  ].map((item) => ({
    ...item,
    severity: [
      "core_or_supporting_citations_only",
      "stay_on_primary_question",
    ].includes(item.id)
      ? "critical"
      : "strong",
  }));
...
const buildSubagents = (plan) => {
  ...
  const counterHypothesisCount = (plan.queryMatrix.counterHypotheses || [])
    .length;
  ...
  if (hasQuery) {
    descriptors.push(
      {
        id: "devilsAdvocateDecomposer",
        label: SUBAGENT_SPECS.devilsAdvocateDecomposer.label,
        scope: SUBAGENT_SPECS.devilsAdvocateDecomposer.scope,
        count: 1,
        detail: `${counterHypothesisCount || 1} counter-hypothesis lane${counterHypothesisCount === 1 ? "" : "s"}`,
      },
      ...(plan.scholarlyHarvest?.active
        ? [
            {
              id: "scholarlySourceHarvester",
              label: SUBAGENT_SPECS.scholarlySourceHarvester.label,
              scope: SUBAGENT_SPECS.scholarlySourceHarvester.scope,
              count: 1,
        ...
        : []),
      {
        id: "forwardCitationTracer",
        label: SUBAGENT_SPECS.forwardCitationTracer.label,
        scope: SUBAGENT_SPECS.forwardCitationTracer.scope,
        count: plan.depthPreference === "deep" ? 2 : 1,
        detail: `depth ${plan.depthPreference === "deep" ? 2 : 1} citation tracing`,
      },
      {
        id: "authorNetworkMapper",
        label: SUBAGENT_SPECS.authorNetworkMapper.label,
        scope: SUBAGENT_SPECS.authorNetworkMapper.scope,
        count: 1,
        detail: "co-authorship watch",
      },
      {
        id: "temporalTrendAnalyzer",
        label: SUBAGENT_SPECS.temporalTrendAnalyzer.label,
        scope: SUBAGENT_SPECS.temporalTrendAnalyzer.scope,
        count: 1,
        detail: `${plan.domain.recencyHalfLifeYears}-year recency horizon`,
      },
      {
        id: "temporalRelevanceDecay",
        label: SUBAGENT_SPECS.temporalRelevanceDecay.label,
        scope: SUBAGENT_SPECS.temporalRelevanceDecay.scope,
        count: 1,
        detail: `${plan.domain.recencyHalfLifeYears}-year half-life`,
      },
      {
        id: "retractedPaperGuard",
        label: SUBAGENT_SPECS.retractedPaperGuard.label,
        scope: SUBAGENT_SPECS.retractedPaperGuard.scope,
        count: 1,
        detail: "quarantine and audit log",
      },
      {
        id: "sampleSizeFilter",
        label: SUBAGENT_SPECS.sampleSizeFilter.label,
        scope: SUBAGENT_SPECS.sampleSizeFilter.scope,
        count: 1,
        detail: "confidence weighting",
      },
      {
        id: "thesisAgent",
        label: SUBAGENT_SPECS.thesisAgent.label,
        scope: SUBAGENT_SPECS.thesisAgent.scope,
        count: 1,
        detail: "dominant-view argument",
      },
      {
        id: "antithesisAgent",
        label: SUBAGENT_SPECS.antithesisAgent.label,
        scope: SUBAGENT_SPECS.antithesisAgent.scope,
        count: 1,
        detail: "counter-evidence argument",
      },
      {
        id: "synthesisMediator",
        label: SUBAGENT_SPECS.synthesisMediator.label,
        scope: SUBAGENT_SPECS.synthesisMediator.scope,
        count: 1,
        detail: "uncertainty-aware reconciliation",
      },
      {
        id: "narrativeArchitect",
        label: SUBAGENT_SPECS.narrativeArchitect.label,
        scope: SUBAGENT_SPECS.narrativeArchitect.scope,
        count: 1,
        detail: plan.outputMode.label,
      },
      {
        id: "quantitativeSynthesizer",
        label: SUBAGENT_SPECS.quantitativeSynthesizer.label,
        scope: SUBAGENT_SPECS.quantitativeSynthesizer.scope,
        count: 1,
        detail: "lightweight meta-analysis",
      },
      {
        id: "evidencePyramidBuilder",
        label: SUBAGENT_SPECS.evidencePyramidBuilder.label,
        scope: SUBAGENT_SPECS.evidencePyramidBuilder.scope,
        count: 1,
        detail: "evidence-type weighting",
      },
      {
        id: "evolvingNarrativeTracker",
        label: SUBAGENT_SPECS.evolvingNarrativeTracker.label,
        scope: SUBAGENT_SPECS.evolvingNarrativeTracker.scope,
        count: 1,
  ...
  if (hasQuery || attachmentCount) {
    descriptors.push(
      {
        id: "statisticalClaimExtractor",
        label: SUBAGENT_SPECS.statisticalClaimExtractor.label,
        scope: SUBAGENT_SPECS.statisticalClaimExtractor.scope,
        count: 1,
        detail: attachmentCount
          ? `${attachmentCount} attachment${attachmentCount === 1 ? "" : "s"} + fetched evidence`
          : "structured claim schema",
      },
      {
        id: "codeRepoAnalyzer",
        label: SUBAGENT_SPECS.codeRepoAnalyzer.label,
        scope: SUBAGENT_SPECS.codeRepoAnalyzer.scope,
        count: 1,
        detail: hasQuery
      ...
      {
        id: "supplementaryMaterialParser",
        label: SUBAGENT_SPECS.supplementaryMaterialParser.label,
        scope: SUBAGENT_SPECS.supplementaryMaterialParser.scope,
        count: 1,
        detail: "supplementary appendix sweep",
      },
      {
        id: "conceptEntityLinker",
        label: SUBAGENT_SPECS.conceptEntityLinker.label,
        scope: SUBAGENT_SPECS.conceptEntityLinker.scope,
        count: 1,
        detail: `${plan.domain.taxonomy} + knowledge graph`,
      },
      {
        id: "statisticalVerifier",
        label: SUBAGENT_SPECS.statisticalVerifier.label,
        scope: SUBAGENT_SPECS.statisticalVerifier.scope,
        count: 1,
  ...
  descriptors.push(
    {
      id: "internalConsistencyCritic",
      label: SUBAGENT_SPECS.internalConsistencyCritic.label,
      scope: SUBAGENT_SPECS.internalConsistencyCritic.scope,
      count: 1,
      detail: `${plan.refinementBudget} cycle budget`,
    },
    {
      id: "claimVerifier",
      label: SUBAGENT_SPECS.claimVerifier.label,
      scope: SUBAGENT_SPECS.claimVerifier.scope,
      count: 1,
      detail: "claim support gate",
    },
    {
      id: "citationVerifier",
      label: SUBAGENT_SPECS.citationVerifier.label,
      scope: SUBAGENT_SPECS.citationVerifier.scope,
      count: 1,
      detail: "citation integrity gate",
    },
    {
      id: "contradictionVerifier",
      label: SUBAGENT_SPECS.contradictionVerifier.label,
      scope: SUBAGENT_SPECS.contradictionVerifier.scope,
      count: 1,
      detail: "counterevidence gate",
    },
    {
      id: "uncertaintyVerifier",
      label: SUBAGENT_SPECS.uncertaintyVerifier.label,
      scope: SUBAGENT_SPECS.uncertaintyVerifier.scope,
      count: 1,
      detail: "confidence calibration gate",
    },
    {
      id: "taskFocusVerifier",
      label: SUBAGENT_SPECS.taskFocusVerifier.label,
      scope: SUBAGENT_SPECS.taskFocusVerifier.scope,
      count: 1,
      detail: `${Math.max(1, plan.taskFocus?.anchorTerms?.length || 0)} focus anchor${Math.max(1, plan.taskFocus?.anchorTerms?.length || 0) === 1 ? "" : "s"}`,
    },
    {
      id: "coverageAuditor",
      label: SUBAGENT_SPECS.coverageAuditor.label,
      scope: SUBAGENT_SPECS.coverageAuditor.scope,
      count: 1,
      detail: plan.scope.label,
    },
    {
      id: "userGoalAlignmentCritic",
      label: SUBAGENT_SPECS.userGoalAlignmentCritic.label,
      scope: SUBAGENT_SPECS.userGoalAlignmentCritic.scope,
      count: 1,
      detail: plan.outputMode.label,
    },
    ...([
      ...
      ? [
          {
            id: "decisionIntelligenceLayer",
            label: SUBAGENT_SPECS.decisionIntelligenceLayer.label,
            scope: SUBAGENT_SPECS.decisionIntelligenceLayer.scope,
            count: 1,
            detail: plan.outputMode.label,
          },
        ]
    ...
    {
      id: "adaptiveDeliveryHub",
      label: SUBAGENT_SPECS.adaptiveDeliveryHub.label,
      scope: SUBAGENT_SPECS.adaptiveDeliveryHub.scope,
      count: 1,
      detail: `${searchLaneCount} streaming lane${searchLaneCount === 1 ? "" : "s"}`,
    },
  );

  return hydrateSubagentDescriptors(descriptors).map((descriptor) => ({
    ...descriptor,
...
const buildConvergenceMetrics = ({
  ...
  const exhaustedBudget =
    resolvedIterations >=
    Math.max(1, Number(refinementBudget) || DEFAULT_REFINEMENT_BUDGET);
...
const buildTribunalSummary = ({
  ...
  return {
    refinement_budget: Math.max(
      1,
      Number(refinementBudget) || DEFAULT_REFINEMENT_BUDGET,
    ),
...
const topologicalBatches = (dag = {}) => {
  const nodes = new Map(
    (Array.isArray(dag.nodes) ? dag.nodes : []).map((node) => [node.id, node]),
  );
...
const compileResearchPlan = (options = {}) => {
  ...
  const refinementBudget = Math.max(
    1,
    Number(options.refinementBudget) || DEFAULT_REFINEMENT_BUDGET,
  );
  ...
  const basePlan = {
    ...
    promptPatches: Array.isArray(memoryContext.promptPatches)
  ...
  const inlineConstraints = buildInlineConstraints({ ...basePlan, taskFocus });
  const plan = {
    ...basePlan,
    taskFocus,
    searchQueries,
