...
const SUBAGENT_SPECS = attachSubagentEquipment({
    ...
    scholarlySourceHarvester: { label: "ScholarlySourceHarvester", scope: "Google Scholar-style discovery, institutional repository, and university-domain harvesting via permitted search paths", phaseId: "intelligentCrawlerMesh" },
...
const shouldActivateScholarlyHarvest = (query, domain, scope, outputMode) => {
    ...
    if (["state_of_the_field", "controversy_map", "gap_analysis", "replication_crisis_report", "foundational_review"].includes(outputMode?.id)) return true;
    return false;
};

const buildScholarlyDiscoveryLanes = (query, domain, scope, outputMode) => {
    const normalized = normalizeText(query);
    if (!normalized || !shouldActivateScholarlyHarvest(normalized, domain, scope, outputMode)) return [];
    ...
    if (domain?.id === "biomedical") lanes.push(`${normalized} site:pubmed.ncbi.nlm.nih.gov`, `${normalized} site:nih.gov`);
    if (domain?.id === "cs_ml") lanes.push(`${normalized} site:arxiv.org`, `${normalized} site:paperswithcode.com`);
    if (domain?.id === "social_science") lanes.push(`${normalized} site:jstor.org`, `${normalized} site:ssrn.com`);
    if (scope?.id === "replication_study") lanes.push(`${normalized} site:osf.io`);
...
const buildScholarlyProviderBias = (domain, scope) => {
    ...
    if (domain?.id === "biomedical" || scope?.id === "replication_study") providers.push("ieee_xplore");
...
const buildResearchQueryMatrix = (query, options = {}) => {
    ...
    return {
        ...
        scholarlyDiscoveryLanes: buildScholarlyDiscoveryLanes(normalized, domain, scope, outputMode),
        scholarlyProviderBias: buildScholarlyProviderBias(domain, scope),
        scholarlyHarvestActive: shouldActivateScholarlyHarvest(normalized, domain, scope, outputMode),
...
const buildSubagents = (plan) => {
    ...
    if (hasQuery) {
        descriptors.push(
            ...
            ...(plan.scholarlyHarvest?.active
                ? [{ id: "scholarlySourceHarvester", label: SUBAGENT_SPECS.scholarlySourceHarvester.label, scope: SUBAGENT_SPECS.scholarlySourceHarvester.scope, count: 1, detail: `${plan.scholarlyHarvest.lanes.length} scholarly lane${plan.scholarlyHarvest.lanes.length === 1 ? "" : "s"}; academic index + .edu sweep` }]
                : []),
    ...
    return hydrateSubagentDescriptors(descriptors);
};
