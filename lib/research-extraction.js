...
const CI_RE =
  /\b(?:95%\s*)?CI\s*[:=]?\s*\[?\s*(-?\d+(?:\.\d+)?)\s*[,–-]\s*(-?\d+(?:\.\d+)?)\s*\]?/gi;
...
const EFFECT_SIZE_RE =
  /\b(Cohen'?s d|effect size|odds ratio|hazard ratio|risk ratio|relative risk|AUC|accuracy|F1|BLEU|ROUGE|HR|OR|RR)\s*[:=]?\s*(-?\d+(?:\.\d+)?)\b/gi;
const FUNDING_RE =
  /\b(funded by|funding|sponsored by|supported by|grant|conflict of interest|competing interests?)\b/i;
const PREDATORY_RE =
...
const extractLinks = (text = "") =>
  [...String(text || "").matchAll(/\bhttps?:\/\/[^\s)<>"']+/gi)].map((match) =>
    match[0].replace(/[),.;:!?]+$/, ""),
  );

const extractSupplementaryLinks = (text = "") =>
  unique(
    extractLinks(text).filter((url) =>
      /\b(supplement|appendix|appendices|dataset|artifact|materials|zenodo|figshare|osf)\b/i.test(
        url,
      ),
    ),
  ).slice(0, 8);
...
const inferStudyDesign = (text = "", source = {}) => {
  const corpus =
    `${source?.title || ""}\n${source?.description || ""}\n${text}`.toLowerCase();
  if (/\bmeta-analysis|systematic review\b/.test(corpus))
    return "meta_analysis";
  if (/\brandomized|rct|controlled trial\b/.test(corpus)) return "rct";
  if (/\bcohort|observational|case control|longitudinal|survey\b/.test(corpus))
    return "observational";
  if (/\bcase study|case report\b/.test(corpus)) return "case_study";
  if (
    /\bbenchmark|ablation|leaderboard\b/.test(corpus) ||
...
const extractExcerpt = (text = "", index = 0, length = 100, radius = 140) => {
  ...
  const end = Math.min(
    String(text || "").length,
    Number(index) + Number(length || 0) + radius,
  );
...
const detectEffectMetricFamily = (metric = "") => {
  ...
  if (/\b(cohen'?s d|effect size)\b/.test(normalized))
    return "standardized_mean_difference";
  if (
    /\b(odds ratio|hazard ratio|risk ratio|relative risk|or|hr|rr)\b/.test(
      normalized,
    )
  )
    return "ratio";
  if (/\b(auc|accuracy|f1|bleu|rouge)\b/.test(normalized))
...
const normalizeEffectMetric = (metric = "", value) => {
  ...
  return {
    ...
    normalizedValue: Number.isFinite(normalizedValue)
      ? Number(normalizedValue.toFixed(4))
      : null,
    effectDirection,
  };
};
...
const extractConceptsFromText = (
  text = "",
  query = "",
  source = {},
  plan = {},
) => {
  const ontologyTerms = Array.isArray(plan?.domain?.ontologyTerms)
    ? plan.domain.ontologyTerms
    : [];
  const queryTerms = extractQueryTerms(
    `${query} ${source?.title || ""} ${ontologyTerms.join(" ")}`,
    18,
  );
  ...
  const seeded = unique([
    ...queryTerms,
    ...textTerms,
    ...ontologyTerms.map((term) => normalizeText(term).toLowerCase()),
  ]).slice(0, 18);
...
const scoreRepositoryReadme = (content = "") => {
  ...
  if (/reproduce|reproducibility|replicate|training|evaluation/.test(text))
    score += 1;
...
const buildRepositoryRubric = ({
  repoMeta = {},
  readmeText = "",
  topLevelNames = [],
} = {}) => {
  const topLevel = new Set(
    (Array.isArray(topLevelNames) ? topLevelNames : []).map((item) =>
  ...
  const environment = environmentSignals.length
    ? 1
    : manifestFiles.length
      ? 0.52
      : 0;
  const artifacts = Math.min(
    1,
    (artifactSignals.length ? 0.7 : 0) + (repoMeta?.license?.spdx_id ? 0.3 : 0),
  );
  const reproducibilityScore = Number(
    clamp(
      documentation + dependencies + automation + environment + artifacts,
      0,
      5,
    ).toFixed(2),
  );
  ...
  if (!ciSignals.length && !testSignals.length)
    blockedBy.push("missing_ci_or_tests");
...
const analyzeGitHubRepository = async (repoUrl = "") => {
  ...
  const [repoMeta, readmePayload, contentsPayload] = await Promise.all([
    readJson(`${GITHUB_API_BASE_URL}/repos/${parsed.owner}/${parsed.repo}`, {
      headers,
    }).catch(() => null),
    readJson(
      `${GITHUB_API_BASE_URL}/repos/${parsed.owner}/${parsed.repo}/readme`,
      { headers },
    ).catch(() => null),
    readJson(
      `${GITHUB_API_BASE_URL}/repos/${parsed.owner}/${parsed.repo}/contents`,
      { headers },
    ).catch(() => null),
  ]);
  ...
  return {
    ...
    notes: normalizeText(
      [
        rubric.manifestFiles.length
          ? `manifests: ${rubric.manifestFiles.join(", ")}`
          : "no dependency manifest",
        rubric.ciSignals.length
          ? "ci/test automation present"
          : "ci/test automation unclear",
        rubric.environmentSignals.length
          ? `env: ${rubric.environmentSignals.join(", ")}`
  ...
  try {
    ...
    return {
      ...
      rubric: {
        ...
        dependencies:
          /\b(requirements|dependency|dependencies|install)\b/i.test(content)
            ? 0.5
            : 0,
        automation: /\b(test|ci|workflow|github actions|docker)\b/i.test(
          content,
        )
          ? 0.5
          : 0,
        environment: /\b(docker|conda|venv|setup)\b/i.test(content) ? 0.5 : 0,
        artifacts: /\b(dataset|data|citation|license)\b/i.test(content)
          ? 0.5
          : 0,
      },
      blockedBy,
...
const analyzeRepositories = async (repoUrls = []) => {
  ...
  return runConcurrent(uniqueRepos, 3, async (repoUrl) => {
    const githubAnalysis = await analyzeGitHubRepository(repoUrl).catch(
      () => null,
    );
...
const buildSampleSizeProfile = (claims = [], plan = {}) => {
  const sampleClaims = claims.filter(
    (claim) => claim.type === "sample_size" && Number.isFinite(claim.value),
  );
  const values = sampleClaims.map((claim) => claim.value);
  const minimumRecommended = getDomainSampleThreshold(plan?.domain);
  const bySource = uniqueBy(
    sampleClaims,
    (claim) => claim.sourceUrl || claim.sourceTitle,
  )
    .map((claim) => ({
  ...
  return {
    ...
    underpoweredCount: sampleClaims.filter((claim) => claim.sampleSizeFlag)
      .length,
...
const buildPValueSummary = (claims = []) => {
  const pValues = claims
    .filter(
      (claim) => claim.type === "p_value" && Number.isFinite(claim.pValue),
    )
    .map((claim) => claim.pValue);
  return {
    count: pValues.length,
    significantCount: pValues.filter((value) => value < 0.05).length,
    nearThresholdCount: pValues.filter(
      (value) => value >= 0.045 && value <= 0.055,
    ).length,
...
const buildContradictionArtifacts = (entries = [], claims = []) => {
  ...
  const effectClaims = claims.filter(
    (claim) =>
      claim.type === "effect_size" && Number.isFinite(claim.normalizedValue),
  );
  const metricConflicts = [];
  const families = unique(effectClaims.map((claim) => claim.metricFamily));
  for (const family of families) {
    const familyClaims = effectClaims.filter(
      (claim) => claim.metricFamily === family,
    );
    const positive = familyClaims.filter(
      (claim) => claim.effectDirection === "positive",
    );
    const negative = familyClaims.filter(
      (claim) => claim.effectDirection === "negative",
    );
    if (positive.length && negative.length) {
      metricConflicts.push({
        metricFamily: family,
        positiveSources: unique(
          positive.map((claim) => claim.sourceTitle || claim.sourceUrl),
        ).slice(0, 8),
        negativeSources: unique(
          negative.map((claim) => claim.sourceTitle || claim.sourceUrl),
        ).slice(0, 8),
  ...
  const supportiveCount = stanceMap.filter(
    (item) => item.stance === "supportive",
  ).length;
  const contraryCount = stanceMap.filter(
    (item) => item.stance === "contrary",
  ).length;
  const mixedCount = stanceMap.filter((item) => item.stance === "mixed").length;
  const contradictionCount =
    metricConflicts.length +
    (supportiveCount && contraryCount ? 1 : 0) +
    mixedCount;
  const falseConsensusRisk = Number(
    clamp(
...
const buildMetaAnalysis = (claims = [], plan = {}) => {
  ...
  if (!effectClaims.length) {
    return {
      ...
      assumption_conflicts: pValueSummary.nearThresholdCount
        ? ["Borderline p-values without normalized effect sizes."]
        : [],
      family_summaries: [],
      effect_size_count: 0,
  ...
  const families = unique(effectClaims.map((claim) => claim.metricFamily));
  const familySummaries = families
    .map((family) => {
      const familyClaims = effectClaims.filter(
        (claim) => claim.metricFamily === family,
      );
      const weighted = familyClaims
        .map((claim) => ({
          value: claim.normalizedValue,
          weight: claim.sampleSize
            ? Math.sqrt(Math.max(1, claim.sampleSize))
            : 1,
        }))
        .filter(
      ...
      const weightSum =
        weighted.reduce((sum, item) => sum + item.weight, 0) || 1;
      const combined =
        weighted.reduce((sum, item) => sum + item.value * item.weight, 0) /
        weightSum;
      const qStatistic = weighted.reduce(
        (sum, item) => sum + item.weight * (item.value - combined) ** 2,
        0,
      );
      const degreesFreedom = Math.max(0, weighted.length - 1);
      const iSquared =
        qStatistic > 0 && degreesFreedom > 0
  ...
  if (familySummaries.length > 1)
    assumptionConflicts.push(
      "Multiple effect metric families required separate aggregation.",
    );
  if ((primary?.i_squared || 0) >= 65)
    assumptionConflicts.push(
      "High heterogeneity across normalized effect sizes.",
    );
  if (sampleProfile.underpoweredCount > 0)
    assumptionConflicts.push(
      `${sampleProfile.underpoweredCount} source(s) fall below the field sample-size threshold.`,
    );
  if (pValueSummary.nearThresholdCount > 0)
    assumptionConflicts.push(
      "Borderline p-values increase statistical fragility.",
    );
...
const verifyStatisticsWithSandbox = (
  claims = [],
  plan = {},
  sampleProfile = null,
  contradictions = null,
  ...
  return {
    ...
    notes: [
      ...
      sandbox.result.sampleSizes
        ? `samples:${sandbox.result.sampleSizes.count}`
        : "",
    ].filter(Boolean),
  };
};
...
const buildSafetySignals = (
  ...
  for (const entry of entries) {
    ...
    const excerpt = normalizeText(
      String(entry?.content || source?.description || "").slice(0, 320),
    );
    const sourceSafety = buildSourceSafetySignals(source);

    if (
      FUNDING_RE.test(text) ||
      sourceSafety.summary.includes("funding_conflict")
    ) {
    ...
    if (
      ...
      sourceSafety.summary.includes("predatory_candidate")
    ) {
    ...
    const sourceClaims = claims.filter(
      (claim) =>
        (claim.sourceUrl || claim.sourceTitle) === (source.url || source.title),
    );
  ...
  const riskPropagationScore = Number(
    clamp(
...
const fetchTieredEvidence = async (sources = [], query = "") => {
  const fullSources = sources
    .filter((source) => source.fetchMode === "full")
    .slice(0, MAX_FULL_FETCH);
  const lightSources = sources
    .filter((source) => source.fetchMode === "light")
    .slice(0, MAX_LIGHT_FETCH);

  const fetchedFull = await runConcurrent(
    fullSources,
    FETCH_CONCURRENCY,
    async (source) => {
      try {
...
const extractResearchArtifacts = async ({
  query,
  plan,
  evidenceEntries = [],
}) => {
  const claims = evidenceEntries.flatMap((entry) =>
    extractStatisticalClaims(entry, plan),
  );
  const repoUrls = unique(
    evidenceEntries.flatMap((entry) => [
      ...(Array.isArray(entry?.source?.repoUrls) ? entry.source.repoUrls : []),
      ...extractRepoLinks(entry?.content || ""),
    ]),
  ).slice(0, 8);
  const supplementaryLinks = unique(
    evidenceEntries.flatMap((entry) =>
      extractSupplementaryLinks(entry?.content || ""),
    ),
  ).slice(0, 8);
  ...
  const concepts = uniqueBy(
    evidenceEntries.flatMap((entry) =>
      extractConceptsFromText(entry?.content || "", query, entry?.source, plan),
    ),
    (item) => item.name,
  ).slice(0, 32);
  const sampleProfile = buildSampleSizeProfile(claims, plan);
  ...
  const statisticalVerification = verifyStatisticsWithSandbox(
    claims,
    plan,
    sampleProfile,
    contradictions,
    ...
    codeArtifacts: {
      ...
      reproducibilityAverage: repositories.length
        ? Number(
            (
              repositories.reduce(
                (sum, repo) => sum + Number(repo.reproducibilityScore || 0),
                0,
              ) / repositories.length
            ).toFixed(2),
          )
        : 0,
      automationReadyCount: repositories.filter(
        (repo) => Array.isArray(repo.ciSignals) && repo.ciSignals.length,
      ).length,
      dependencyReadyCount: repositories.filter(
        (repo) =>
          Array.isArray(repo.manifestFiles) && repo.manifestFiles.length,
      ).length,
    },
    query,
