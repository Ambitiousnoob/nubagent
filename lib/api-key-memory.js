...
const DEFAULT_CEREBRAS_MEMORY_MODEL =
  process.env.CEREBRAS_MEMORY_MODEL || "qwen-3-235b-a22b-instruct-2507";
...
const scoreEntry = (queryTerms, entryText, updatedAt) => {
  ...
  const ageHours = Math.max(
    0,
    (Date.now() - new Date(updatedAt || Date.now()).getTime()) / 36e5,
  );
...
const buildFallbackResults = (
  ...
  const lexicalMatches = dedupeRows(
    [...rows].filter((row) => row.score > 0).sort(byScoreThenUpdatedAtDesc),
    normalizedLimit,
  );
...
const rerankWithCerebras = async (
  query,
  candidates = [],
  limit = MEMORY_RESULT_LIMIT,
) => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Cerebras rerank failed (${response.status}): ${responseText.slice(0, 240)}`,
      );
    }
...
const saveApiKeyMemoryEntries = async (
  apiKeyHash,
  messages = [],
  assistantReply = "",
) => {
...
const searchApiKeyMemoryDetailed = async (
  ...
  const db = getPool();
  const selectedLimit = clampInteger(limit, 1, 10, MEMORY_RESULT_LIMIT);
  const selectedCandidateLimit = clampInteger(
    candidateLimit,
    selectedLimit,
    200,
  ...
  if (rerankCandidates.length) {
    try {
      const reranked = await rerankWithCerebras(
        normalizedQuery,
        rerankCandidates,
        selectedLimit,
      );
...
const formatApiKeyMemoryContext = (entries = []) => {
  ...
  items.forEach((entry, index) => {
    const label =
      entry.role === "assistant" ? "Assistant memory" : "User memory";
