...
const USER_SETTINGS_TABLE = "messenger_user_settings";
...
export function normalizePostgresConnectionString(connectionString) {
  ...
  if (normalizeBooleanLike(parsedUrl.searchParams.get("uselibpqcompat"))) {
    return normalizedConnectionString;
  }
...
async function ensureSchema(pool) {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS ${USER_SETTINGS_TABLE} (
      sender_psid TEXT PRIMARY KEY,
      thinking_level TEXT CHECK (thinking_level IN ('off', 'minimal', 'low', 'medium', 'high')),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    )
  `);
...
export function createConversationStore({
  ...
  return {
    ...
    async getUserSettings(senderId) {
      if (!senderId) {
        return {
          thinkingLevel: null,
        };
      ...
      const result = await activePool.query(
        `
          SELECT thinking_level, updated_at
      ...
      return {
        thinkingLevel: result.rows[0].thinking_level || null,
        updatedAt: result.rows[0].updated_at,
      };
    },

    async setThinkingLevel(senderId, thinkingLevel) {
      const normalizedThinkingLevel = normalizeText(thinkingLevel).toLowerCase();

      if (!senderId || !THINKING_LEVEL_VALUES.has(normalizedThinkingLevel)) {
        throw new Error(
          "senderId and a valid thinking level are required for user settings.",
        );
      }
      ...
      const result = await activePool.query(
        `
          INSERT INTO ${USER_SETTINGS_TABLE} (sender_psid, thinking_level)
          VALUES ($1, $2)
          ON CONFLICT (sender_psid) DO UPDATE
          SET thinking_level = EXCLUDED.thinking_level,
              updated_at = NOW()
          RETURNING thinking_level, updated_at
        `,
        [senderId, normalizedThinkingLevel],
      );

      return {
        thinkingLevel: result.rows[0].thinking_level,
        updatedAt: result.rows[0].updated_at,
      };
    },

    async clearThinkingLevel(senderId) {
      if (!senderId) {
        throw new Error("senderId is required to clear the thinking level.");
      }
      ...
      const result = await activePool.query(
        `
          DELETE FROM ${USER_SETTINGS_TABLE}
          WHERE sender_psid = $1
        `,
        [senderId],
      );
