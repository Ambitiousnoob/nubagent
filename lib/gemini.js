...
const TOOL_CAPABILITY_HINT_PREFIX = "Available tools in this request:";
...
const MAPS_SELF_LOCATION_PATTERN =
  /\b(?:where am i|where i am|my location|current location|what city am i in|what area am i in)\b/i;
const MAPS_ADDRESS_LOOKUP_PATTERN =
...
function looksLikeMapsPrompt(prompt) {
  ...
  return (
    ...
    MAPS_SELF_LOCATION_PATTERN.test(normalized)
  );
}
...
export function getPromptCapabilities(prompt, config) {
  ...
  return {
    ...
    wantsAddressLookup: MAPS_ADDRESS_LOOKUP_PATTERN.test(normalizedPrompt),
...
function hasGoogleMapsTool(tools) {
  return (
    Array.isArray(tools) && tools.some((tool) => Boolean(tool?.googleMaps))
  );
}
...
function buildToolConfig({ config, tools }) {
  if (!hasGoogleMapsTool(tools) || !config.geminiGoogleMapsLocation) {
    return undefined;
  }
...
function buildAddressLookupPromptHint(config, promptCapabilities) {
  if (
    !promptCapabilities?.wantsAddressLookup ||
    !config?.geminiGoogleMapsLocation
  ) {
    return "";
  }

  const { latitude, longitude } = config.geminiGoogleMapsLocation;

  if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) {
    return "";
  }

  return `Address lookup instruction:\nUse Google Maps grounding with the current location latitude ${latitude} and longitude ${longitude} to identify the most precise nearby address or named place available. Prefer a street address when Maps can resolve one. If Maps cannot provide an exact address, return the nearest labeled address or landmark and clearly say it is approximate.`;
}

function buildUserParts({ prompt, inlineParts = [], config, promptCapabilities }) {
  const addressLookupHint = buildAddressLookupPromptHint(
    config,
    promptCapabilities,
  );
...
function buildSystemInstruction({
  ...
  if (
    ...
    hints.push(
      "For address or 'where exactly am I' questions, use Google Maps grounding with the current location to identify the most precise nearby address or named place available. Do not stop at only the country or city when more precise Maps data is available.",
    );
  }
