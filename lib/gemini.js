...
export function getPromptCapabilities(prompt, config) {
  return {
    wantsExactReasoning: looksLikeExactReasoningPrompt(prompt),
    wantsUrlContext:
      config.geminiEnableUrlContext && extractUrls(prompt).length > 0,
    wantsMapsGrounding:
      config.geminiEnableGoogleMaps && looksLikeMapsPrompt(prompt),
  };
}

function buildTools({ config, modelName, promptCapabilities }) {
  const tools = [];
  const usesGoogleMaps =
...
function buildToolConfig({ config, modelName, promptCapabilities }) {
  if (
    !promptCapabilities.wantsMapsGrounding ||
    !supportsGoogleMapsGrounding(modelName) ||
...
function buildSystemInstruction({ config, modelName, promptCapabilities }) {
