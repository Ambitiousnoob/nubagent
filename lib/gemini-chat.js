const DEFAULT_MODEL = process.env.GEMINI_CHAT_MODEL || "gemini-3-flash-preview";
const DEFAULT_THINKING_LEVEL = process.env.GEMINI_CHAT_THINKING_LEVEL || "low";
...
const normalizeMessages = (body = {}) => {
  ...
  if (!normalizedMessages.length) {
    throw createError(
      400,
      "Provide a non-empty `message` or `messages` array.",
    );
  }
  ...
  return {
    ...
    system:
      ...
      messages
        ...
        .join("\n\n"),
    temperature: temperature === null ? null : clamp(temperature, 0, 2),
    maxOutputTokens,
