...
describe('Library Store', () => {
  ...
  it('persists edited sessions and the typed library query', () => {
    ...
    const persistedSearchState = JSON.parse(window.localStorage.getItem('nubagent-library-storage'));
  ...
  it('builds root-based session share URLs and parses the shared session id back out', () => {
    const { buildSessionShareUrl, getSharedSessionIdFromLocation } = require('../lib/library');
    ...
    expect(getSharedSessionIdFromLocation({ search: '?session=session-42' })).toBe('session-42');
  });
});
