...
describe("SearchEngine shell handoff", () => {
  it("loads a saved library session passed in from the app shell", async () => {
    ...
    expect(
      (await screen.findAllByText("A research assistant.")).length,
    ).toBeGreaterThan(0);
    expect(
      (await screen.findAllByText("What is NubAgent?")).length,
    ).toBeGreaterThan(0);
  ...
  it("creates a library record for a typed query even if the search request fails", async () => {
    ...
    const onSessionRouteChange = vi.fn();

    render(<SearchEngine onSessionRouteChange={onSessionRouteChange} />);

    const input = screen.getByLabelText("Message NubAgent");
    fireEvent.change(input, {
      target: { value: "Will this save to library?" },
    });
    ...
    expect(onSessionRouteChange).toHaveBeenCalledWith(
      expect.any(String),
  ...
  it("renders compiled research metadata and verifier ownership for a saved research session", async () => {
      ...
      <SearchEngine
        session={{
          ...
          researchMeta: {
            ...
            synthesisWorkers: 3,
            dagSummary:
              "Cognitive Command Layer -> Dialectical Synthesis Engine -> Decision Intelligence Layer -> Adaptive Delivery Hub",
            domain: { label: "CS + ML" },
            scope: { label: "Decision Briefing" },
            outputMode: { label: "Decision Brief" },
            pareto: {
              mode: "deep",
              explanation: "favor completeness over latency",
            },
    ...
    expect(
      screen.getByText(
        "Research Framework v3.1 with compiled DAG orchestration",
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /Linked prior session context into Phase 1 with 67% overlap/i,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /2\/3 cycles; targeted coverage; critics: consistency 0.91, coverage 0.88, alignment 0.94/i,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /stability 0.92; coverage delta 0.03; residual uncertainty 0.18; stop condition budget exhausted/i,
      ),
    ).toBeInTheDocument();
