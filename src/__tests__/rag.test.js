...
describe("rag utilities", () => {
  it("canonicalizes URLs by stripping tracking params and normalizing slashes", () => {
    expect(
      canonicalizeSourceUrl(
        "https://Example.com/path///to/page/?utm_source=test&ref=abc&x=1#section",
      ),
    ).toBe("https://example.com/path/to/page?x=1");
  });

  it("exposes canonical source merging on the company facade", () => {
    expect(Company.rag.mergeSourcesByCanonicalUrl).toBe(
      mergeSourcesByCanonicalUrl,
    );
  });
  ...
  it("prefers authoritative relevant sources over generic blogs", () => {
    const ranked = rerankSourcesForQuery("CRISPR base editing study", [
      ...
      },
      {
        ...
        description:
          "Peer-reviewed study describing programmable base editing outcomes.",
      },
    ]);
  ...
  it("selects diverse fetch candidates and removes canonical duplicates", () => {
    const selected = selectSourcesForFetch(
      "latest ai regulation 2026",
      [
        {
  ...
  it("promotes evidence with stronger content matches while limiting same-domain clustering", () => {
    const ranked = rankEvidenceEntriesForQuery(
      "FDA AI regulation update",
      [
        {
          source: {
            title: "Blog recap",
          ...
          content:
            "This page mostly summarizes general AI news without much detail about the FDA announcement.",
        },
        {
          source: {
          ...
          content:
            "The FDA announced an AI regulation update covering safety review, submission expectations, and deployment oversight.",
        },
        {
          source: {
          ...
          content:
            "This FAQ covers follow-up questions about the FDA AI regulation update and filing process.",
        },
        {
          source: {
            title: "Reuters coverage",
          ...
          content:
            "Reuters reports the FDA AI regulation update with timing, scope, and industry reaction.",
        },
      ],
      {
        maxPerDomain: 1,
    ...
    expect(
      new URL(ranked[0]?.source?.url).hostname.replace(/^www\./, ""),
    ).not.toBe(new URL(ranked[1]?.source?.url).hostname.replace(/^www\./, ""));
  });
});
