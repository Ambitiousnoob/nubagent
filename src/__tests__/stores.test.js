...
describe("Settings Store", () => {
  ...
  it("initializes with default state", () => {
    ...
    expect(state.researchSelectedModel).toBe(
  ...
  it("sets the research subagent model separately from the main model", () => {
    const { setSelectedModel, setResearchSelectedModel } =
    ...
    expect(useSettingsStore.getState().researchSelectedModel).toBe(
...
describe("Library Store", () => {
  ...
  it("updates a saved session and keeps selected session in sync", async () => {
    ...
    expect(useLibraryStore.getState().selectedSession.query).toBe(
      "Updated query",
    );
  });
