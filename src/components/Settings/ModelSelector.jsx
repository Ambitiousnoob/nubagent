...
export function ModelSelector() {
  const { selectedModel, researchSelectedModel, setResearchSelectedModel } =
    useSettingsStore();
  ...
  const models = [
    ...
    },
    {
      ...
      description:
        "Rotate orchestration subagents across the configured research model chain",
      badge: "Adaptive",
    },
    {
  ...
  return (
    <div className="model-selector">
      ...
      <p className="model-selector__description">
        The main NubAgent lane stays on {mainModelLabel}. Only the research
        orchestration subagents use the selector below.
      </p>
        ...
        {models.map((model) => (
          ...
          >
            <div className="model-selector__card-header">
              ...
              {model.badge && (
                <span
                  className={`model-selector__badge model-selector__badge--${model.badge.toLowerCase()}`}
                >
                  {model.badge}
                </span>
              )}
            ...
            <p className="model-selector__card-description">
              {model.description}
            </p>
      ...
      <div className="model-selector__info">
        ...
        <ul>
          <li>
            <strong>Main agent:</strong> NubAgent remains on {mainModelLabel}{" "}
            for the primary chat lane
          </li>
          <li>
            <strong>Nemotron 3 Super 120B:</strong> OpenRouter-backed default
            for synthesis, critics, and verifier subagents
          </li>
          <li>
            <strong>OpenRouter Round Robin:</strong> Uses the backend research
            model chain so subagents can rotate instead of pinning to one model
          </li>
          <li>
            <strong>Gemini 2.5 Flash Lite:</strong> Fast fallback lane for
            lighter research passes
          </li>
          <li>
            <strong>Gemini 2.5 Flash:</strong> Deeper Google fallback when you
