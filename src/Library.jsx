...
import {
  SessionActions,
  BulkActions,
} from "./components/Library/SessionActions.jsx";
...
import { Search, ArrowLeft, Plus, CheckSquare, Square } from "lucide-react";
...
export default function Library({ onBack, onViewSession, onNewSearch }) {
  ...
  const totalSources = sessions.reduce(
    (count, session) => count + (session.sources?.length || 0),
    0,
  );
  const totalAttachments = sessions.reduce(
    (count, session) => count + (session.attachments?.length || 0),
    0,
  );
  ...
  const handleDelete = useCallback(
    (id) => {
      ...
      success(
        "Session Deleted",
        "The session has been removed from your library",
      );
    },
  ...
  const handleBulkDelete = useCallback(() => {
    ...
    success(
      "Sessions Deleted",
      `${selectedSessions.length} sessions have been removed`,
    );
  ...
  const handleBulkExport = useCallback(
    async (format = "markdown") => {
      try {
        ...
        success(
          "Export Ready",
          `${selectedSessions.length} session${selectedSessions.length === 1 ? "" : "s"} exported.`,
        );
      } catch (err) {
    ...
    async (session, options = {}) => {
      try {
        ...
        if (options?.copied) {
          success(
            "Link Copied",
            "A direct link to this saved session is in your clipboard.",
          );
          return;
        }
        ...
        if (promptToCopySessionUrl(shareUrl)) {
          success(
            "Copy Link",
            "Use the dialog to copy your saved session link.",
          );
          return;
        }
  ...
  const handleEdit = useCallback(
    (session, updates = {}) => {
      ...
      const nextQuery =
        updates.query?.trim() || session.query || "Untitled session";
  ...
  const handleExport = useCallback(
    async (session, format) => {
      try {
        const content = await exportSessions(
          format,
          session?.id ? [session.id] : [],
        );
        if (!content || !session?.id)
          throw new Error("Unable to export this session.");
    ...
    <div className="library-page library-page--refined">
      <section className="library-page__hero">
        <div className="library-page__hero-copy">
          ...
          <h1 className="library-page__hero-title">
            Saved research, ready to reopen.
          </h1>
          <p className="library-page__hero-body">
            Reopen strong runs, export what matters, and get back to prior work
            without digging through old transcripts.
          </p>
        </div>
        ...
        <div className="library-page__search-shell">
          <div className="library-page__search-intro">
            ...
            <p className="library-page__search-body">
              Search saved sessions, then narrow the current slice with simple
              filters.
            </p>
          </div>
