...
import { buildAppViewUrl } from "./appRoutes.js";
  ...
  const sessionToSave = {
    ...
    createdAt:
      existingIndex >= 0
        ...
        : getSessionTimestamp(normalizedSession?.createdAt, now),
    updatedAt: now,
  };
...
export function buildSessionShareUrl(
  ...
  return buildAppViewUrl("chat", { sessionId, locationLike });
}
...
export function getSharedSessionIdFromLocation(
  locationLike = typeof window !== "undefined" ? window.location : null,
) {
...
export function searchSessions(searchTerm) {
  ...
  return sessions.filter(
    (session) =>
      ...
      session.attachments?.some((attachment) =>
